...
fn write_default_approved_single_step_execution_fixture(repo: &Path) {
...
fn invalidate_v2_attempt(repo: &Path, task: u32, step: u32, reason: &str) {
    ...
    assert!(
        status_replaced,
        "target attempt status should be invalidated"
    );
    assert!(
        reason_replaced,
...
fn authoritative_state_digest(repo: &Path, state: &Path) -> String {
    let state_path = harness_state_file_path(repo, state);
    if !state_path.exists() {
        return String::from("missing");
    }
    let contents = fs::read(&state_path)
        .unwrap_or_else(|error| panic!("authoritative state should be readable: {error}"));
    sha256_hex(&contents)
}
...
fn republish_authoritative_artifact_from_path(
    ...
    match artifact_prefix {
        "final-review" => retarget_current_record_binding(
            repo,
            state,
        ...
        "release-docs" => retarget_current_record_binding(
            repo,
            state,
        ...
        "browser-qa" => retarget_current_record_binding(
            repo,
            state,
...
fn retarget_current_record_binding(
    repo: &Path,
    state: &Path,
    ...
    let Some(previous_record_id) = root
    ...
    let Some(history) = root.get_mut(history_field).and_then(Value::as_object_mut) else {
        return;
    };
    let Some(mut record) = history.remove(&previous_record_id) else {
        return;
    };
    let new_record_id = format!("{record_id_prefix}-{new_fingerprint}");
    let Some(record_object) = record.as_object_mut() else {
        panic!("current record history entry should remain an object");
    };
    record_object.insert(
        String::from("record_id"),
        Value::from(new_record_id.clone()),
    );
    record_object.insert(
        String::from(fingerprint_field),
        Value::from(new_fingerprint.to_owned()),
    );
...
fn retarget_current_qa_source_test_plan_fingerprint(repo: &Path, state: &Path, fingerprint: &str) {
    ...
    let Some(current_record_id) = root
        .get("current_qa_record_id")
        .and_then(Value::as_str)
        .map(str::to_owned)
    else {
        return;
...
fn clear_current_record_binding(
    ...
    if let Some(current_record_id) = root
        .get(current_id_field)
        .and_then(Value::as_str)
        .map(str::to_owned)
        && let Some(history) = root.get_mut(history_field).and_then(Value::as_object_mut)
    {
...
fn write_minimal_unit_review_receipt_for_step(
    repo: &Path,
    state: &Path,
    execution_run_id: &str,
    task_number: u32,
    ...
    write_file(
        &path,
        &format!(
            "# Unit Review Result\n**Review Stage:** featureforge:unit-review\n**Reviewer Provenance:** dedicated-independent\n**Reviewer Source:** fresh-context-subagent\n**Source Plan:** {PLAN_REL}\n**Source Plan Revision:** 1\n**Execution Run ID:** {execution_run_id}\n**Execution Unit ID:** {execution_unit_id}\n**Reviewed Checkpoint SHA:** {reviewed_checkpoint_sha}\n**Approved Task Packet Fingerprint:** {approved_task_packet_fingerprint}\n**Result:** pass\n**Generated By:** featureforge:unit-review\n**Generated At:** 2026-03-29T21:00:00Z\n",
        ),
    );
    path
}

fn write_minimal_task_verification_receipt_for_task(
    repo: &Path,
    state: &Path,
    execution_run_id: &str,
    task_number: u32,
...
fn setup_task_boundary_prior_task_fixture(
    ...
    let status_before_task1 = run_rust_json(
        ...
        "status before task 1 begin for task-boundary fixture setup",
    );
    let begin_task1_step1 = run_rust_json(
        ...
        "begin task 1 step 1 for task-boundary fixture setup",
    );
    let complete_task1_step1 = run_rust_json(
        ...
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for task-boundary fixture setup.",
            "--file",
            "README.md",
        ...
        "complete task 1 step 1 for task-boundary fixture setup",
    );
    let begin_task1_step2 = run_rust_json(
        ...
        "begin task 1 step 2 for task-boundary fixture setup",
    );
    let complete_task1_step2 = run_rust_json(
        ...
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for task-boundary fixture setup.",
            "--file",
            "README.md",
            "--manual-verify-summary",
        ...
        "complete task 1 step 2 for task-boundary fixture setup",
    );
    ...
    let task1_step1_receipt = write_minimal_unit_review_receipt_for_step(
        repo,
        state,
        &execution_run_id,
        1,
    ...
    let task1_step2_receipt = write_minimal_unit_review_receipt_for_step(
        repo,
        state,
        &execution_run_id,
        1,
    ...
    let dispatch_id = dispatch["dispatch_id"]
        .as_str()
        .expect("record-review-dispatch should expose dispatch_id")
        .to_owned();
    ...
    let review_summary = "Task 1 independent review passed.\n";
    let verification_summary = "Task 1 verification passed against the current reviewed state.\n";
    write_harness_state_payload(
        repo,
        state,
        &json!({
            "current_task_closure_records": {
                "task-1": {
                    ...
                    "closure_record_id": deterministic_record_id(
                        "task-closure",
                        &[PLAN_REL, "1", strategy_checkpoint_fingerprint]
                    ),
                    "source_plan_path": PLAN_REL,
                    "source_plan_revision": 1,
                    "execution_run_id": execution_run_id,
                    "reviewed_state_id": format!("git_tree:{}", current_head_tree_sha(repo)),
                    "contract_identity": deterministic_record_id(
                        "task-contract",
                        &[PLAN_REL, "1", "1"]
                    ),
                    "effective_reviewed_surface_paths": ["README.md", "docs/example-output.md"],
                    "review_result": "pass",
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for task-boundary fixture setup",
    );
    let execution_fingerprint = status_before_task2["execution_fingerprint"]
        .as_str()
        .expect("status should expose execution_fingerprint before task 2 begin")
        .to_owned();
...
fn prepare_finished_single_step_finish_gate_fixture_with_plan_qa_requirement(
    ...
    let branch_contract_identity =
        branch_contract_identity(PLAN_REL, 1, &repo_slug, &branch, base_branch);
    ...
    let qa_summary = "Browser QA artifact fixture for gate-finish coverage.";
...
fn run_rust(repo: &Path, state: &Path, args: &[&str], context: &str) -> Output {
    if let Some(output) = match plan_execution_direct_support::try_run_plan_execution_output_direct(
...
fn run_rust_json(repo: &Path, state: &Path, args: &[&str], context: &str) -> Value {
    match plan_execution_direct_support::try_run_plan_execution_output_direct(
        ...
        Ok(Some(output)) => parse_json(&output, context),
        Ok(None) => parse_json(&run_rust(repo, state, args, context), context),
        Err(error) => panic!("{error}"),
    }
}
...
fn write_summary_fixture_file(repo: &Path, rel: &str, contents: &str) -> String {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).expect("summary fixture parent should be creatable");
    }
    write_file(&path, contents);
    path.display().to_string()
}

fn assert_mutator_direct_vs_shell_parity_case(
    case_name: &str,
    command_args_for_repo: impl Fn(&Path) -> Vec<String>,
) {
    let (repo_dir, shell_state_dir) =
        init_repo(&format!("plan-execution-mutator-parity-{case_name}"));
    let rust_state_dir = TempDir::new().expect("rust parity state tempdir should exist");
    ...
    let shell_args_owned = command_args_for_repo(repo);
    let rust_args_owned = command_args_for_repo(repo);
    let shell_args = shell_args_owned
        .iter()
        .map(String::as_str)
        .collect::<Vec<_>>();
    let rust_args = rust_args_owned
        .iter()
        .map(String::as_str)
        .collect::<Vec<_>>();
    ...
    let shell_output = run_shell(
        ...
        shell_state,
        &shell_args,
        &format!("shell mutator parity {case_name}"),
    );
    let rust_output = run_rust(
        repo,
        rust_state,
        &rust_args,
        &format!("rust mutator parity {case_name}"),
    );

    assert_eq!(
        rust_output.status.success(),
        shell_output.status.success(),
        "success status should match for {case_name}\nshell stdout:\n{}\nshell stderr:\n{}\nrust stdout:\n{}\nrust stderr:\n{}",
    ...
    let shell_json = if shell_output.stdout.is_empty() {
        serde_json::from_slice::<Value>(&shell_output.stderr)
            .expect("shell mutator parity output should be json")
    } else {
        serde_json::from_slice::<Value>(&shell_output.stdout)
            .expect("shell mutator parity output should be json")
    };
    let rust_json = if rust_output.stdout.is_empty() {
        serde_json::from_slice::<Value>(&rust_output.stderr)
            .expect("rust mutator parity output should be json")
    } else {
        serde_json::from_slice::<Value>(&rust_output.stdout)
            .expect("rust mutator parity output should be json")
    };
    assert_eq!(
        rust_json, shell_json,
        "json payload should match for {case_name}"
    );
}
...
fn direct_helper_attempts_aggregate_workflow_commands_without_real_cli_oracle() {
    ...
    let review_summary = repo.join("review-summary.md").display().to_string();
    let verification_summary = repo.join("verification-summary.md").display().to_string();
    let release_summary = repo.join("release-summary.md").display().to_string();
    let final_review_summary = repo.join("final-review-summary.md").display().to_string();
    let qa_summary = repo.join("qa-summary.md").display().to_string();

    let scenarios: [(&str, Vec<&str>); 6] = [
        (
            "close-current-task",
            vec![
                "close-current-task",
        ...
        (
            ...
            vec!["record-branch-closure", "--plan", PLAN_REL],
        ),
        (
    ...
    for (command, args) in scenarios {
        assert!(
            matches!(
                plan_execution_direct_support::try_run_plan_execution_output_direct(
                    repo,
                    state,
                    &args,
...
#[test]
fn canonical_gate_and_record_authority_commands_accept_explicit_plan_in_ambiguous_repo() {
    let (repo_dir, state_dir) = init_repo("plan-execution-authority-exact-plan-ambiguity");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_plan(repo, "featureforge:executing-plans");
    write_second_approved_plan_same_spec(repo, "featureforge:executing-plans");

    let cases = [
        (
            "gate-contract",
            "--contract",
            "docs/featureforge/execution-evidence/missing-contract.md",
            "ContractMismatch",
        ),
        (
            "record-contract",
        ...
        (
            "gate-evaluator",
            "--evaluation",
            "docs/featureforge/execution-evidence/missing-evaluation.md",
            "EvaluationMismatch",
        ),
        (
            "record-evaluation",
        ...
        (
            "gate-handoff",
            "--handoff",
            "docs/featureforge/execution-evidence/missing-handoff.md",
    ...
    for (command, flag, artifact, expected_failure_class) in cases {
        let output = run_rust_json(
            repo,
            state,
            &[command, "--plan", PLAN_REL, flag, artifact],
            &format!("{command} should resolve explicit --plan in ambiguous approved-plan repos"),
        );
        assert_eq!(
            output["failure_class"],
            Value::from(expected_failure_class),
            "{command} should resolve explicit --plan and continue to artifact validation instead of failing with ambiguous approved plan candidates: {output}"
        );
    }
}
...
fn canonical_gate_review_returns_blocking_result_for_newer_sibling_spec() {
    ...
    assert_eq!(gate_review["failure_class"], "StaleProvenance");
    assert!(
        gate_review["reason_codes"]
            .as_array()
            .is_some_and(|codes| !codes.is_empty()),
        "gate-review stale sibling spec coverage should expose actionable reason codes: {gate_review}"
    );
}

#[test]
fn canonical_gate_review_dispatch_routes_final_review_scope_to_branch_closure_remediation() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-stale-sibling-spec");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    assert_eq!(
        gate_review_dispatch["failure_class"],
        "ExecutionStateNotReady"
    );
    assert!(
        gate_review_dispatch["reason_codes"]
            .as_array()
            .is_some_and(|codes| !codes.is_empty()),
        "record-review-dispatch stale sibling coverage should expose actionable reason codes: {gate_review_dispatch}"
    );
    assert!(
        gate_review_dispatch["diagnostics"]
            ...
            .is_some_and(|diagnostics| diagnostics.iter().any(|diagnostic| {
                diagnostic["remediation"]
                    .as_str()
                    .is_some_and(|remediation| remediation.contains("advance-late-stage"))
            })),
        "record-review-dispatch should surface branch-closure refresh remediation text when final-review scope lacks a current branch closure: {gate_review_dispatch}"
    );
}
...
#[test]
fn canonical_mutator_commands_match_shell_when_direct_helper_handles_them() {
    assert_mutator_direct_vs_shell_parity_case("record-review-dispatch", |_repo| {
        vec![
            "record-review-dispatch".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
            "--scope".to_owned(),
            "task".to_owned(),
    ...
    assert_mutator_direct_vs_shell_parity_case("close-current-task", |repo| {
        let review_summary = write_summary_fixture_file(
            repo,
            "tmp/review-summary.md",
            "review summary parity fixture\n",
        );
        let verification_summary = write_summary_fixture_file(
            repo,
            "tmp/verification-summary.md",
            "verification summary parity fixture\n",
        );
        vec![
            "close-current-task".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
            "--task".to_owned(),
            "1".to_owned(),
            "--dispatch-id".to_owned(),
            "dispatch-fixture".to_owned(),
            "--review-result".to_owned(),
            "pass".to_owned(),
            "--review-summary-file".to_owned(),
            review_summary,
            "--verification-result".to_owned(),
            "pass".to_owned(),
            "--verification-summary-file".to_owned(),
            verification_summary,
        ]
    });

    assert_mutator_direct_vs_shell_parity_case("record-branch-closure", |_repo| {
        vec![
            "record-branch-closure".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
        ]
    });

    assert_mutator_direct_vs_shell_parity_case("record-release-readiness", |repo| {
        let release_summary = write_summary_fixture_file(
            repo,
            "tmp/release-summary.md",
            "release summary parity fixture\n",
        );
        vec![
            "record-release-readiness".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
            "--branch-closure-id".to_owned(),
            "branch-closure-fixture".to_owned(),
            "--result".to_owned(),
            "ready".to_owned(),
    ...
    assert_mutator_direct_vs_shell_parity_case("advance-late-stage", |repo| {
        let release_summary = write_summary_fixture_file(
            repo,
            "tmp/advance-summary.md",
            "advance summary parity fixture\n",
        );
        vec![
            "advance-late-stage".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
            "--result".to_owned(),
            "ready".to_owned(),
    ...
    assert_mutator_direct_vs_shell_parity_case("record-final-review", |repo| {
        let final_review_summary = write_summary_fixture_file(
            repo,
            "tmp/final-review-summary.md",
            "final review summary parity fixture\n",
        );
        vec![
            "record-final-review".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
            "--branch-closure-id".to_owned(),
            "branch-closure-fixture".to_owned(),
            ...
            "fresh-context-subagent".to_owned(),
            "--reviewer-id".to_owned(),
            "reviewer-fixture-001".to_owned(),
            "--result".to_owned(),
            "pass".to_owned(),
            "--summary-file".to_owned(),
            final_review_summary,
    ...
    assert_mutator_direct_vs_shell_parity_case("record-qa", |repo| {
        let qa_summary =
            write_summary_fixture_file(repo, "tmp/qa-summary.md", "qa summary parity fixture\n");
        vec![
            "record-qa".to_owned(),
            "--plan".to_owned(),
            PLAN_REL.to_owned(),
...
fn begin_blocks_interrupted_same_step_resume_without_prior_task_closure() {
    ...
    let status_before_task1 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 1 begin for interrupted resume test",
    );
    let begin_task1_step1 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 1 for interrupted resume test",
    );
    let complete_task1_step1 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for interrupted resume test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 1.",
        ...
        "complete task 1 step 1 for interrupted resume test",
    );
    let begin_task1_step2 = run_rust_json(
        repo,
        state,
    ...
    let complete_task1_step2 = run_rust_json(
        ...
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for interrupted resume test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 2.",
    ...
    let status_before_resume = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
    ...
    let resumed = run_rust(
        ...
        &[
            ...
            "featureforge:executing-plans",
            "--expect-execution-fingerprint",
            status_before_resume["execution_fingerprint"]
                .as_str()
                .expect("status should include execution_fingerprint before resume"),
        ],
        "begin interrupted same-step resume should fail without prior-task closure",
    );
    let failure = parse_failure_json(&resumed, "interrupted same-step resume closure gate");
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
            .as_str()
...
fn task_boundary_status_reports_prior_task_review_not_green_before_task2() {
    ...
    let status_before_task1 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 1 begin for task-boundary status review gate test",
    );
    let begin_task1_step1 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 1 for task-boundary status review gate test",
    );
    let complete_task1_step1 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for task-boundary status review gate test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 1.",
        ...
        "complete task 1 step 1 for task-boundary status review gate test",
    );
    let begin_task1_step2 = run_rust_json(
        repo,
        state,
    ...
    let complete_task1_step2 = run_rust_json(
        ...
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for task-boundary status review gate test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 2.",
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for task-boundary status review gate test",
    );
...
#[test]
fn task_boundary_begin_reports_prior_task_verification_missing_after_review_closure() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-begin-verification-receipt-missing");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_plan(repo, "none");
    ...
    let status_before_task1 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 1 begin for verification-missing test",
    );
    let begin_task1_step1 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 1 for verification-missing test",
    );
    let complete_task1_step1 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 1.",
        ...
        "complete task 1 step 1 for verification-missing test",
    );
    let begin_task1_step2 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 2 for verification-missing test",
    );
    let complete_task1_step2 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 2.",
        ...
        "complete task 1 step 2 for verification-missing test",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for verification-missing test",
    );
    let begin_task2_step1 = run_rust(
        repo,
        state,
        ...
        "begin task 2 step 1 should fail on missing task verification receipt",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin verification-missing gate",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
...
#[test]
fn begin_blocks_cross_task_when_legacy_run_is_missing_task_verification_receipt() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-begin-verification-receipt-missing-legacy");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_plan(repo, "none");
    ...
    let status_before_task1 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 1 begin for legacy verification-missing test",
    );
    let begin_task1_step1 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 1 for legacy verification-missing test",
    );
    let complete_task1_step1 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for legacy verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 1.",
        ...
        "complete task 1 step 1 for legacy verification-missing test",
    );
    let begin_task1_step2 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 2 for legacy verification-missing test",
    );
    let complete_task1_step2 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for legacy verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 2.",
        ...
        "complete task 1 step 2 for legacy verification-missing test",
    );
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail on legacy missing task verification receipt",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin legacy verification-missing gate",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
...
#[test]
fn complete_allows_cross_task_legacy_backfill_after_begin() {
    let (repo_dir, state_dir) = init_repo("plan-execution-legacy-begin-then-complete");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should allow legacy evidence backfill when prior task closure is complete",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));

    let complete_task2_step1 = run_rust_json(
        repo,
        state,
        &[
            "complete",
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 2 step 1 after legacy begin-path backfill.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 2 step 1 legacy backfill flow.",
            "--expect-execution-fingerprint",
            begin_task2_step1["execution_fingerprint"]
                .as_str()
                .expect("execution fingerprint should be present after begin"),
        ],
        "complete task 2 step 1 should succeed after legacy begin-path backfill",
    );
    assert_eq!(complete_task2_step1["active_task"], Value::Null);
    assert_eq!(complete_task2_step1["active_step"], Value::Null);
}

#[test]
fn task_boundary_status_reports_prior_task_verification_missing_after_review_closure() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-verification-receipt-missing");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_plan(repo, "none");
    ...
    let status_before_task1 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 1 begin for task-boundary status verification-missing test",
    );
    let begin_task1_step1 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 1 for task-boundary status verification-missing test",
    );
    let complete_task1_step1 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 1 for task-boundary status verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 1.",
        ...
        "complete task 1 step 1 for task-boundary status verification-missing test",
    );
    let begin_task1_step2 = run_rust_json(
        repo,
        state,
        ...
        "begin task 1 step 2 for task-boundary status verification-missing test",
    );
    let complete_task1_step2 = run_rust_json(
        repo,
        state,
        &[
            ...
            "featureforge:executing-plans",
            "--claim",
            "Completed task 1 step 2 for task-boundary status verification-missing test.",
            "--file",
            "README.md",
            "--manual-verify-summary",
            "Fixture verification for task 1 step 2.",
        ...
        "complete task 1 step 2 for task-boundary status verification-missing test",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for task-boundary status verification-missing test",
    );
    assert_eq!(status_before_task2["active_task"], Value::Null);
    assert_eq!(status_before_task2["active_step"], Value::Null);
    assert_eq!(status_before_task2["blocking_task"], Value::from(1));
...
#[test]
fn task_boundary_begin_allows_missing_prior_task_review_dispatch_when_current_closure_exists() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-boundary-review-dispatch-missing");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore missing prior-task review dispatch once a current closure exists",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_blocks_when_current_closure_overlay_requires_repair() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-overlay-restore-required");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current task-closure overlays for begin coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current task-closure overlays for begin coverage",
    );
    let closure_record_id =
        harness_state_json["current_task_closure_records"]["task-1"]["closure_record_id"]
            ...
            .to_owned();
    let mut history_payload = harness_state_json["current_task_closure_records"]["task-1"].clone();
    history_payload["task"] = Value::from(1);
    history_payload["record_id"] = Value::from(closure_record_id.clone());
    history_payload["record_status"] = Value::from("current");
    if history_payload["closure_status"].is_null() {
        history_payload["closure_status"] = Value::from("current");
    }
    harness_state_json["task_closure_record_history"][&closure_record_id] = history_payload;
    harness_state_json["current_task_closure_records"] = serde_json::json!({});
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current task-closure overlays for begin coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current task-closure overlays for begin coverage",
    );
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when current task-closure overlays must be repaired first",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin current closure overlay repair gate",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
...
fn assert_task_boundary_current_closure_repair_status(
    status: &Value,
    expected_reason_code: &str,
    expected_review_state_status: &str,
) {
    assert_eq!(status["blocking_task"], Value::from(1));
    assert_eq!(status["blocking_step"], Value::Null);
    assert_eq!(
        status["review_state_status"],
        Value::from(expected_review_state_status)
    );
    assert_eq!(
        status["phase_detail"],
    ...
    assert!(
        status["reason_codes"].as_array().is_some_and(|codes| {
            codes
                .iter()
                .any(|code| code.as_str() == Some(expected_reason_code))
        }),
        "status should surface {expected_reason_code}, got {status}"
    );
    assert_eq!(
...
#[test]
fn task_boundary_begin_fails_closed_when_current_closure_reviewed_state_id_uses_noncanonical_git_commit_alias()
 {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-noncanonical-review-state");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path)
            .expect("harness state should be readable before converting reviewed_state_id alias"),
    )
    .expect("harness state should remain valid json before converting reviewed_state_id alias");
    harness_state_json["current_task_closure_records"]["task-1"]["reviewed_state_id"] =
        Value::from(format!("git_commit:{}", current_head_sha(repo)));
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json)
            .expect("harness state should serialize after converting reviewed_state_id alias"),
    )
    .expect("harness state should be writable after converting reviewed_state_id alias");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when the stored current closure uses a noncanonical git_commit reviewed_state_id alias",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin noncanonical git_commit reviewed_state_id coverage",
    );
    ...
    assert!(
        ...
        "begin should surface a malformed reviewed_state_id diagnostic for noncanonical git_commit current closure state, got {failure}"
    );
}

#[test]
fn task_boundary_begin_fails_closed_when_current_closure_reviewed_state_id_uses_git_tree_commit_sha_alias()
 {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-git-tree-commit-review-state");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path)
            .expect("harness state should be readable before converting git_tree commit alias"),
    )
    .expect("harness state should remain valid json before converting git_tree commit alias");
    harness_state_json["current_task_closure_records"]["task-1"]["reviewed_state_id"] =
        Value::from(format!("git_tree:{}", current_head_sha(repo)));
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json)
            .expect("harness state should serialize after converting git_tree commit alias"),
    )
    .expect("harness state should be writable after converting git_tree commit alias");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when the stored current closure uses a git_tree commit alias instead of a canonical tree object id",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin git_tree commit alias reviewed_state_id coverage",
    );
    assert_eq!(failure["error_class"], "MalformedExecutionState");
    assert!(
        failure["message"].as_str().is_some_and(|message| {
        ...
        "begin should surface a malformed reviewed_state_id diagnostic for git_tree commit aliases, got {failure}"
    );
}

#[test]
fn task_boundary_begin_fails_closed_when_current_closure_reviewed_state_id_points_to_missing_tree()
{
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-missing-tree-review-state");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure reviewed_state_id",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure reviewed_state_id",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["reviewed_state_id"] =
        Value::from("git_tree:deadbeefdeadbeefdeadbeefdeadbeefdeadbeef");
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure reviewed_state_id",
        ),
    )
    .expect("harness state should be writable after corrupting current closure reviewed_state_id");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when the current closure reviewed_state_id points to a missing canonical tree",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin missing-tree current closure reviewed_state_id coverage",
    );
    assert_eq!(failure["error_class"], "MalformedExecutionState");
    assert!(
        failure["message"].as_str().is_some_and(|message| {
        ...
        "begin should surface a malformed reviewed_state_id diagnostic, got {failure}"
    );
}

#[test]
fn task_boundary_status_requires_execution_reentry_when_current_closure_reviewed_state_id_is_malformed()
 {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-current-closure-malformed-review-state");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, _execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);

    let harness_state_path = harness_state_file_path(repo, state);
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure reviewed_state_id for status coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure reviewed_state_id for status coverage",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["reviewed_state_id"] =
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure reviewed_state_id for status coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure reviewed_state_id for status coverage",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should require execution reentry when the current closure reviewed_state_id is malformed",
    );
    assert_task_boundary_current_closure_repair_status(
        &status_before_task2,
...
fn task_boundary_repair_review_state_routes_malformed_current_closure_to_execution_reentry() {
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before repair-review-state malformed current closure coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before repair-review-state malformed current closure coverage",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["reviewed_state_id"] =
        Value::from("unsupported-reviewed-state");
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure reviewed_state_id for repair coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure reviewed_state_id for repair coverage",
    );
...
#[test]
fn task_boundary_begin_blocks_when_current_closure_plan_revision_mismatches_active_plan() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-plan-revision-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure plan revision",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure plan revision",
    );
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure plan revision",
        ),
    )
    .expect("harness state should be writable after corrupting current closure plan revision");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when the current closure plan revision mismatches the active plan",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin current closure plan revision mismatch coverage",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
        ...
        "begin should reject current closures that are not bound to the active plan revision, got {failure}"
    );
}

#[test]
fn task_boundary_begin_blocks_when_current_closure_contract_identity_mismatches_active_task() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-contract-identity-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure contract identity",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure contract identity",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["contract_identity"] =
        Value::from("task-contract-foreign");
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure contract identity",
        ),
    )
    .expect("harness state should be writable after corrupting current closure contract identity");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail closed when the current closure contract identity mismatches the active task",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin current closure contract identity mismatch coverage",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
            .as_str()
            .is_some_and(|message| message.contains("prior_task_current_closure_invalid")),
        "begin should reject current closures that are not bound to the active task contract, got {failure}"
    );
}

#[test]
fn task_boundary_status_requires_execution_reentry_when_current_closure_plan_revision_mismatches_active_plan()
 {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-current-closure-plan-revision-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, _execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure plan revision for status coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure plan revision for status coverage",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["source_plan_revision"] =
        Value::from(999);
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure plan revision for status coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure plan revision for status coverage",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should require execution reentry when the current closure plan revision mismatches the active plan",
    );
    assert_task_boundary_current_closure_repair_status(
        &status_before_task2,
...
#[test]
fn task_boundary_status_requires_execution_reentry_when_current_closure_reviewed_surface_is_empty()
{
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-empty-reviewed-surface");
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure reviewed surface for status coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure reviewed surface for status coverage",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["effective_reviewed_surface_paths"] =
        Value::Array(Vec::new());
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure reviewed surface for status coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure reviewed surface for status coverage",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should require execution reentry when the current closure reviewed surface is empty",
    );
    assert_task_boundary_current_closure_repair_status(
        &status_before_task2,
        "prior_task_current_closure_invalid",
...
#[test]
fn task_boundary_repair_review_state_restores_recoverable_current_closure_overlay_from_history() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-repair-current-closure-plan-revision-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, _execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let dispatch_id =
        harness_state_json["strategy_review_dispatch_lineage"]["task-1"]["dispatch_id"]
            .as_str()
            .expect("fixture should expose the current task dispatch lineage id")
            .to_owned();
    let mut history_payload = harness_state_json["current_task_closure_records"]["task-1"].clone();
    history_payload["task"] = Value::from(1);
    history_payload["record_id"] = Value::from(closure_record_id.clone());
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure plan revision for repair coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure plan revision for repair coverage",
    );
    ...
    let repair_json = run_rust_json(
        ...
        state,
        &["repair-review-state", "--plan", PLAN_REL],
        "repair-review-state should restore recoverable current task-closure overlay corruption from history",
    );
    ...
    let task_closure_history = harness_state_json["task_closure_record_history"]
        .as_object()
        .expect("repair should preserve task-closure history");
    let task_history_record = task_closure_history
        .values()
...
#[test]
fn task_boundary_status_requires_repair_when_current_closure_execution_run_id_is_missing() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-current-closure-missing-execution-run-id");
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current closure execution_run_id for status coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current closure execution_run_id for status coverage",
    );
    harness_state_json["current_task_closure_records"]["task-1"]["execution_run_id"] = Value::Null;
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current closure execution_run_id for status coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current closure execution_run_id for status coverage",
    );
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should require execution reentry when the current closure execution_run_id is missing",
    );
    assert_task_boundary_current_closure_repair_status(
        &status_before_task2,
        "prior_task_current_closure_invalid",
...
fn task_boundary_repair_review_state_clears_stale_current_closure_for_execution_reentry() {
    ...
    fs::write(
        repo.join("README.md"),
        "# task-boundary drift fixture\npost-closure drift on a covered path\n",
    )
    ...
    assert!(
        ...
        "repair-review-state should remove stale current task-closure truth from the authoritative current set, got {harness_state_json}"
    );
}

#[test]
fn task_boundary_begin_blocks_when_current_closure_drifted_on_covered_path() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-boundary-current-closure-drift");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    .expect("README should be writable for current-closure drift coverage");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should fail when the current prior-task closure drifted on a covered path",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin current closure drift coverage",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
            .as_str()
            .is_some_and(|message| message.contains("prior_task_current_closure_stale")),
        "begin should surface prior_task_current_closure_stale once a covered path drifts, got {failure}"
    );
}

#[test]
fn task_boundary_status_requires_execution_reentry_when_current_closure_drifted_on_covered_path() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-current-closure-drift");
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    .expect("README should be writable for current-closure drift status coverage");

    let status_before_task2 = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should require execution reentry when the current prior-task closure drifted on a covered path",
    );
    assert_eq!(status_before_task2["blocking_task"], Value::from(1));
    assert_eq!(status_before_task2["blocking_step"], Value::Null);
    assert_eq!(
    ...
    assert!(
        status_before_task2["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|code| { code.as_str() == Some("prior_task_current_closure_stale") })),
        "status should surface prior_task_current_closure_stale once a covered path drifts, got {status_before_task2}"
    );
    assert_eq!(
...
#[test]
fn record_review_dispatch_task_scope_blocks_when_requested_task_current_closure_is_stale() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-current-task-closure-stale");
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    .expect("README should be writable for stale task review-dispatch coverage");

    let harness_state_before: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state)).expect(
            "harness state should be readable before blocked stale current-closure dispatch",
        ),
    )
    .expect("harness state should remain valid json before blocked stale current-closure dispatch");
    ...
    let blocked = run_rust_json(
        ...
        "record-review-dispatch should fail closed when the requested task current closure is stale",
    );
    ...
    assert!(
        blocked["reason_codes"].as_array().is_some_and(|codes| {
            codes
                .iter()
                .any(|code| code.as_str() == Some("prior_task_current_closure_stale"))
        }),
        "blocked dispatch should surface the stale current-closure reason, got {blocked}"
    );
    assert!(
        blocked["diagnostics"]
            .as_array()
            .is_some_and(|diagnostics| diagnostics.iter().any(|diagnostic| {
                diagnostic["remediation"]
                    .as_str()
                    .is_some_and(|remediation| remediation.contains("repair-review-state"))
            })),
        "blocked dispatch should route through repair-review-state before recording fresh lineage, got {blocked}"
    );

    let harness_state_after: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state)).expect(
            "harness state should be readable after blocked stale current-closure dispatch",
        ),
    )
    .expect("harness state should remain valid json after blocked stale current-closure dispatch");
    assert_eq!(
        harness_state_after, harness_state_before,
        "blocked stale current-closure dispatch must not mutate authoritative harness state"
    );
}

#[test]
fn record_review_dispatch_task_scope_blocks_when_current_closure_overlay_requires_repair() {
    let (repo_dir, state_dir) = init_repo(
        "plan-execution-record-review-dispatch-current-task-closure-overlay-restore-required",
    );
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting current task-closure overlays for dispatch coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting current task-closure overlays for dispatch coverage",
    );
    let closure_record_id =
        harness_state_json["current_task_closure_records"]["task-1"]["closure_record_id"]
            .as_str()
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting current task-closure overlays for dispatch coverage",
        ),
    )
    .expect(
        "harness state should be writable after corrupting current task-closure overlays for dispatch coverage",
    );
    ...
    let blocked = run_rust_json(
        ...
        "record-review-dispatch should fail closed when current task-closure overlays must be repaired first",
    );
    assert_eq!(blocked["allowed"], Value::Bool(false));
    assert_eq!(blocked["action"], Value::from("blocked"));
    assert_eq!(
    ...
    assert!(
        blocked["reason_codes"].as_array().is_some_and(|codes| {
            codes
                .iter()
                .any(|code| code.as_str() == Some("current_task_closure_overlay_restore_required"))
        }),
        "blocked dispatch should surface the current task-closure overlay restore requirement, got {blocked}"
    );
    assert_eq!(
...
#[test]
fn record_review_dispatch_task_scope_blocks_when_requested_task_already_has_current_closure() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-current-task-closure-already-current");
    let repo = repo_dir.path();
    let state = state_dir.path();
    setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let harness_state_before: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state)).expect(
            "harness state should be readable before blocked current-closure dispatch rerun",
        ),
    )
    .expect("harness state should remain valid json before blocked current-closure dispatch rerun");
    ...
    let blocked = run_rust_json(
        ...
        "record-review-dispatch should fail closed when the requested task already has a current closure",
    );
    assert_eq!(blocked["allowed"], Value::Bool(false));
    assert_eq!(blocked["action"], Value::from("blocked"));
    assert_eq!(
    ...
    assert!(
        blocked["reason_codes"].as_array().is_some_and(|codes| {
            codes
                .iter()
                .any(|code| code.as_str() == Some("task_current_closure_already_current"))
        }),
        "blocked dispatch rerun should surface the already-current task-closure reason, got {blocked}"
    );
    assert!(
        blocked["diagnostics"]
            .as_array()
            .is_some_and(|diagnostics| {
                diagnostics.iter().any(|diagnostic| {
                    diagnostic["message"].as_str().is_some_and(|message| {
                        message.contains("already has a current passing task closure")
                    })
                })
            }),
        "blocked dispatch rerun should explain why the task is out of phase, got {blocked}"
    );

    let harness_state_after: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state)).expect(
            "harness state should be readable after blocked current-closure dispatch rerun",
        ),
    )
    .expect("harness state should remain valid json after blocked current-closure dispatch rerun");
    assert_eq!(
        harness_state_after, harness_state_before,
        "blocked current-closure dispatch rerun must not mutate authoritative harness state"
    );
}

#[test]
fn task_boundary_begin_blocks_on_stale_prior_task_review_dispatch_before_current_closure_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-stale-review-dispatch-before-closure");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for stale review dispatch pre-closure coverage",
    );
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path)
            .expect("harness state should be readable before corrupting stale dispatch coverage"),
    )
    .expect("harness state should remain valid json before corrupting stale dispatch coverage");
    harness_state_json["current_task_closure_records"] = json!({});
    harness_state_json["strategy_review_dispatch_lineage"]["task-1"] = json!({
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json)
            .expect("harness state should serialize after corrupting stale dispatch coverage"),
    )
    .expect("harness state should be writable after corrupting stale dispatch coverage");
    ...
    let begin_task2_step1 = run_rust(
        ...
        "begin task 2 step 1 should still block on stale review dispatch before a current closure exists",
    );
    let failure = parse_failure_json(
        &begin_task2_step1,
        "task-boundary begin stale review dispatch before current closure coverage",
    );
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
...
#[test]
fn task_boundary_status_reports_stale_prior_task_review_dispatch_before_current_closure_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-stale-review-dispatch-before-closure");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin for stale review dispatch pre-closure status coverage",
    );
    let strategy_checkpoint_fingerprint = status_before_task2
        .get("last_strategy_checkpoint_fingerprint")
        .and_then(Value::as_str)
    ...
    let mut harness_state_json: Value = serde_json::from_str(
        &fs::read_to_string(&harness_state_path).expect(
            "harness state should be readable before corrupting stale dispatch status coverage",
        ),
    )
    .expect(
        "harness state should remain valid json before corrupting stale dispatch status coverage",
    );
    harness_state_json["current_task_closure_records"] = json!({});
    harness_state_json["strategy_review_dispatch_lineage"]["task-1"] = json!({
        "dispatch_id": "task-1-stale-dispatch",
    ...
    fs::write(
        &harness_state_path,
        serde_json::to_string_pretty(&harness_state_json).expect(
            "harness state should serialize after corrupting stale dispatch status coverage",
        ),
    )
    .expect("harness state should be writable after corrupting stale dispatch status coverage");

    let status_before_task2 = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should still surface stale review dispatch before a current closure exists",
    );
    assert_eq!(status_before_task2["blocking_task"], Value::from(1));
    assert_eq!(status_before_task2["blocking_step"], Value::Null);
    assert!(
...
#[test]
fn task_boundary_status_ignores_prior_task_review_dispatch_missing_when_current_closure_exists() {
    let (repo_dir, state_dir) =
    ...
    assert!(
        status_before_task2["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.is_empty()),
        "status should not retain prior_task_review_dispatch_missing once a current closure exists, got {status_before_task2}"
    );
    assert!(
        status_before_task2["recommended_command"]
            .as_str()
...
#[test]
fn task_boundary_begin_allows_stale_prior_task_review_dispatch_when_current_closure_exists() {
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore stale task-boundary review dispatch evidence after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_allows_invalid_dispatch_source_step_when_current_closure_exists() {
    let (repo_dir, state_dir) =
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore invalid task-boundary review dispatch source step after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_allows_mismatched_dispatch_run_id_when_current_closure_exists() {
    let (repo_dir, state_dir) =
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore mismatched task-boundary review dispatch run identity after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_allows_mismatched_dispatch_checkpoint_fingerprint_when_current_closure_exists()
 {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-review-dispatch-invalid-checkpoint-fingerprint");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    harness_state_json["strategy_review_dispatch_lineage"]["task-1"]["strategy_checkpoint_fingerprint"] =
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore mismatched task-boundary review dispatch checkpoint fingerprint after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_reports_task_cycle_break_active() {
    ...
    let failure = parse_failure_json(&begin_task2_step1, "task-boundary begin cycle-break gate");
    assert_eq!(failure["error_class"], "ExecutionStateNotReady");
    assert!(
        failure["message"]
            .as_str()
            .is_some_and(|message| message.contains("task_cycle_break_active")),
        "task-boundary begin should expose task_cycle_break_active diagnostics, got {failure}"
    );
...
#[test]
fn task_boundary_begin_allows_stale_review_receipt_binding_when_current_closure_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-stale-review-receipt-binding");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);

    replace_in_file(
        &task1_step1_receipt,
        &format!("**Reviewed Checkpoint SHA:** {}", current_head_sha(repo)),
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore stale task-boundary review receipt binding after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_allows_non_independent_review_receipt_when_current_closure_exists() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-boundary-review-not-independent");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore non-independent task-boundary review receipt after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_status_ignores_non_independent_review_receipt_when_current_closure_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-status-review-not-independent");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, _execution_fingerprint, task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let status_before_task2 = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before task 2 begin should ignore non-independent review receipt after current closure",
    );
    assert_eq!(status_before_task2["active_task"], Value::Null);
    assert_eq!(status_before_task2["active_step"], Value::Null);
    assert_eq!(status_before_task2["blocking_task"], Value::Null);
    ...
    assert!(
        status_before_task2["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.is_empty()),
        "status should not surface stale review receipt diagnostics once a current closure exists, got {status_before_task2}"
    );
    assert_eq!(
        status_before_task2["phase_detail"],
        Value::from("branch_closure_recording_required_for_release_readiness")
...
#[test]
fn task_boundary_begin_allows_malformed_review_receipt_when_current_closure_exists() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-boundary-malformed-review-receipt");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (_execution_run_id, execution_fingerprint, task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore malformed task-boundary review receipt after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
#[test]
fn task_boundary_begin_allows_malformed_task_verification_receipt_when_current_closure_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-task-boundary-malformed-task-verification-receipt");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let (execution_run_id, execution_fingerprint, _task1_step1_receipt, _task1_step2_receipt) =
        setup_task_boundary_prior_task_fixture(repo, state);
    ...
    let begin_task2_step1 = run_rust_json(
        ...
        "begin task 2 step 1 should ignore malformed task verification receipt after current closure",
    );
    assert_eq!(begin_task2_step1["active_task"], Value::from(2));
    assert_eq!(begin_task2_step1["active_step"], Value::from(1));
}
...
fn preflight_acceptance_bootstraps_harness_state_without_clobbering_existing_fields() {
    ...
    assert_eq!(
        harness_state_json["chunk_id"], status_after_preflight["chunk_id"],
...
fn preflight_bootstraps_authoritative_run_identity_and_dependency_index_truth() {
    ...
    assert_eq!(
        harness_state["chunk_id"], status_after_preflight["chunk_id"],
...
#[test]
fn status_and_finish_gates_expose_april_contract_shape_fields() {
    let (repo_dir, state_dir) = init_repo("plan-execution-april-status-gate-shape");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    ...
    let current_branch_closure_id = "branch-closure-ready";
    let current_branch_reviewed_state_id = format!("git_tree:{}", current_head_tree_sha(repo));
    let current_branch_contract_identity = branch_contract_identity(
        PLAN_REL,
        1,
        &repo_slug(repo),
        &branch_name(repo),
    ...
    write_harness_state_payload(
        ...
        &json!({
            ...
            "current_branch_closure_reviewed_state_id": current_branch_reviewed_state_id,
            "current_branch_closure_contract_identity": current_branch_contract_identity,
            "finish_review_gate_pass_branch_closure_id": Value::Null
        }),
    );

    let status_before_gate_review = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status before gate-review should expose April contract shape fields",
    );
    let authoritative_state: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("authoritative state should be readable for status shape checks"),
    )
    .expect("authoritative state should remain valid json for status shape checks");

    assert_eq!(
        status_before_gate_review["workspace_state_id"],
        Value::from(format!("git_tree:{}", current_head_tree_sha(repo)))
    );
    assert_eq!(
        status_before_gate_review["current_branch_reviewed_state_id"],
        authoritative_state["current_branch_closure_reviewed_state_id"]
    );
    assert_eq!(
        status_before_gate_review["current_branch_closure_id"],
        authoritative_state["current_branch_closure_id"]
    );
    assert!(
        status_before_gate_review["current_task_closures"].is_array(),
        "status should expose current_task_closures as an array: {status_before_gate_review}"
    );
    assert!(
        status_before_gate_review["blocking_records"].is_array(),
        "status should expose blocking_records as an array: {status_before_gate_review}"
    );
    assert_eq!(
        status_before_gate_review["review_state_status"],
        Value::from("clean")
    );
    assert!(
        status_before_gate_review["phase_detail"].as_str().is_some(),
        "status should expose phase_detail as a string: {status_before_gate_review}"
    );
    assert!(
        status_before_gate_review["next_action"].as_str().is_some(),
        "status should expose next_action as a string: {status_before_gate_review}"
    );
    assert!(
        status_before_gate_review["recommended_command"].is_string()
            || status_before_gate_review["recommended_command"].is_null(),
        "status should expose recommended_command as a string or null: {status_before_gate_review}"
    );
    assert!(
        status_before_gate_review["current_final_review_state"].is_string(),
        "status should expose current_final_review_state as a string: {status_before_gate_review}"
    );
    assert!(
        status_before_gate_review["current_qa_state"].is_string(),
        "status should expose current_qa_state as a string: {status_before_gate_review}"
    );
    ...
    let gate_review = run_rust_json(
        ...
        state,
        &["gate-review", "--plan", PLAN_REL],
        "gate-review should expose April contract shape fields",
    );
    assert_eq!(gate_review["action"], Value::from("passed"));
    assert_eq!(
        gate_review["current_branch_closure_id"],
        authoritative_state["current_branch_closure_id"]
    );
    assert_eq!(
    ...
    let status_after_gate_review = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status after gate-review should expose the refreshed finish checkpoint",
    );
    assert_eq!(
        status_after_gate_review["phase_detail"],
        Value::from("finish_completion_gate_ready")
    );
    assert_eq!(
        status_after_gate_review["review_state_status"],
        Value::from("clean")
    );
    assert_eq!(
    ...
    let gate_finish = run_rust_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate-finish should expose April contract shape fields",
    );
    assert_eq!(gate_finish["action"], Value::from("passed"));
    assert_eq!(
        gate_finish["current_branch_closure_id"],
        authoritative_state["current_branch_closure_id"]
    );
    assert_eq!(
        gate_finish["finish_review_gate_pass_branch_closure_id"],
        authoritative_state["current_branch_closure_id"]
    );
}
...
fn gate_finish_accepts_richer_additive_test_plan_sections() {
    ...
    let qa_path = write_qa_result_artifact(repo, state, &test_plan);
    let base_branch = branch_name(repo);
    ...
    let git_dir = discover_repository(repo)
    ...
    assert!(gate_finish.allowed, "json: {gate_finish:?}");
...
fn gate_finish_reports_invalid_plan_qa_requirement_before_qa_provenance_failures() {
    ...
    assert_eq!(
        gate_finish["failure_class"], "ExecutionStateNotReady",
        "json: {gate_finish}"
    );
    assert_eq!(
        gate_finish["reason_codes"],
...
#[test]
fn gate_finish_accepts_test_plan_and_qa_receipt_regressions_when_authoritative_record_is_current() {
    let mut cases = vec![
        (
            "malformed_test_plan",
            "malformed_test_plan",
    ...
    for (case_name, mutator, _expected_failure_class, _expected_reason_code) in cases {
        let (repo_dir, state_dir) = init_repo(&format!("plan-execution-finish-{case_name}"));
        let repo = repo_dir.path();
        let state = state_dir.path();
        let base_branch = branch_name(repo);
        ...
        if matches!(
            ...
            let (updated_test_plan_path, updated_test_plan_fingerprint) =
                republish_authoritative_artifact(
                    repo,
                    state,
                    "test-plan",
            ...
            retarget_current_qa_source_test_plan_fingerprint(
                repo,
                state,
                &updated_test_plan_fingerprint,
            );
...
fn write_authoritative_downstream_fixture_state(
    ...
    let base_branch = resolve_release_base_branch(&runtime.git_dir, &runtime.branch_name)
    ...
    let qa_summary = "Browser QA artifact fixture for gate-finish coverage.";
    let qa_summary_hash = sha256_hex(qa_summary.as_bytes());
    let task_review_summary = "Task closure review fixture for downstream finish-gate coverage.";
    let task_review_summary_hash = sha256_hex(task_review_summary.as_bytes());
    let task_verification_summary =
        "Task closure verification fixture for downstream finish-gate coverage.";
    let task_verification_summary_hash = sha256_hex(task_verification_summary.as_bytes());
    ...
    let task_closure_record = json!({
        ...
        "source_plan_revision": 1,
        "execution_run_id": format!("run-{safe_branch}-finish"),
        "reviewed_state_id": format!("git_tree:{current_tree}"),
        "contract_identity": deterministic_record_id("task-contract", &[PLAN_REL, "1", "1"]),
        "effective_reviewed_surface_paths": ["README.md"],
        "review_result": "pass",
        "review_summary_hash": task_review_summary_hash,
    ...
    let browser_qa_record_history = json!({
        qa_record_id.clone(): {
            "record_id": qa_record_id.clone(),
            "record_sequence": 1,
            "record_status": "current",
            "branch_closure_id": branch_closure_id,
            "final_review_record_id": final_review_record_id.clone(),
    ...
    let mut harness_state = json!({
        ...
        "current_qa_result": "pass",
        "current_qa_summary_hash": qa_summary_hash,
        "current_qa_record_id": qa_record_id,
        "browser_qa_record_history": browser_qa_record_history,
        "last_final_review_artifact_fingerprint": authoritative_review_fingerprint,
        "last_browser_qa_artifact_fingerprint": authoritative_qa_fingerprint,
        "last_release_docs_artifact_fingerprint": authoritative_release_fingerprint,
        "current_branch_closure_id": branch_closure_id,
        "current_branch_closure_reviewed_state_id": format!("git_tree:{current_tree}"),
        "current_branch_closure_contract_identity": branch_contract_identity,
        "branch_closure_records": branch_closure_records,
        "finish_review_gate_pass_branch_closure_id": branch_closure_id,
        "active_contract_path": format!("contract-{active_contract_fingerprint}.md"),
...
fn gate_finish_rejects_malformed_authoritative_worktree_state() {
    ...
    let failure = parse_failure_json(&output, "gate finish malformed authoritative harness state");
    ...
    assert!(
        failure["message"]
            .as_str()
            .unwrap_or_default()
            .contains("Authoritative harness state is malformed")
    );
}
...
fn gate_finish_rejects_release_artifact_head_mismatch() {
    ...
    write_file(&release_path, &stale_release);
    let _ = republish_authoritative_artifact_from_path(
        repo,
        state,
        &release_path,
    ...
    assert_eq!(gate_finish["allowed"], true, "{gate_finish}");
}

#[test]
fn gate_finish_accepts_release_receipt_regressions_when_authoritative_record_is_current() {
    for (case_name, mutator, _expected_reason_code) in [
        (
...
fn gate_finish_ignores_tracked_execution_evidence_writeback_after_artifact_generation() {
    ...
    run_checked(
        git_add,
        "git add tracked execution evidence for finish gate test",
    );
...
fn gate_review_rejects_v2_plan_fingerprint_mismatch() {
    ...
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_default_approved_single_step_execution_fixture(repo);
...
fn gate_review_accepts_latest_proof_for_shared_file() {
    ...
    let base_branch = branch_name(repo);
    let runtime = execution_runtime(repo, state);
    let release_base_branch = resolve_release_base_branch(&runtime.git_dir, &runtime.branch_name)
    ...
    let test_plan_path = write_test_plan_artifact(repo, state, "yes");
    let qa_path = write_qa_result_artifact(repo, state, &test_plan_path);
    let review_path = write_code_review_artifact(repo, state, &release_base_branch);
    let release_path = write_release_readiness_artifact(repo, state, &release_base_branch);
    write_authoritative_downstream_fixture_state(
        repo,
        state,
        &test_plan_path,
    ...
    assert_eq!(
        gate_review["allowed"], true,
        "gate-review should accept the current shared-file proof once authoritative late-stage state is present: {gate_review}",
    );
    assert_eq!(gate_review["action"], Value::from("passed"));
    assert_eq!(
        gate_review["current_branch_closure_id"],
    ...
    let authoritative_state: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("authoritative state should be readable after shared-file gate-review"),
    )
    .expect("authoritative state should remain valid json after shared-file gate-review");
    assert_eq!(
...
#[test]
fn gate_review_is_read_only_before_dispatch() {
    let (repo_dir, state_dir) = init_repo("plan-execution-gate-review-read-only");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    let contract_rel = "docs/featureforge/execution-evidence/task4-read-only-review-contract.md";
    let contract_fingerprint = write_execution_contract_artifact(repo, contract_rel, None);
    write_harness_state_fixture!(
        repo,
        state,
    ...
    let status_before_begin = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before read-only gate-review begin",
    );
    run_rust_json(
        repo,
        state,
        ...
        "begin active work before read-only gate-review check",
    );
    let status_before_gate = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status before read-only gate-review",
    );
    ...
    let gate = run_rust_json(
        ...
        state,
        &["gate-review", "--plan", PLAN_REL],
        "gate-review should stay read-only before dispatch",
    );
    ...
    assert!(
        ...
        "gate-review should still fail closed while active work remains in progress"
    );

    let status_after_gate = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status after read-only gate-review",
    );
    assert_eq!(
        status_after_gate["strategy_checkpoint_kind"],
        status_before_gate["strategy_checkpoint_kind"],
        "read-only gate-review should not mutate strategy checkpoint kind"
    );
    assert_eq!(
        status_after_gate["last_strategy_checkpoint_fingerprint"],
        status_before_gate["last_strategy_checkpoint_fingerprint"],
        "read-only gate-review should not mutate strategy checkpoint fingerprint"
    );
    assert_eq!(
        status_after_gate["strategy_state"], status_before_gate["strategy_state"],
        "read-only gate-review should not mutate strategy state"
    );
}
...
#[test]
fn record_review_dispatch_task_target_mismatch_fails_before_authoritative_mutation() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-target-mismatch-no-mutation");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    let status = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before record-review-dispatch target mismatch invariant",
    );
    run_rust_json(
        repo,
        state,
        ...
        "begin active work before record-review-dispatch target mismatch invariant",
    );
    ...
    let output = run_rust(
        ...
        "record-review-dispatch target mismatch should fail before mutation",
    );
    let failure = parse_failure_json(
        &output,
        "record-review-dispatch target mismatch should fail before mutation",
    ...
    assert!(
        ...
        "target-mismatch failure should explain the current dispatch target contract: {failure}"
    );
    assert_eq!(
...
#[test]
fn record_review_dispatch_final_review_scope_rejects_task_field_before_authoritative_mutation() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-final-review-task-field-no-mutation");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    let output = run_rust(
        ...
        "record-review-dispatch final-review scope should reject task field before mutation",
    );
    let failure = parse_failure_json(
        &output,
        "record-review-dispatch final-review scope should reject task field before mutation",
    ...
    assert_eq!(
        authoritative_state_digest(repo, state),
        digest_before,
        "unsupported final-review task field must not mutate authoritative state"
    );
}
...
fn record_review_dispatch_task_scope_requires_current_target_when_no_reviewable_work_exists() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-no-reviewable-work");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
...
#[test]
fn record_review_dispatch_final_review_scope_requires_completed_plan_target() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-final-review-bound");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    let contract_rel =
        "docs/featureforge/execution-evidence/task4-review-dispatch-final-review-bound-contract.md";
    let contract_fingerprint = write_execution_contract_artifact(repo, contract_rel, None);
    write_harness_state_fixture!(
        repo,
        state,
    ...
    let status = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before bound final-review dispatch validation",
    );
    run_rust_json(
        repo,
        state,
        ...
        "begin active work before bound final-review dispatch validation",
    );
    let harness_state_before: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("harness state should be readable before blocked bound final-review dispatch"),
    )
    ...
    let harness_state: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("harness state should be readable after blocked bound final-review dispatch"),
    )
    .expect("harness state should be valid json after blocked bound final-review dispatch");
    assert!(
        harness_state["final_review_dispatch_lineage"].is_null(),
        "blocked bound final-review dispatch should not persist final-review lineage early: {harness_state}"
    );
    assert_eq!(
        harness_state["strategy_checkpoints"], harness_state_before["strategy_checkpoints"],
        "blocked bound final-review dispatch should not append new strategy checkpoints: {harness_state}"
    );
    assert_eq!(
        harness_state["last_strategy_checkpoint_fingerprint"],
        harness_state_before["last_strategy_checkpoint_fingerprint"],
        "blocked bound final-review dispatch should not rotate the strategy checkpoint fingerprint: {harness_state}"
    );
}
...
fn gate_review_dispatch_backfills_initial_dispatch_before_review_remediation() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-record-review-dispatch-backfills-initial");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
...
#[test]
fn gate_review_dispatch_blocked_request_does_not_bootstrap_missing_authoritative_state() {
    let (repo_dir, state_dir) =
    ...
    assert!(
        ...
        "record-review-dispatch should still return a blocked gate payload when state is missing: {gate_review_dispatch}"
    );
}
...
#[test]
fn begin_reclaims_stale_write_authority_lock_before_mutation() {
    let (repo_dir, state_dir) = init_repo("plan-execution-begin-reclaims-stale-write-authority");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "none");
    ...
    let status = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before begin stale write-authority reclaim test",
    );
    let begin = run_rust_json(
        ...
        "begin should reclaim a stale write-authority lock",
    );
    assert_eq!(begin["active_task"], Value::from(1), "json: {begin}");
    assert_eq!(begin["active_step"], Value::from(1), "json: {begin}");
    assert!(
        !lock_path.exists(),
        "stale write-authority lock should be removed after begin reclaims it"
    );
}
...
fn rebuild_evidence_session_not_found() {
    ...
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("session_not_found"))
    );
}
...
fn rebuild_evidence_invalid_scope() {
    ...
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("scope_no_matches"))
    );
}
...
fn rebuild_evidence_scope_empty() {
    ...
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("scope_empty"))
    );
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("1:1"))
    );
}
...
fn rebuild_evidence_rejects_zero_max_jobs() {
    ...
    let output = run_rust(
        repo,
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--max-jobs", "0"],
    ...
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("--max-jobs"))
    );
}
...
fn rebuild_evidence_rejects_parallel_max_jobs() {
    ...
    assert!(
        json["message"]
            .as_str()
            .is_some_and(|message| message.contains("max_jobs_parallel_unsupported"))
    );
}
...
fn rebuild_candidate_discovery_source_spec_fingerprint_drift() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-source-spec-fingerprint-drift");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_default_approved_single_step_execution_fixture(repo);
    let evidence_path = repo.join(evidence_rel_path());
...
fn gate_review_rejects_contractless_current_run_unit_review_receipt_with_stale_provenance() {
    ...
    assert_eq!(
        rebuilt["counts"]["manual"],
        Value::from(1),
        "json: {rebuilt}"
    );
    assert_eq!(
        rebuilt["targets"][0]["status"],
    ...
    let current_receipt_fingerprint =
        canonical_unit_review_receipt_fingerprint(&current_receipt_source);
    write_file(
        &current_run_receipt_path,
        &format!(
            "# Unit Review Result\n**Receipt Fingerprint:** {current_receipt_fingerprint}\n{}",
            current_receipt_source.trim_start_matches("# Unit Review Result\n")
        ),
    );
...
#[test]
fn record_branch_closure_ignores_stale_execution_reentry_follow_up_when_current_truth_is_clean() {
    let (repo_dir, state_dir) =
        init_repo("record-branch-closure-ignores-stale-execution-follow-up");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    prepare_finished_single_step_finish_gate_fixture(repo, state, "no", false, &base_branch);
    ...
    let baseline = run_rust_json(
        repo,
        state,
        &["record-branch-closure", "--plan", PLAN_REL],
        "record-branch-closure baseline before stale execution follow-up injection",
    );
    write_harness_state_payload(
        repo,
        state,
    ...
    let after_stale_follow_up = run_rust_json(
        repo,
        state,
        &["record-branch-closure", "--plan", PLAN_REL],
        "record-branch-closure should ignore stale persisted execution follow-up when branch truth is current",
    );

    assert_eq!(
        after_stale_follow_up["action"], baseline["action"],
        "json: {after_stale_follow_up}"
    );
    assert_eq!(
        after_stale_follow_up["branch_closure_id"], baseline["branch_closure_id"],
        "json: {after_stale_follow_up}"
    );
    assert_eq!(
        after_stale_follow_up["code"], baseline["code"],
        "json: {after_stale_follow_up}"
    );
    assert_eq!(
        after_stale_follow_up["recommended_command"], baseline["recommended_command"],
        "json: {after_stale_follow_up}"
    );
    assert_ne!(
        after_stale_follow_up["recommended_command"],
        Value::from(format!(
            "featureforge plan execution repair-review-state --plan {PLAN_REL}"
        )),
        "record-branch-closure should not be forced back through stale repair-review-state reroutes: {after_stale_follow_up}",
    );
}
...
fn rebuild_evidence_json_output_fields() {
    ...
    for key in [
        ...
        assert!(
            ...
            "json should include top-level key {key}: {json}"
        );
    }
    for key in ["planned", "rebuilt", "manual", "failed", "noop"] {
        assert!(
            json["counts"].get(key).is_some(),
            "json counts should include {key}: {json}"
        );
    }
    ...
    for key in [
        ...
        assert!(
            ...
            "json target should include {key}: {json}"
        );
    }
    ...
    assert!(
        target["error"]
            .as_str()
            .is_some_and(|message| message.contains("projection_only")),
        "json: {json}"
    );
}
...
fn rebuild_evidence_text_failure_output() {
    {
        ...
        assert_eq!(
            stderr,
            "error error_class=\"InvalidCommandInput\" message=\"max_jobs_parallel_unsupported: rebuild-evidence currently supports only --max-jobs 1.\"\n",
            "stderr:\n{stderr}"
        );
    }

    {
        ...
        let stdout =
            String::from_utf8(output.stdout).expect("text precondition failure should be utf-8");

        assert_eq!(output.status.code(), Some(0), "stdout:\n{stdout}");
        assert!(
            stdout.starts_with(
                "summary scope=\"all\" dry_run=false planned=1 rebuilt=0 manual=1 failed=0 noop=0\n"
            ),
            "stdout:\n{stdout}"
        );
        assert!(
            stdout.contains("failure_class=\"manual_required\""),
            "stdout:\n{stdout}"
        );
    }
}
...
fn rebuild_evidence_noop_and_partial_failures() {
    ...
    {
        let (repo_dir, state_dir) =
            init_repo("plan-execution-rebuild-evidence-step4-command-failure");
        let repo = repo_dir.path();
        let state = state_dir.path();
        write_default_approved_single_step_execution_fixture(repo);
        replace_manual_verification_with_command(
...
"#
        ...
        let plan_fingerprint = sha256_hex(
            &fs::read(repo.join(PLAN_REL))
                .expect("plan should be readable for mixed batch evidence"),
        );
        let spec_fingerprint = sha256_hex(
            &fs::read(repo.join(SPEC_REL))
                .expect("spec should be readable for mixed batch evidence"),
        );
        write_file(
            &repo.join("docs/example-output.md"),
        ...
        write_file(
            &repo.join("docs/secondary-output.md"),
            "original secondary output\n",
        );
        let primary_digest = sha256_hex(
            &fs::read(repo.join("docs/example-output.md"))
                .expect("primary output should be readable"),
        );
        let secondary_digest = sha256_hex(
            &fs::read(repo.join("docs/secondary-output.md"))
                .expect("secondary output should be readable"),
        );
        let head_sha = current_head_sha(repo);
        write_file(
            &repo.join(evidence_rel_path()),
        ...
        write_file(
            &repo.join("docs/example-output.md"),
            "drifted primary output\n",
        );
        write_file(
            &repo.join("docs/secondary-output.md"),
            "drifted secondary output\n",
        );
        accept_execution_preflight(repo, state, PLAN_REL);
        ...
        assert_eq!(json["counts"]["manual"], Value::from(2), "json: {json}");
        assert_eq!(json["counts"]["rebuilt"], Value::from(0), "json: {json}");
        assert_eq!(json["counts"]["failed"], Value::from(0), "json: {json}");
        assert_eq!(
            json["targets"][0]["step_id"],
            Value::from(1),
            "json: {json}"
        );
        assert_eq!(
            json["targets"][0]["status"],
            Value::from("manual_required"),
            "json: {json}"
        );
        assert_eq!(
            json["targets"][1]["step_id"],
            Value::from(2),
            "json: {json}"
        );
        assert_eq!(
            json["targets"][1]["failure_class"],
            Value::from("manual_required"),
            "json: {json}"
        );
    }
    ...
    {
        let (repo_dir, state_dir) =
            init_repo("plan-execution-rebuild-evidence-step4-strict-manual");
        let repo = repo_dir.path();
        let state = state_dir.path();
        write_default_approved_single_step_execution_fixture(repo);
        write_file(
...
#[test]
fn rebuild_evidence_noop_preserves_receipt_only_strategy_checkpoint_drift() {
    let (repo_dir, state_dir) = init_repo("plan-execution-rebuild-evidence-noop-receipt-refresh");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_default_approved_single_step_execution_fixture(repo);
    accept_execution_preflight(repo, state, PLAN_REL);
    ...
    let status_after_preflight = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status after preflight for receipt-only drift rebuild fixture",
    );
    let execution_run_id = status_after_preflight["execution_run_id"]
        .as_str()
    ...
    let rotated_strategy_checkpoint = sha256_hex(b"receipt-only-strategy-drift");
    write_harness_state_payload(
        repo,
        state,
        &json!({
            "last_strategy_checkpoint_fingerprint": rotated_strategy_checkpoint,
            "strategy_state": "ready",
            "strategy_checkpoint_kind": "review_remediation"
        }),
    );

    let gate_before_rebuild = run_rust_json(
        repo,
        state,
        &["gate-review", "--plan", PLAN_REL],
        "gate-review before receipt-only drift rebuild refresh",
    );
    assert!(
        gate_before_rebuild["reason_codes"]
            .as_array()
            .is_some_and(|codes| {
                codes.iter().any(|code| {
                    matches!(
        ...
        "gate-review should expose receipt-only strategy drift before rebuild refresh, got {gate_before_rebuild}"
    );
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate projections only and leave receipt drift untouched",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    let gate_after_rebuild = run_rust_json(
        repo,
        state,
        &["gate-review", "--plan", PLAN_REL],
        "gate-review after projection-only noop rebuild",
    );
    assert!(
        gate_after_rebuild["reason_codes"]
            .as_array()
            .is_some_and(|codes| {
                codes.iter().any(|code| {
                    matches!(
        ...
        "projection-only noop rebuild should not repair receipt-only drift, got {gate_after_rebuild}"
    );
}
...
fn gate_review_fails_closed_when_ready_finish_fixture_has_malformed_authoritative_active_contract()
{
    ...
    write_harness_state_payload(
        ...
        &json!({
            ...
            "current_branch_closure_reviewed_state_id": format!("git_tree:{}", current_head_tree_sha(repo)),
            "current_branch_closure_contract_identity": branch_contract_identity(
                PLAN_REL,
                1,
                &repo_slug(repo),
    ...
    assert_eq!(
        gate_review["allowed"],
        Value::Bool(false),
        "gate-review should fail closed on malformed authoritative active-contract fixtures: {gate_review}",
    );
    assert_eq!(gate_review["action"], Value::from("blocked"));
    assert_eq!(
        gate_review["failure_class"],
        Value::from("MalformedExecutionState")
    );
    assert!(
        gate_review["reason_codes"].as_array().is_some_and(|codes| {
            codes
                .iter()
                .any(|code| code.as_str() == Some("authoritative_transition_state_unavailable"))
        }),
        "gate-review should surface authoritative transition state unavailability when the authoritative active contract is malformed: {gate_review}",
    );
    assert_eq!(
        gate_review["code"],
        Value::from("out_of_phase_requery_required")
    ...
    assert!(gate_review["finish_review_gate_pass_branch_closure_id"].is_null());
...
fn rebuild_evidence_noop_refuses_historical_late_gate_truth_refresh() {
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate derived projections without mutating authoritative truth",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    assert_eq!(
        status_after["final_review_state"],
        Value::from("stale"),
        "json: {status_after}"
    );
    assert_eq!(
        status_after["browser_qa_state"],
        Value::from("stale"),
        "json: {status_after}"
    );
    assert_eq!(
        status_after["release_docs_state"],
        Value::from("stale"),
        "json: {status_after}"
    );
...
#[test]
fn rebuild_evidence_noop_regenerates_stale_release_projection_from_authoritative_record() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-noop-regenerate-stale-release-projection");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, release_path) =
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before stale projection regeneration",
    );
    let sequence_before = status_before["latest_authoritative_sequence"].clone();
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate stale release projection from authoritative records",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    let status_after = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status after stale projection regeneration",
    );
    assert_eq!(
        status_after["latest_authoritative_sequence"], sequence_before,
        "projection regeneration must not mutate authoritative ordering: before={status_before} after={status_after}",
    );

    let regenerated_release_path = fs::read_dir(project_artifact_dir(repo, state))
        ...
        .max()
        .expect("projection regeneration should emit at least one release projection");
    let regenerated_release_source = fs::read_to_string(&regenerated_release_path)
        .expect("regenerated release projection should be readable");
    assert!(
        regenerated_release_source.contains("# Release Readiness Result"),
        "stale release projection regeneration should emit a release-readiness receipt"
    );
    assert!(
        regenerated_release_source.contains("**Result:** pass"),
        "stale release projection regeneration should preserve pass result semantics"
    );
    assert!(
        !regenerated_release_source.contains("Tampered projection content."),
        "stale release projection regeneration should not preserve tampered projection text"
    );
}

#[test]
fn rebuild_evidence_noop_regenerates_failed_final_review_projection_from_authoritative_record() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-noop-regenerate-failed-final-review-projection");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, _release_path) =
    ...
    let current_authoritative_path = harness_authoritative_artifact_path(
        ...
        &repo_slug(repo),
        &branch_name(repo),
        &format!("final-review-{current_fingerprint}.md"),
    );
    let failed_authoritative_source = fs::read_to_string(&current_authoritative_path)
        .expect("fixture authoritative final-review artifact should be readable")
        .replace("**Result:** pass", "**Result:** fail");
    let failed_fingerprint = sha256_hex(failed_authoritative_source.as_bytes());
    write_file(
        &harness_authoritative_artifact_path(
            state,
            &repo_slug(repo),
            &branch_name(repo),
            &format!("final-review-{failed_fingerprint}.md"),
        ),
        &failed_authoritative_source,
    );
    harness_state["current_final_review_result"] = Value::from("fail");
    harness_state["last_final_review_artifact_fingerprint"] =
        Value::from(failed_fingerprint.clone());
    harness_state["harness_phase"] = Value::from("final_review_pending");
    harness_state["finish_review_gate_pass_branch_closure_id"] = Value::Null;
    harness_state["final_review_record_history"][&current_record_id]["result"] =
        Value::from("fail");
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before failed final-review projection regeneration",
    );
    let sequence_before = status_before["latest_authoritative_sequence"].clone();
    let final_review_fingerprint = status_before["last_final_review_artifact_fingerprint"]
        .as_str()
        .expect("failed final-review record should persist an authoritative fingerprint");
    let authoritative_review_path = harness_authoritative_artifact_path(
        state,
        &repo_slug(repo),
        &branch_name(repo),
        &format!("final-review-{final_review_fingerprint}.md"),
    );
    let _authoritative_review_source = fs::read_to_string(&authoritative_review_path)
        .expect("authoritative failed final-review artifact should be readable");

    let stale_review_projection = fs::read_dir(project_artifact_dir(repo, state))
        .expect("project artifact directory should be readable")
        .filter_map(|entry| entry.ok())
        .map(|entry| entry.path())
        .filter(|path| {
            path.file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| name.contains("code-review"))
        })
        .max()
        .expect("fixture should produce a final-review projection");
    write_file(
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate failed final-review projection from authoritative records",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    let status_after = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status after failed final-review projection regeneration",
    );
    assert_eq!(
        status_after["latest_authoritative_sequence"], sequence_before,
        "projection regeneration must not mutate authoritative ordering: before={status_before} after={status_after}",
    ...
    let regenerated_review_projection = fs::read_dir(project_artifact_dir(repo, state))
        ...
        .max()
        .expect("projection regeneration should emit a final-review projection");
    let regenerated_review_source = fs::read_to_string(&regenerated_review_projection)
        .expect("regenerated final-review projection should be readable");
    assert!(
        regenerated_review_source.contains("# Code Review Result"),
        "failed final-review projection regeneration should emit a code-review receipt",
    );
    assert!(
        regenerated_review_source.contains("**Result:** fail"),
        "failed final-review projection regeneration should preserve fail result semantics",
    );
    assert!(
        !regenerated_review_source.contains("Tampered failed final-review projection."),
        "failed final-review projection regeneration should not preserve tampered projection text",
    );
}

#[test]
fn rebuild_evidence_noop_regenerates_failed_browser_qa_projection_from_authoritative_record() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-noop-regenerate-failed-browser-qa-projection");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, _release_path) =
    ...
    let current_authoritative_qa_path = harness_authoritative_artifact_path(
        ...
        &repo_slug(repo),
        &branch_name(repo),
        &format!("browser-qa-{current_qa_fingerprint}.md"),
    );
    let failed_authoritative_qa_source = fs::read_to_string(&current_authoritative_qa_path)
        .expect("fixture authoritative browser-QA artifact should be readable")
        .replace("**Result:** pass", "**Result:** fail");
    let failed_qa_fingerprint = sha256_hex(failed_authoritative_qa_source.as_bytes());
    write_file(
        &harness_authoritative_artifact_path(
            state,
            &repo_slug(repo),
            &branch_name(repo),
            &format!("browser-qa-{failed_qa_fingerprint}.md"),
        ),
        &failed_authoritative_qa_source,
    );
    harness_state["current_qa_result"] = Value::from("fail");
    harness_state["last_browser_qa_artifact_fingerprint"] =
        Value::from(failed_qa_fingerprint.clone());
    harness_state["harness_phase"] = Value::from("qa_pending");
    harness_state["finish_review_gate_pass_branch_closure_id"] = Value::Null;
    harness_state["browser_qa_record_history"][&current_qa_record_id]["result"] =
        Value::from("fail");
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before failed browser-QA projection regeneration",
    );
    let sequence_before = status_before["latest_authoritative_sequence"].clone();
    let qa_fingerprint = status_before["last_browser_qa_artifact_fingerprint"]
        .as_str()
        .expect("failed browser-QA record should persist an authoritative fingerprint");
    ...
    let _authoritative_qa_source = fs::read_to_string(&authoritative_qa_path)
        .expect("authoritative failed browser-QA artifact should be readable");

    let stale_qa_projection = fs::read_dir(project_artifact_dir(repo, state))
        .expect("project artifact directory should be readable")
        .filter_map(|entry| entry.ok())
        .map(|entry| entry.path())
        .filter(|path| {
        ...
        .max()
        .expect("fixture should produce a browser-QA projection");
    write_file(
        &stale_qa_projection,
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate failed browser-QA projection from authoritative records",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    let status_after = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status after failed browser-QA projection regeneration",
    );
    assert_eq!(
        status_after["latest_authoritative_sequence"], sequence_before,
        "projection regeneration must not mutate authoritative ordering: before={status_before} after={status_after}",
    ...
    let regenerated_qa_projection = fs::read_dir(project_artifact_dir(repo, state))
        .expect("project artifact directory should be readable")
        .filter_map(|entry| entry.ok())
        .map(|entry| entry.path())
        .filter(|path| {
            path.file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| {
                    name.starts_with("featureforge-") && name.contains("test-outcome")
                })
        })
        .max()
        .expect("projection regeneration should emit a browser-QA projection");
    let regenerated_qa_source = fs::read_to_string(&regenerated_qa_projection)
        .expect("regenerated browser-QA projection should be readable");
    assert!(
        regenerated_qa_source.contains("# QA Result"),
        "failed browser-QA projection regeneration should emit a QA receipt",
    );
    assert!(
        regenerated_qa_source.contains("**Result:** fail"),
        "failed browser-QA projection regeneration should preserve fail result semantics",
    );
    assert!(
        !regenerated_qa_source.contains("Tampered failed browser QA projection."),
        "failed browser-QA projection regeneration should not preserve tampered projection text",
    );
    assert!(
        regenerated_qa_source.contains("**Source Test Plan:**"),
        "regenerated failed browser-QA projection should preserve source test-plan binding"
    );
}
...
fn rebuild_evidence_noop_regenerates_release_projection_when_authoritative_release_artifact_is_missing()
 {
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before missing-authoritative-release projection regeneration",
    );
    let authoritative_release_fingerprint = status_before["last_release_docs_artifact_fingerprint"]
        .as_str()
        .expect("status should expose the current authoritative release fingerprint");
    ...
    let status_after = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status after missing-authoritative-release projection regeneration",
    );
    assert_eq!(
        status_after["latest_authoritative_sequence"],
        status_before["latest_authoritative_sequence"],
        "projection regeneration must not mutate authoritative sequence ordering"
    );
    ...
    assert!(
        regenerated_release_source.contains("# Release Readiness Result"),
        "regenerated release projection should remain a release-readiness receipt"
    );
    assert!(
        regenerated_release_source.contains("**Result:** pass"),
...
#[test]
fn rebuild_evidence_noop_regenerates_final_review_projection_when_reviewer_projection_is_tampered()
{
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-noop-tampered-reviewer-projection");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, _release_path) =
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before tampered-reviewer projection regeneration",
    );
    let final_review_fingerprint = status_before["last_final_review_artifact_fingerprint"]
        .as_str()
    ...
    let reviewer_artifact_path = final_review_source
        .lines()
        .find_map(|line| {
            line.trim()
    ...
    let regenerated_review_source = fs::read_to_string(&regenerated_review_projection)
        .expect("regenerated code-review projection should be readable");
    assert!(
        regenerated_review_source.contains("# Code Review Result"),
        "regenerated code-review projection should remain a final-review receipt"
    );
    assert!(
...
#[test]
fn rebuild_evidence_noop_regenerates_reviewer_projection_when_reviewer_projection_is_missing() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-noop-missing-reviewer-projection");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, _release_path) =
    ...
    let status_before = run_rust_json(
        ...
        state,
        &["status", "--plan", PLAN_REL],
        "status before missing-reviewer projection regeneration",
    );
    let final_review_fingerprint = status_before["last_final_review_artifact_fingerprint"]
        .as_str()
        .expect("status should expose the current final-review fingerprint");
    ...
    let regenerated_reviewer_projection = fs::read_dir(project_artifact_dir(repo, state))
        ...
        .filter(|path| {
            path.file_name()
                .and_then(|name| name.to_str())
                .is_some_and(|name| {
                    name.starts_with("featureforge-")
                        && name.contains("independent-review")
                        && name.ends_with(".md")
                })
        })
        .max()
        .expect("projection regeneration should emit a reviewer projection artifact");
    let regenerated_reviewer_source = fs::read_to_string(&regenerated_reviewer_projection)
        .expect("regenerated reviewer projection should be readable");
    assert!(
...
#[test]
fn rebuild_evidence_projection_regeneration_prefers_current_over_superseded_release_record() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-rebuild-evidence-regeneration-prefers-current-release-record");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (_test_plan_path, _qa_path, _review_path, _release_path) =
    ...
    let mut authoritative_state: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("authoritative state should be readable"),
    )
    .expect("authoritative state should be valid JSON");
    let current_record_id = authoritative_state["current_release_readiness_record_id"]
        .as_str()
        .expect("fixture should expose a current release-readiness record id")
        .to_owned();
    let current_record =
        authoritative_state["release_readiness_record_history"][&current_record_id].clone();
    let current_fingerprint = current_record["release_docs_fingerprint"]
        .as_str()
        .expect("fixture should expose current release fingerprint")
        .to_owned();
    let superseded_release_source = "# Release Readiness Result\n**Result:** pass\n\n## Summary\n- superseded historical release readiness projection.\n";
    let superseded_fingerprint = sha256_hex(superseded_release_source.as_bytes());
    write_file(
        &harness_authoritative_artifact_path(
            state,
            &repo_slug(repo),
            &branch_name(repo),
            &format!("release-docs-{superseded_fingerprint}.md"),
        ),
        superseded_release_source,
    );

    let mut superseded_record = current_record.clone();
    superseded_record["record_id"] = Value::from("release-readiness-record-superseded");
    superseded_record["record_status"] = Value::from("superseded");
    superseded_record["record_sequence"] = Value::from(0_u64);
    superseded_record["release_docs_fingerprint"] = Value::from(superseded_fingerprint.clone());
    superseded_record["summary"] = Value::from("Superseded release record should stay historical.");
    authoritative_state["release_readiness_record_history"]["release-readiness-record-superseded"] =
        superseded_record;
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence should regenerate release projection from the current release-readiness record",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    let current_authoritative_release_source =
        fs::read_to_string(harness_authoritative_artifact_path(
            ...
            &repo_slug(repo),
            &branch_name(repo),
            &format!("release-docs-{current_fingerprint}.md"),
        ))
        .expect("current authoritative release artifact should be readable");
    let regenerated_release_path = fs::read_dir(project_artifact_dir(repo, state))
        .expect("project artifact directory should be readable")
        .filter_map(|entry| entry.ok())
        .map(|entry| entry.path())
    ...
    let regenerated_release_source = fs::read_to_string(&regenerated_release_path)
        .expect("regenerated release projection should be readable");
    assert_eq!(
        regenerated_release_source, current_authoritative_release_source,
        "projection regeneration must use the current release-readiness record, not superseded history"
    );
}
...
fn rebuild_evidence_noop_refuses_historical_final_review_refresh_without_release_readiness() {
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate derived projections even when release-readiness artifacts are absent",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    assert!(
        status_after["last_final_review_artifact_fingerprint"].is_null(),
        "json: {status_after}"
    );
    assert!(
        status_after["last_release_docs_artifact_fingerprint"].is_null(),
        "json: {status_after}"
    );
}
...
fn rebuild_evidence_noop_not_required_without_branch_test_plan_skips_missing_test_plan_precondition()
 {
    ...
    let (
        _authoritative_test_plan_path,
        _qa_path,
        _authoritative_review_path,
        _authoritative_release_path,
    ) = prepare_finished_single_step_finish_gate_fixture(repo, state, "no", false, &base_branch);
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should not fail on missing branch test-plan when approved-plan QA is not required",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
...
fn rebuild_evidence_noop_preserves_late_gate_head_drift_block_without_rebinding() {
    ...
    let safe_branch = normalize_identifier(&branch_name(repo));
    let review_path = project_artifact_dir(repo, state).join(format!(
        "tester-{safe_branch}-code-review-20260322-171100.md"
    ));
    let test_plan_path = project_artifact_dir(repo, state)
        .join(format!("tester-{safe_branch}-test-plan-20260322-170500.md"));
    let release_path = project_artifact_dir(repo, state).join(format!(
        "tester-{safe_branch}-release-readiness-20260322-171500.md"
    ));
    ...
    assert_ne!(
        ...
        "fixture should advance HEAD without changing proven files"
    );
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate projections without rebinding authoritative late-gate truth after rebase-only drift",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
    ...
    assert!(
        !review_source.contains(&format!("**Head SHA:** {current_head}")),
        "review:\n{review_source}"
    );
...
fn rebuild_evidence_noop_refuses_late_gate_artifact_rebinding_after_replayed_rebase_changes() {
    ...
    let rebuild = run_rust_json(
        ...
        state,
        &["rebuild-evidence", "--plan", PLAN_REL, "--json"],
        "rebuild-evidence noop should regenerate projections without rebinding authoritative late-gate truth after replayed rebase changes",
    );
    assert_eq!(
        rebuild["counts"]["planned"],
        Value::from(0),
...
#[test]
fn late_stage_recording_equivalent_reruns_keep_direct_release_and_final_idempotent_while_qa_requeries()
 {
    ...
    let qa = run_rust_json(
        ...
        "record-qa equivalent rerun after workflow reroute",
    );
    assert_eq!(qa["action"], Value::from("blocked"));
...
fn late_stage_primitive_rerun_equivalence_requires_matching_supplied_bindings() {
    ...
    let release_digest_before = authoritative_state_digest(repo, state);
    ...
    assert_eq!(
        authoritative_state_digest(repo, state),
        release_digest_before,
        "record-release-readiness branch-closure mismatch must fail before mutating authoritative state"
    );
    let final_digest_before = authoritative_state_digest(repo, state);
    ...
    assert_eq!(
        authoritative_state_digest(repo, state),
        final_digest_before,
        "record-final-review branch-closure mismatch must fail before mutating authoritative state"
    );
}

#[test]
fn late_stage_final_review_recording_does_not_require_generated_code_review_receipt() {
    let (repo_dir, state_dir) =
        init_repo("plan-execution-late-stage-final-review-no-generated-receipt-gate");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let _ =
    ...
    let release_summary_path = repo.join("release-readiness-no-generated-receipt-gate-summary.md");
    write_file(
        &release_summary_path,
        "Release-readiness artifact fixture for finish-gate coverage.\n",
    );
    let _ = run_rust_json(
        ...
        "advance-late-stage release-readiness equivalence setup for generated receipt gate test",
    );
    let _ = run_rust_json(
        repo,
        state,
        ...
        "record-release-readiness equivalence setup for generated receipt gate test",
    );

    let final_summary_path = repo.join("final-review-no-generated-receipt-gate-summary.md");
    write_file(
        &final_summary_path,
        "Final whole-diff review artifact fixture for finish-gate coverage.\n",
    );
    let baseline = run_rust_json(
        ...
        &[
            ...
            final_summary_path
                .to_str()
                .expect("final review summary path should be utf-8"),
        ],
        "advance-late-stage final-review baseline before decoy generated receipt",
    );
    assert_eq!(
        baseline["action"],
        Value::from("already_current"),
    ...
    assert!(baseline["code"].is_null());

    let artifact_dir = project_artifact_dir(repo, state);
    let decoy_code_review = artifact_dir.join("decoy-code-review-99999999-999999.md");
    write_file(
        &decoy_code_review,
        &format!(
            "# Code Review Result\n**Review Stage:** featureforge:requesting-code-review\n**Branch:** {}\n**Generated By:** featureforge:requesting-code-review\n\n## Summary\n- intentionally malformed decoy that must not block authoritative late-stage mutation.\n",
            branch_name(repo)
        ),
    );

    let result = run_rust_json(
        repo,
        state,
        &[
            "advance-late-stage",
        ...
        "advance-late-stage final-review should use authoritative late-stage truth instead of generated code-review receipt presence",
    );
