...
#[test]
fn gate_finish_blocks_resolution_required_worktree_lane() {
    let (repo_dir, state_dir) = init_repo("plan-execution-gate-finish-resolution-required-lane");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (test_plan_path, qa_path, review_path, release_path) =
    ...
    append_active_worktree_lease_binding(
        ...
        json!({
            ...
            "review_receipt_fingerprint": receipt_fingerprint,
            "review_receipt_artifact_path": receipt_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_path": "resolution-required-files.json",
            "diff_stat": "2 files changed, 5 insertions(+), 1 deletion(-)",
    ...
    let gate_finish = run_rust_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate finish with resolution_required worktree lane",
    );
    ...
    assert!(
        gate_finish["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|value| value == &Value::String(String::from("worktree_lane_resolution_required")))),
        "gate-finish should fail closed when a lane remains in resolution_required state, got {gate_finish}"
    );
}

#[test]
fn gate_finish_blocks_merge_ready_worktree_lane_missing_manifest_artifact() {
    let (repo_dir, state_dir) = init_repo("plan-execution-gate-finish-merge-ready-missing-manifest");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (test_plan_path, qa_path, review_path, release_path) =
    ...
    let (lease_path, lease_fingerprint) = write_worktree_lease_artifact_for_run_identity_with_fingerprint!(
        ...
        "cleaned",
        Some(&current_head),
        "merge-ready-missing-manifest",
        true,
    );
    let reviewed_worktree_string = state
        .join("worktrees")
        .join("merge-ready-missing-manifest")
        .display()
        .to_string();
    let (receipt_path, receipt_fingerprint) = write_unit_review_receipt_artifact_for_lease!(
        repo,
        ...
        &lease_fingerprint,
        &approved_task_packet_fingerprint,
        "unit-merge-ready-missing-manifest",
        PLAN_REL,
        1,
        &reviewed_worktree_string,
        &current_head,
    ...
    let patch_path = harness_authoritative_artifact_path(
        state,
        &repo_slug(repo),
        &branch_name(repo),
        "merge-ready.patch",
    );
    write_file(&patch_path, "diff --git a/file b/file\n");
    let patch_fingerprint = sha256_hex(
        fs::read(&patch_path)
            .expect("patch should be readable")
            .as_slice(),
    );
    append_active_worktree_lease_binding(
        repo,
        ...
        json!({
            ...
            "review_receipt_fingerprint": receipt_fingerprint,
            "review_receipt_artifact_path": receipt_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_path": "merge-ready-files.json",
            "changed_files_manifest_fingerprint": "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc",
            "diff_stat": "1 file changed, 2 insertions(+)",
            "harvested_patch_artifact_path": patch_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "harvested_patch_fingerprint": patch_fingerprint,
    ...
    let gate_finish = run_rust_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate finish with merge_ready worktree lane missing manifest artifact",
    );
    ...
    assert!(
        gate_finish["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|value| value == &Value::String(String::from("worktree_lane_manifest_missing")))),
        "gate-finish should fail closed when a merge-ready lane is missing its manifest artifact, got {gate_finish}"
    );
}

#[test]
fn gate_finish_blocks_merge_ready_worktree_lane_with_patch_fingerprint_mismatch() {
    let (repo_dir, state_dir) = init_repo("plan-execution-gate-finish-merge-ready-patch-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (test_plan_path, qa_path, review_path, release_path) =
    ...
    let (lease_path, lease_fingerprint) = write_worktree_lease_artifact_for_run_identity_with_fingerprint!(
        ...
        "cleaned",
        Some(&current_head),
        "merge-ready-patch-mismatch",
        true,
    );
    let reviewed_worktree_string = state
        .join("worktrees")
        .join("merge-ready-patch-mismatch")
        .display()
        .to_string();
    let (receipt_path, receipt_fingerprint) = write_unit_review_receipt_artifact_for_lease!(
        repo,
        ...
        &lease_fingerprint,
        &approved_task_packet_fingerprint,
        "unit-merge-ready-patch-mismatch",
        PLAN_REL,
        1,
        &reviewed_worktree_string,
        &current_head,
    ...
    let manifest_path = harness_authoritative_artifact_path(
        state,
        &repo_slug(repo),
        &branch_name(repo),
        "merge-ready-files.json",
    );
    write_file(&manifest_path, "[\"src/lib.rs\"]\n");
    let manifest_fingerprint = sha256_hex(
    ...
    append_active_worktree_lease_binding(
        ...
        json!({
            ...
            "review_receipt_fingerprint": receipt_fingerprint,
            "review_receipt_artifact_path": receipt_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_path": manifest_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_fingerprint": manifest_fingerprint,
            "diff_stat": "1 file changed, 2 insertions(+)",
            "harvested_patch_artifact_path": patch_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "harvested_patch_fingerprint": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            "lane_terminal_state": "merge_ready"
        }),
    );
    ...
    let gate_finish = run_rust_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate finish with merge_ready worktree lane patch fingerprint mismatch",
    );
    ...
    assert!(
        gate_finish["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|value| value == &Value::String(String::from("worktree_lane_patch_fingerprint_mismatch")))),
        "gate-finish should fail closed when a merge-ready lane patch fingerprint drifts, got {gate_finish}"
    );
}

#[test]
fn gate_finish_blocks_merge_ready_worktree_lane_with_manifest_fingerprint_mismatch() {
    let (repo_dir, state_dir) = init_repo("plan-execution-gate-finish-merge-ready-manifest-mismatch");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (test_plan_path, qa_path, review_path, release_path) =
    ...
    let (lease_path, lease_fingerprint) = write_worktree_lease_artifact_for_run_identity_with_fingerprint!(
        ...
        "cleaned",
        Some(&current_head),
        "merge-ready-manifest-mismatch",
        true,
    );
    let reviewed_worktree_string = state
        .join("worktrees")
        .join("merge-ready-manifest-mismatch")
        .display()
        .to_string();
    let (receipt_path, receipt_fingerprint) = write_unit_review_receipt_artifact_for_lease!(
        repo,
        ...
        &lease_fingerprint,
        &approved_task_packet_fingerprint,
        "unit-merge-ready-manifest-mismatch",
        PLAN_REL,
        1,
        &reviewed_worktree_string,
        &current_head,
    ...
    append_active_worktree_lease_binding(
        ...
        json!({
            ...
            "review_receipt_artifact_path": receipt_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_path": manifest_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "changed_files_manifest_fingerprint": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
            "diff_stat": "1 file changed, 2 insertions(+)",
            "harvested_patch_artifact_path": patch_path.file_name().and_then(|value| value.to_str()).unwrap_or_default(),
            "harvested_patch_fingerprint": patch_fingerprint,
            "lane_terminal_state": "merge_ready"
    ...
    let gate_finish = run_rust_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate finish with merge_ready worktree lane manifest fingerprint mismatch",
    );
    ...
    assert!(
        gate_finish["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|value| value == &Value::String(String::from("worktree_lane_manifest_fingerprint_mismatch")))),
        "gate-finish should fail closed when a merge-ready lane manifest fingerprint drifts, got {gate_finish}"
    );
}

#[test]
fn write_authoritative_worktree_lease_artifact_publishes_active_index_entry() {
    let (repo_dir, state_dir) = init_repo("plan-execution-worktree-lease-helper-publishes-index");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_plan(repo, "featureforge:executing-plans");
    ...
    let branch = branch_name(repo);
    let safe_branch = normalize_identifier(&branch);
    let execution_run_id = format!("run-{safe_branch}-helper");
    let chunk_id = format!("chunk-{safe_branch}-helper");
    write_harness_state_payload(
        repo,
        state,
        &json!({
    ...
    let lease_json = json!({
        ...
        "source_plan_path": PLAN_REL,
        "source_plan_revision": 1,
        "execution_unit_id": "unit-helper-publish-index",
        "source_branch": branch,
        "authoritative_integration_branch": branch,
        "worktree_path": state.join("worktrees").join("helper-publish-index").display().to_string(),
        "repo_state_baseline_head_sha": current_head,
        "repo_state_baseline_worktree_fingerprint": "2222222222222222222222222222222222222222222222222222222222222222",
    ...
    .expect("lease fixture should deserialize");
    let artifact_path = write_authoritative_worktree_lease_artifact(&runtime, &lease)
        .expect("authoritative worktree lease should publish");

    let overlay = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status after authoritative worktree lease publish");
    let bindings = overlay["active_worktree_lease_bindings"]
        .as_array()
        .expect("status should expose active worktree lease bindings after helper publish");
    assert!(
        bindings.iter().any(|binding| {
            binding["lease_fingerprint"] == Value::String(lease_fingerprint.clone())
        ...
        "authoritative worktree lease helper should upsert an active lease index entry"
    );
    let expected_artifact_name = format!(
        "worktree-lease-{safe_branch}-{}-{}.json",
        lease.execution_run_id,
        lease.execution_context_key,
    );
    assert_eq!(
        artifact_path.file_name().and_then(|value| value.to_str()),
        Some(expected_artifact_name.as_str())
    );
}

#[test]
fn complete_in_audit_mode_records_out_of_slice_writes_in_evidence() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-slice-fence-audit");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    write_file(&repo.join("docs/example-output.md"), "declared output\n");
    write_file(&repo.join("src/out-of-slice.rs"), "pub const OUT_OF_SLICE: &str = \"x\";\n");
    accept_execution_preflight(repo, state, PLAN_REL);

    let before_begin = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before audit begin");
    run_rust_json(
        repo,
        state,
        &[
            "begin", "--plan", PLAN_REL, "--task", "1", "--step", "1",
            "--execution-mode", "featureforge:executing-plans",
            "--expect-execution-fingerprint",
            before_begin["execution_fingerprint"].as_str().expect("fingerprint"),
        ],
        "begin audit fence step",
    );
    let before_complete = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before audit complete");
    run_rust_json(
        repo,
        state,
        &[
            "complete", "--plan", PLAN_REL, "--task", "1", "--step", "1",
            "--source", "featureforge:executing-plans",
            "--claim", "Completed with one out-of-slice write in audit mode.",
            "--manual-verify-summary", "Observed audit-only fence behavior.",
            "--file", "docs/example-output.md",
            "--file", "src/out-of-slice.rs",
            "--expect-execution-fingerprint",
            before_complete["execution_fingerprint"].as_str().expect("fingerprint"),
        ],
        "complete audit fence step",
    );

    let evidence = fs::read_to_string(repo.join("docs/featureforge/execution-evidence/2026-03-17-example-execution-plan-r1-evidence.md"))
        .expect("execution evidence should be readable after audit completion");
    assert!(
        evidence.contains("**Task Slice Fence Outcome:** detected"),
        "audit mode should record detected out-of-slice writes in execution evidence"
    );
}

#[test]
fn complete_in_guarded_mode_blocks_subagent_out_of_slice_writes_without_override() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-slice-fence-guarded");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:subagent-driven-development");
    write_file(&repo.join("docs/example-output.md"), "declared output\n");
    write_file(&repo.join("src/out-of-slice.rs"), "pub const OUT_OF_SLICE: &str = \"x\";\n");
    accept_execution_preflight(repo, state, PLAN_REL);
    write_harness_state_payload(repo, state, &json!({ "task_slice_fence_mode": "guarded" }));

    let before_begin = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before guarded begin");
    run_rust_json(
        repo,
        state,
        &[
            "begin", "--plan", PLAN_REL, "--task", "1", "--step", "1",
            "--execution-mode", "featureforge:subagent-driven-development",
            "--expect-execution-fingerprint",
            before_begin["execution_fingerprint"].as_str().expect("fingerprint"),
        ],
        "begin guarded fence step",
    );
    let before_complete = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before guarded complete");
    let failure = parse_failure_json(
        &run(
            {
                let mut command = Command::cargo_bin("featureforge").expect("binary should exist");
                command.current_dir(repo).env("FEATUREFORGE_STATE_DIR", state).args([
                    "plan", "execution", "complete", "--plan", PLAN_REL,
                    "--task", "1", "--step", "1",
                    "--source", "featureforge:subagent-driven-development",
                    "--claim", "Attempted guarded out-of-slice write.",
                    "--manual-verify-summary", "Guarded mode should fail closed.",
                    "--file", "docs/example-output.md",
                    "--file", "src/out-of-slice.rs",
                    "--expect-execution-fingerprint",
                    before_complete["execution_fingerprint"].as_str().expect("fingerprint"),
            ...
            "complete guarded fence step",
        ),
        "complete guarded fence step",
    );

    assert_eq!(failure["error_class"], "InvalidRepoPath");
    assert!(
        failure["message"].as_str().is_some_and(|value| value.contains("out-of-slice")),
        "guarded mode should fail closed on out-of-slice writes without an override, got {failure}"
    );
}

#[test]
fn complete_in_full_mode_accepts_explicit_override_and_updates_rollout_counters() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-slice-fence-full-override");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:executing-plans");
    ...
    write_harness_state_payload(
        ...
        state,
        &json!({
            "task_slice_fence_mode": "full",
            "blocked_write_override_count": 0,
            "rollout_window_run_count": 10,
            "representative_parallel_run_validation_count": 2
        }),
    );

    let before_begin = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before full begin");
    run_rust_json(
        repo,
        state,
        &[
        ...
        "begin full fence step",
    );
    let before_complete = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before full complete");
    let status = run_rust_json(
        repo,
        state,
        &[
            "complete", "--plan", PLAN_REL, "--task", "1", "--step", "1",
            "--source", "featureforge:executing-plans",
            "--claim", "Completed with explicit full-mode override.",
            "--manual-verify-summary", "Override flow accepted.",
            "--file", "docs/example-output.md",
            "--file", "src/out-of-slice.rs",
            "--fence-override-reason", "Explicitly approved task-slice expansion for rollout validation.",
            "--expect-execution-fingerprint",
            before_complete["execution_fingerprint"].as_str().expect("fingerprint"),
        ],
        "complete full fence step",
    );

    assert_eq!(status["task_slice_fence_mode"], "full");
    assert_eq!(status["blocked_write_override_count"], 1);
    assert_eq!(status["rollout_window_run_count"], 11);
    assert!(
        status["blocked_write_override_rate"]
            .as_f64()
            .is_some_and(|value| (value - (1.0 / 11.0)).abs() < f64::EPSILON),
        "full mode should recompute blocked_write_override_rate after override capture"
    );
}

#[test]
fn complete_can_record_fence_false_positive_and_parallel_validation_counters() {
    let (repo_dir, state_dir) = init_repo("plan-execution-task-slice-fence-rollout-observations");
    let repo = repo_dir.path();
    let state = state_dir.path();
    write_approved_spec(repo);
    write_single_step_plan(repo, "featureforge:subagent-driven-development");
    ...
    let before_begin = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before rollout-observation begin");
    run_rust_json(
        repo,
        state,
        &[
        ...
        "begin rollout-observation fence step",
    );
    let before_complete = run_rust_json(repo, state, &["status", "--plan", PLAN_REL], "status before rollout-observation complete");
    let status = run_rust_json(
        repo,
        state,
        &[
            "complete", "--plan", PLAN_REL, "--task", "1", "--step", "1",
            "--source", "featureforge:subagent-driven-development",
            "--claim", "Completed an audit-mode out-of-slice write flagged as a false positive during representative parallel validation.",
            "--manual-verify-summary", "Observed representative parallel validation behavior.",
            "--file", "docs/example-output.md",
            "--file", "src/out-of-slice.rs",
            "--fence-false-positive",
            "--representative-parallel-validation",
            "--expect-execution-fingerprint",
            before_complete["execution_fingerprint"].as_str().expect("fingerprint"),
        ],
        "complete rollout-observation fence step",
    );

    assert_eq!(status["fence_false_positive_count"], 1);
    assert_eq!(status["rollout_window_run_count"], 1);
    assert_eq!(status["representative_parallel_run_validation_count"], 1);
    assert!(
        status["fence_false_positive_rate"]
            .as_f64()
            .is_some_and(|value| (value - 1.0).abs() < f64::EPSILON),
        "fence false-positive rate should recompute from runtime-owned counters"
    );
}
...
#[test]
fn write_authoritative_unit_review_receipt_artifact_publishes_serial_lease_binding() {
    let (repo_dir, state_dir) = init_repo("plan-execution-serial-unit-review-publishes-lease");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let base_branch = branch_name(repo);
    let (test_plan_path, qa_path, review_path, release_path) =
    ...
    let (execution_run_id, _chunk_id) = current_authoritative_run_identity(repo, state);
    let current_head = current_head_sha(repo);
    let execution_unit_id = "unit-serial-publish";
    let approved_task_packet_fingerprint = expected_packet_fingerprint(repo, 1, 1);
    let reviewed_worktree = fs::canonicalize(repo)
    ...
    let state_json: Value = serde_json::from_str(
        &fs::read_to_string(harness_state_file_path(repo, state))
            .expect("harness state should be readable for serial receipt fixture"),
    )
    .expect("harness state should remain valid json");
    let authoritative_sequence = state_json
        .get("latest_authoritative_sequence")
        .and_then(Value::as_u64)
        .expect("authoritative downstream fixture should set latest_authoritative_sequence")
        + 1;
    let active_contract_fingerprint = state_json
        .get("active_contract_fingerprint")
        .and_then(Value::as_str)
    ...
    .expect("authoritative helper should write serial unit-review receipt");

    let status = run_rust_json(
        repo,
        state,
        &["status", "--plan", PLAN_REL],
        "status after serial unit-review receipt authoritative publish",
    );
    let bindings = status["active_worktree_lease_bindings"]
        .as_array()
        .expect("serial receipt writer should publish active worktree lease bindings");
    let binding = bindings
        .iter()
        .find(|candidate| candidate["lease_fingerprint"] == Value::String(lease_fingerprint.clone()))
        .expect("serial receipt writer should publish the matching lease binding");
    let lease_artifact_path = binding["lease_artifact_path"]
        .as_str()
        .expect("serial receipt writer should publish a lease artifact path");
    assert_eq!(
        binding["review_receipt_artifact_path"],
        Value::String(
            receipt_path
                .file_name()
                .and_then(|value| value.to_str())
    ...
    assert!(
        ...
        .exists(),
        "serial receipt writer should publish the authoritative lease artifact",
    );
}
