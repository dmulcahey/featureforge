...
fn try_run_workflow_emission_direct(
...
) -> Option<DirectWorkflowEmission> {
    {
        ...
        if matches!(workflow_cli.command, WorkflowCommand::PlanFidelity(_)) {
        ...
        let emission =
            match workflow_cli.command {
                ...
                WorkflowCommand::Status(args) => {
                    match status::WorkflowRuntime::discover_for_state_dir(repo, state) {
                        Ok(mut workflow) => {
                            ...
                            DirectWorkflowEmission::Text(
                                route.map(|route| render_workflow_status_summary(&route)),
                            )
                        }
                ...
                WorkflowCommand::Phase(args) if args.json => {
                    DirectWorkflowEmission::Json(serialize_json(
                        load_read_only_runtime()
                ...
                WorkflowCommand::Doctor(args) if args.json => {
                ...
                WorkflowCommand::RecordPivot(args) if args.json => DirectWorkflowEmission::Json(
                    serialize_json(load_runtime().and_then(|runtime| {
                ...
                WorkflowCommand::Preflight(args) if args.json => {
                    DirectWorkflowEmission::Json(serialize_json(
                        load_runtime()
                            .and_then(|runtime| operator::preflight_for_runtime(&runtime, &args)),
                    ))
                }
                WorkflowCommand::Gate(gate_cli) => match gate_cli.command {
                    WorkflowGateCommand::Review(args) if args.json => {
                        DirectWorkflowEmission::Json(serialize_json(load_runtime().and_then(
                            |runtime| operator::gate_review_for_runtime(&runtime, &args),
                        )))
                    }
                    WorkflowGateCommand::Finish(args) if args.json => {
                        DirectWorkflowEmission::Json(serialize_json(load_runtime().and_then(
                            |runtime| operator::gate_finish_for_runtime(&runtime, &args),
                        )))
                    }
                    _ => return None,
                },
...
fn serialize_resolved_route(route: status::WorkflowRoute) -> Vec<u8> {
    ...
    let mut value = serde_json::to_value(route)
        .expect_or_abort("workflow resolve route should serialize to JSON");
    let object = value
        .as_object_mut()
        .expect_or_abort("workflow resolve route should serialize as a JSON object");
    object.insert(String::from("outcome"), Value::from("resolved"));
    object.insert(
        String::from("manifest_source_path"),
        Value::from(manifest_source_path),
    );
    json_line(&value).expect_or_abort("workflow resolve route should serialize to JSON")
}

fn serialize_json<T: Serialize>(value: Result<T, JsonFailure>) -> Result<Vec<u8>, JsonFailure> {
    value.map(|value| {
        json_line(&value).expect_or_abort("direct workflow output should serialize to JSON")
    })
}
...
fn json_output_result(result: Result<Vec<u8>, JsonFailure>) -> Output {
    match result {
        ...
        Err(failure) => output_with_code(
            1,
            Vec::new(),
            json_line(&failure).expect_or_abort("direct workflow json failure should serialize"),
        ),
    }
}
...
fn workflow_resolve_output_result(result: Result<status::WorkflowRoute, JsonFailure>) -> Output {
    match result {
        ...
        Err(failure) => output_with_code(
            ...
            .expect_or_abort("direct workflow resolve failure should serialize"),
        ),
    }
}
