//! Runtime instruction contracts integration/benchmark crate.
...
fn assert_file_contains_in_order(path: impl AsRef<Path>, needles: &[&str]) {
    ...
    for needle in needles {
        let relative_index = content[previous_index..].find(needle).unwrap_or_else(|| {
            featureforge::abort!("{} should contain {needle:?}", path_ref.display())
        });
...
fn parse_runtime_gate_state(block: &str, field: &str) -> String {
    ...
    let start = block.find(&needle).unwrap_or_else(|| {
        featureforge::abort!("runtime precedence row should contain {needle:?}: {block}")
    });
    ...
    if rest.starts_with("Blocked") {
        ...
        featureforge::abort!(
            "runtime precedence row should use Ready/Blocked for {field}: {block}"
        );
    }
}
...
fn parse_runtime_quoted_field(block: &str, field: &str) -> String {
    ...
    let end = rest.find('"').unwrap_or_else(|| {
        featureforge::abort!("runtime precedence row should close quoted field {field:?}: {block}")
    });
    rest[..end].to_owned()
}

fn parse_runtime_late_stage_rows(source: &str) -> Vec<LateStageRuntimeRow> {
    let table_start = source.find("const PRECEDENCE_ROWS").unwrap_or_else(|| {
        featureforge::abort!("runtime precedence source should define PRECEDENCE_ROWS")
    });
    let table_source = &source[table_start..];
    let table_end = table_source.find("];").unwrap_or_else(|| {
        featureforge::abort!("runtime precedence source should close PRECEDENCE_ROWS with ];")
    });
...
fn expected_phase_action_and_skill(phase: &str) -> (&'static str, &'static str) {
    match phase {
        ...
        _ => featureforge::abort!("unexpected late-stage phase in precedence row: {phase}"),
    }
}

fn generated_skill_doc_paths() -> Vec<PathBuf> {
    fs::read_dir(repo_root().join("skills"))
        .expect_or_abort("skills dir should be readable")
...
fn write_executable(path: &Path, body: &str) {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).expect_or_abort("executable parent dir should exist");
    }
    fs::write(path, body).expect_or_abort("executable should be writable");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(path, fs::Permissions::from_mode(0o755))
            .expect_or_abort("executable should stay executable");
    }
}
...
fn write_logging_packaged_runtime(
    ...
    if let Some(parent) = log_path.parent() {
        fs::create_dir_all(parent).expect_or_abort("log parent dir should exist");
    }
    write_executable(
        packaged_bin,
        &format!(
...
fn make_runtime_root(dir: &Path) {
    fs::create_dir_all(dir.join("bin")).expect_or_abort("runtime bin dir should exist");
    let launcher_source = concat!(
        "#!/usr/bin/env bash\ncase \"${",
        "1:-}\" in\n  repo)\n    if [ \"${",
        "2:-}\" = \"runtime-root\" ] && [ \"${",
    ...
    fs::write(dir.join("bin/featureforge"), launcher_source)
        .expect_or_abort("runtime launcher should be writable");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(
        ...
        .expect_or_abort("runtime launcher should be executable");
    }
    fs::write(dir.join("VERSION"), "1.0.0\n").expect_or_abort("VERSION should be writable");
}
...
fn repo_checkout_ships_the_canonical_runtime_launcher() {
    ...
    #[cfg(unix)]
    {
        ...
        let mode = fs::metadata(&launcher)
            .expect_or_abort("repo-local launcher should be stat-able")
            .permissions()
            .mode();
        assert!(
            mode & 0o111 != 0,
...
fn repo_checkout_canonical_launcher_runs_without_recursive_fallback() {
    ...
    let output = AssertCommand::new(launcher)
        ...
        .arg("--version")
        .output()
        .expect_or_abort("repo-local launcher --version should run");
...
fn repo_checkout_canonical_launcher_supports_runtime_root_helper_contract() {
    ...
    let output = AssertCommand::new(launcher)
        ...
        .output()
        .expect_or_abort("repo-local launcher repo runtime-root --json should run");
    ...
    let stdout =
        String::from_utf8(output.stdout).expect_or_abort("runtime-root stdout should be utf-8");
    assert_contains(
        &stdout,
        "\"resolved\":true",
...
fn repo_checkout_canonical_launcher_supports_runtime_root_path_contract() {
    ...
    let output = AssertCommand::new(launcher)
        ...
        .output()
        .expect_or_abort("repo-local launcher repo runtime-root --path should run");
    ...
    let stdout = String::from_utf8(output.stdout)
        .expect_or_abort("runtime-root --path stdout should be utf-8");
    assert_eq!(
        stdout.trim_end(),
...
fn repo_checkout_canonical_launcher_avoids_non_binary_repo_fallbacks() {
    ...
    for relative in ["commands", "compat/bash", "compat/powershell"] {
        ...
        assert!(
            fs::read_dir(&dir)
                .expect_or_abort("compat/commands dir should be readable")
                .next()
                .is_none(),
...
fn runtime_instruction_docs_point_at_rust_as_the_primary_oracle() {
    {
        ...
        assert_contains(&readme_content, "cargo fmt --all --check", "README.md");
        assert_contains(
            &readme_content,
        ...
        assert_contains(&readme_content, "cargo tree -d --target all", "README.md");
        assert_contains(
            &readme_content,
...
fn workflow_sequencing_contracts_and_fixtures_are_documented_consistently() {
    {
        ...
        let completion_start = readme
            ...
            .expect_or_abort("README completion flow heading should be present");
        let completion_end = readme[completion_start..]
            .find("## Project Memory")
            .map(|offset| completion_start + offset)
            .expect_or_abort("README completion flow should end before project memory section");
        ...
        let release_index = completion_block
            ...
            .expect_or_abort("completion flow should mention document-release");
        let review_index = completion_block
            .find("featureforge:requesting-code-review")
            .expect_or_abort("completion flow should mention requesting-code-review");
        assert!(
...
fn using_featureforge_preamble_uses_only_the_packaged_runtime_binary() {
    ...
    let tmp_root = TempDir::new().expect_or_abort("temp root should exist");

    assert_no_runtime_fallback_execution(&preamble, "using-featureforge preamble");

    let shared_home = tmp_root.path().join("shared-home");
    fs::create_dir_all(&shared_home).expect_or_abort("shared home should exist");
    let packaged_runtime = tmp_root.path().join("packaged-runtime");
    fs::create_dir_all(&packaged_runtime).expect_or_abort("packaged runtime should exist");
    make_runtime_repo(&packaged_runtime);
    let packaged_bin = canonical_install_bin(&shared_home);
    fs::create_dir_all(
        packaged_bin
            .parent()
            .expect_or_abort("packaged install binary should have a parent"),
    )
    .expect_or_abort("packaged install parent should exist");
    let expected_runtime_root =
        fs::canonicalize(&packaged_runtime).expect_or_abort("packaged runtime should canonicalize");
    fs::write(
        &packaged_bin,
        format!(
    ...
    .expect_or_abort("packaged runtime binary should be writable");
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        fs::set_permissions(&packaged_bin, fs::Permissions::from_mode(0o755))
            .expect_or_abort("packaged runtime binary should stay executable");
    }

    let repo_candidate = tmp_root.path().join("repo-candidate");
    fs::create_dir_all(&repo_candidate).expect_or_abort("repo candidate should exist");
    ...
    let packaged_stdout =
        String::from_utf8(packaged.stdout).expect_or_abort("preamble output should be utf8");
    assert_contains(
        &packaged_stdout,
    ...
    fs::create_dir_all(&non_runtime_repo).expect_or_abort("non-runtime repo should exist");
    ...
    fs::create_dir_all(&missing_packaged_home)
        .expect_or_abort("missing packaged home should exist");
    ...
    let no_fallback_stdout =
        String::from_utf8(no_fallback.stdout).expect_or_abort("no-fallback output should be utf8");
    assert_contains(
        &no_fallback_stdout,
        "FEATUREFORGE_ROOT=",
...
fn generated_skill_preamble_never_executes_repo_or_root_selected_launchers() {
    ...
    fs::create_dir_all(&home_dir).expect_or_abort("home dir should exist");
    fs::create_dir_all(&state_dir).expect_or_abort("state dir should exist");
    fs::create_dir_all(&repo_candidate).expect_or_abort("repo candidate should exist");
    fs::create_dir_all(&resolved_runtime_root)
        .expect_or_abort("resolved runtime root should exist");
    ...
    let stdout = String::from_utf8(output.stdout).expect_or_abort("preamble stdout should be utf8");
