//! Contracts spec plan integration/benchmark crate.
...
fn unique_temp_dir_under_docs(label: &str) -> PathBuf {
    ...
    fs::create_dir_all(&dir).expect_or_abort("temp dir under docs should be created");
    dir
}
...
fn active_engineering_approved_plans_reference_existing_source_specs() {
    ...
    for entry in entries {
        let entry = entry.expect_or_abort("plan directory entry should be readable");
        let path = entry.path();
...
fn write_plan_fidelity_receipt(path: &Path, receipt: &PlanFidelityReceipt) {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).expect_or_abort("receipt parent directories should exist");
    }
    fs::write(
        path,
        serde_json::to_string_pretty(receipt).expect_or_abort("receipt should serialize"),
    )
    .expect_or_abort("receipt should write");
}
...
fn write_plan_fidelity_review_artifact(
    ...
    let plan_fingerprint = featureforge::git::sha256_hex(
        &fs::read(repo_root.join(input.plan_path))
            .expect_or_abort("plan fixture should be readable"),
    );
    let spec_fingerprint = featureforge::git::sha256_hex(
        &fs::read(repo_root.join(input.spec_path))
            .expect_or_abort("spec fixture should be readable"),
    );
    ...
    if let Some(parent) = artifact_path.parent() {
        fs::create_dir_all(parent)
            .expect_or_abort("review artifact parent directories should exist");
    }
    fs::write(
        artifact_path,
        format!(
...
fn seed_direct_plan_fidelity_review_artifact(repo_root: &Path) -> (String, String) {
    ...
    let artifact_source = fs::read(repo_root.join(artifact_rel))
        .expect_or_abort("direct review artifact should be readable after write");
    (
...
fn build_matching_plan_fidelity_receipt(repo_root: &Path) -> PlanFidelityReceipt {
    let spec =
        parse_spec_file(repo_root.join(SPEC_REL)).expect_or_abort("spec fixture should parse");
    let plan =
        parse_plan_file(repo_root.join(PLAN_REL)).expect_or_abort("plan fixture should parse");
    let report = analyze_plan(repo_root.join(SPEC_REL), repo_root.join(PLAN_REL))
        .expect_or_abort("report should build");
...
fn parse_success_json(output: &Output, context: &str) -> Value {
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|error| {
        featureforge::abort!("{context} should emit valid success json: {error}")
    })
}
...
fn replace_in_file(path: &Path, search: &str, replacement: &str) {
    let source = fs::read_to_string(path).expect_or_abort("fixture should be readable");
    assert!(
        source.contains(search),
        "fixture should contain target text: {search}"
    );
    fs::write(path, source.replacen(search, replacement, 1))
        .expect_or_abort("fixture should be writable");
}

fn inline_spec_with_fenced_requirement_index(repo_root: &Path) {
    let spec_path = repo_root.join(SPEC_REL);
    if let Some(parent) = spec_path.parent() {
        fs::create_dir_all(parent).expect_or_abort("spec fixture parent should exist");
    }
    fs::write(
        spec_path,
...
",
    ...
    .expect_or_abort("inline spec fixture should write");
}
...
fn parse_spec_headers_and_index_exactly() {
    ...
    .expect_or_abort("valid spec fixture should parse");
...
fn parse_spec_headers_and_index_with_trailing_ceo_review_summary() {
    ...
    let spec_path = repo_root.join(SPEC_REL);
    let source = fs::read_to_string(&spec_path).expect_or_abort("valid spec fixture should read");
    fs::write(
        &spec_path,
        format!(
    ...
    .expect_or_abort("spec fixture with trailing summary should write");

    let spec =
        parse_spec_file(&spec_path).expect_or_abort("spec with trailing summary should parse");
    assert_eq!(spec.workflow_state, "CEO Approved");
    assert_eq!(spec.spec_revision, 1);
    assert_eq!(spec.last_reviewed_by, "plan-ceo-review");
    assert_eq!(spec.requirements.len(), 6);
...
fn analyze_valid_contract_fixture_reports_clean_coverage() {
    ...
    let report = analyze_plan(repo_root.join(SPEC_REL), repo_root.join(PLAN_REL))
        .expect_or_abort("valid fixture should analyze");
...
fn analyze_plan_rejects_missing_execution_strategy() {
    ...
    let report = analyze_plan(repo_root.join(SPEC_REL), &plan_path)
        .expect_or_abort("analysis should still produce a report");
...
fn analyze_plan_rejects_invalid_path_traversal_fixture_in_library_path() {
    ...
    let error = analyze_plan(repo_root.join(SPEC_REL), repo_root.join(PLAN_REL))
        .expect_err_or_abort("library analyzer should reject path traversal files entries");
...
fn analyze_plan_rejects_invalid_plan_headers_in_library_path() {
    ...
    let error = analyze_plan(repo_root.join(SPEC_REL), &plan_path)
        .expect_err_or_abort("library analyzer should reject invalid plan headers");
...
fn analyze_plan_requires_last_directive_to_wait_for_all_current_sink_tasks() {
    ...
    fs::create_dir_all(
        plan_path
            .parent()
            .expect_or_abort("custom last-plan fixture should have a parent directory"),
    )
    .expect_or_abort("custom last-plan fixture parent should exist");
    fs::write(
        &plan_path,
        format!(
    ...
    .expect_or_abort("custom last-plan fixture should write");
...
fn analyze_plan_accepts_multi_task_serial_reintegration_seam() {
    ...
    fs::create_dir_all(
        plan_path
            .parent()
            .expect_or_abort("custom seam-plan fixture should have a parent directory"),
    )
    .expect_or_abort("custom seam-plan fixture parent should exist");
    fs::write(
        &plan_path,
        format!(
    ...
    .expect_or_abort("custom seam-plan fixture should write");
...
fn analyze_plan_rejects_multi_task_serial_plain_seam_wording() {
    ...
    fs::create_dir_all(
        plan_path
            .parent()
            .expect_or_abort("custom plain-seam fixture should have a parent directory"),
    )
    .expect_or_abort("custom plain-seam fixture parent should exist");
    fs::write(
        &plan_path,
        format!(
    ...
    .expect_or_abort("custom plain-seam fixture should write");
...
fn plan_fidelity_receipt_validation_rejects_missing_or_drifted_review_artifacts() {
    ...
    fs::remove_file(repo_root.join(&receipt.review_artifact_path))
        .expect_or_abort("review artifact should be removable for missing-artifact coverage");
    write_plan_fidelity_receipt(&receipt_path, &receipt);
    ...
    .expect_or_abort("tampered review artifact should write");
...
fn exported_analyze_plan_reads_runtime_owned_plan_fidelity_receipt_path() {
    ...
    let report = analyze_plan(repo_root.join(SPEC_REL), repo_root.join(PLAN_REL))
        .expect_or_abort("direct analyze_plan helper should read the runtime-owned receipt path");
...
fn analyze_plan_cli_resolves_repo_relative_paths_from_subdirectories() {
    ...
    fs::create_dir_all(repo_root.join("src/runtime")).expect_or_abort("subdirectory should exist");
...
fn analyze_valid_contract_fixture_with_trailing_engineering_review_summary() {
    ...
    let plan_path = repo_root.join(PLAN_REL);
    let source = fs::read_to_string(&plan_path).expect_or_abort("valid plan fixture should read");
    fs::write(
        &plan_path,
        format!(
    ...
    .expect_or_abort("plan fixture with trailing summary should write");

    let report = analyze_plan(repo_root.join(SPEC_REL), plan_path)
        .expect_or_abort("analysis should tolerate trailing engineering summary");
    assert_eq!(report.contract_state, "valid");
    assert_eq!(report.task_count, 3);
    assert_eq!(report.packet_buildable_tasks, 3);
    assert!(report.coverage_complete);
...
fn analyze_plan_detects_stale_source_spec_linkage() {
    ...
    let plan_path = repo_root.join(PLAN_REL);
    let source = fs::read_to_string(&plan_path).expect_or_abort("plan fixture should read");
    fs::write(
        &plan_path,
        source.replace("**Source Spec Revision:** 1", "**Source Spec Revision:** 2"),
    )
    .expect_or_abort("stale plan fixture should write");

    let report = analyze_plan(repo_root.join(SPEC_REL), plan_path)
        .expect_or_abort("analysis should succeed");
    assert_eq!(report.contract_state, "invalid");
    assert_eq!(
        report.reason_codes,
...
fn analyze_valid_contract_fixture_with_checked_steps_and_fenced_details() {
    ...
    fs::write(&plan_path, source)
        .expect_or_abort("plan fixture with checked step details should write");

    let report = analyze_plan(repo_root.join(SPEC_REL), &plan_path)
        .expect_or_abort("checked steps with fenced details should analyze");
...
fn analyze_plan_rejects_malformed_checked_step_entries() {
    ...
    let error = analyze_plan(repo_root.join(SPEC_REL), plan_path)
        .expect_err_or_abort("malformed checked step entry should fail closed");
...
fn build_task_packet_preserves_contract_text_and_regenerates_persisted_cache() {
    {
        ...
        assert!(
            json_packet["packet_markdown"]
                .as_str()
                .is_some_and(|packet| packet.contains(
                    "Execution-bound specs must include a parseable `Requirement Index`"
                ))
        );
        let packet_path = PathBuf::from(
            json_packet["packet_path"]
                .as_str()
                .expect_or_abort("persisted task packet path should exist"),
        );
        let first_fingerprint = json_packet["packet_fingerprint"]
            .as_str()
            .expect_or_abort("packet fingerprint should exist")
            .to_owned();
        ...
        let markdown = String::from_utf8(markdown_packet.stdout)
            .expect_or_abort("markdown task packet output should be utf8");
        ...
        fs::write(&packet_path, "tampered\n").expect_or_abort("packet cache should be writable");
        ...
        assert!(
            fs::read_to_string(&packet_path)
                .expect_or_abort("packet cache should remain readable")
...
fn lint_invalid_contract_failures_and_packet_cache_retention_match_cli_contract() {
    {
        ...
        let packet_path = PathBuf::from(
            packet["packet_path"]
                .as_str()
                .expect_or_abort("packet path should exist for retained cache"),
        );
        let packet_dir = packet_path
            .parent()
            .expect_or_abort("packet directory should exist")
            .to_path_buf();
        fs::write(packet_dir.join("stale-one.packet.md"), "stale one\n")
            .expect_or_abort("stale one packet");
        fs::write(packet_dir.join("stale-two.packet.md"), "stale two\n")
            .expect_or_abort("stale two packet");
        fs::write(packet_dir.join("stale-three.packet.md"), "stale three\n")
            .expect_or_abort("stale three packet");

        let mut retained =
            Command::cargo_bin("featureforge").expect_or_abort("featureforge cargo binary");
        retained
            .current_dir(&repo_root)
            .env("FEATUREFORGE_STATE_DIR", &state_dir)
        ...
        let retained_packets = fs::read_dir(&packet_dir)
            .expect_or_abort("packet directory should remain readable")
            .flatten()
...
fn analyze_plan_reports_missing_plan_fidelity_receipt_for_draft_plan() {
    ...
    assert!(
        report["reason_codes"]
            .as_array()
            .expect_or_abort("reason_codes should be present")
            .iter()
...
fn analyze_plan_rejects_stale_or_non_independent_plan_fidelity_receipts() {
    {
        ...
        fs::remove_file(&runtime_receipt_path)
            .expect_or_abort("invalid runtime receipt should be removable");
        write_plan_fidelity_review_artifact(
            &repo_root,
            PlanFidelityReviewArtifactInput {
...
fn analyze_plan_reports_invalid_fidelity_gate_when_spec_requirement_index_is_malformed() {
    ...
    .expect_or_abort("malformed spec fixture should write");
...
fn analyze_plan_rejects_pass_receipt_when_source_spec_is_not_workflow_valid_ceo_review() {
    ...
    let report = analyze_plan(repo_root.join(SPEC_REL), repo_root.join(PLAN_REL))
        .expect_or_abort("analyze_plan should still return a report for draft plans with invalid spec reviewer provenance");
...
fn plan_contract_schemas_exist_with_expected_titles() {
    ...
    write_contract_schemas(&generated_schema_dir)
        .expect_or_abort("generated contract schemas should write");

    let analyze_schema: Value = serde_json::from_str(
        &fs::read_to_string(&analyze_schema_path).expect_or_abort("analyze schema should exist"),
    )
    .expect_or_abort("analyze schema should be valid json");
    let packet_schema: Value = serde_json::from_str(
        &fs::read_to_string(&packet_schema_path).expect_or_abort("packet schema should exist"),
    )
    .expect_or_abort("packet schema should be valid json");
    let generated_analyze_schema =
        fs::read_to_string(generated_schema_dir.join("plan-contract-analyze.schema.json"))
            .expect_or_abort("generated analyze schema should exist");
    ...
    assert_eq!(
        fs::read_to_string(&analyze_schema_path)
            .expect_or_abort("checked-in analyze schema should be readable")
            .trim_end(),
