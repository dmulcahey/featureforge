...
use featureforge::execution::state::current_repo_state_fingerprint;
...
- Versioning Rationale: focused final-review workflow fixture with no published package surface.
...
fn write_security_review_artifact(repo: &Path, state: &Path, result: &str) -> PathBuf {
    let branch = branch_name(repo);
    let safe_branch = branch_storage_key(&branch);
    let head_sha = current_head_sha(repo);
    let diff_fingerprint = current_repo_state_fingerprint(repo)
        .expect("repo state fingerprint should be available for security review fixture");
    let artifact_path = project_artifact_dir(repo, state)
        .join(format!("tester-{safe_branch}-security-review-20260322-171700.md"));
    let unsigned = format!(
        "# Security Review Result\n**Artifact Kind:** security-review\n**Schema Version:** 1\n**Artifact Provenance:** runtime-owned\n**Retention Policy:** featureforge:authoritative-runtime-artifact\n**Source Plan:** `{PLAN_REL}`\n**Source Plan Revision:** 1\n**Branch:** {branch}\n**Repo:** {}\n**Head SHA:** {head_sha}\n**Execution Diff Fingerprint:** {diff_fingerprint}\n**Receipt Fingerprint:** __FINGERPRINT__\n**Result:** {result}\n**Generated By:** featureforge:security-review\n**Generated At:** 2026-03-22T17:17:00Z\n\n## Summary\n- security review fixture\n",
        repo_slug(repo)
    );
    let fingerprint = canonical_unit_review_receipt_fingerprint(&unsigned);
    write_file(&artifact_path, &unsigned.replace("__FINGERPRINT__", &fingerprint));
    artifact_path
}
...
#[test]
fn workflow_phase_routes_required_security_review_before_requesting_code_review() {
    let (repo_dir, state_dir) = init_repo("workflow-runtime-required-security-review");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let session_key = "workflow-runtime-required-security-review";
    ...
    replace_in_file(
        &repo.join(PLAN_REL),
        "## Task 1: Single Step Task\n",
        "## Risk & Gate Signals\n- Delivery Lane: standard\n- UI Scope: none\n- Browser QA Required: no\n- Design Review Required: no\n- Security Review Required: yes\n- Performance Review Required: no\n- Release Surface: code_only_no_deploy\n- Distribution Impact: low\n- Deploy Impact: low\n- Migration Risk: low\n\n## Release & Distribution Notes\n- Discoverability / Distribution Path: internal runtime-only change\n- Versioning Decision: unknown\n- Versioning Rationale: pending document-release\n- Deployment / Rollout Notes: none\n\n## Task 1: Single Step Task\n",
    );
    mark_all_plan_steps_checked(repo);
    write_single_step_v2_completed_attempt(repo, &expected_packet_fingerprint(repo, 1, 1));
    write_authoritative_strategy_checkpoint_state(repo, state);
    ...
    let phase_json = run_featureforge_with_env(
        ...
        &["workflow", "phase", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow phase for required-security-review shard",
    );
    let handoff_json = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "handoff", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow handoff for required-security-review shard",
    );
    let gate_finish_json = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "gate", "finish", "--plan", PLAN_REL, "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow finish gate for required-security-review shard",
    );

    assert_eq!(phase_json["phase"], "security_review_pending");
    assert_eq!(phase_json["next_action"], "run_security_review");
    assert_eq!(handoff_json["phase"], "security_review_pending");
    assert_eq!(
        handoff_json["recommended_skill"],
        "featureforge:security-review"
    );
    assert_eq!(gate_finish_json["allowed"], false);
    assert!(
        gate_finish_json["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code == "security_review_required")),
        "gate-finish should block on required security review before final review, got {gate_finish_json:?}"
    );
}

#[test]
fn workflow_phase_routes_legacy_plan_without_signals_to_security_review() {
    let (repo_dir, state_dir) = init_repo("workflow-runtime-legacy-security-review");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let session_key = "workflow-runtime-legacy-security-review";
    ...
    let phase_json = run_featureforge_with_env(
        ...
        &["workflow", "phase", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow phase for legacy no-signal security-review shard",
    );
    let gate_finish_json = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "gate", "finish", "--plan", PLAN_REL, "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow finish gate for legacy no-signal security-review shard",
    );
    ...
    assert!(
        gate_finish_json["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code == "security_review_required")),
        "legacy plans without approved risk signals should route conservatively through security review, got {gate_finish_json:?}"
    );
}

#[test]
fn workflow_phase_routes_missing_release_readiness_before_requesting_code_review() {
    let (repo_dir, state_dir) = init_repo("workflow-runtime-release-before-review");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let session_key = "workflow-runtime-release-before-review";
    ...
    let phase_json = run_featureforge_with_env(
        ...
        &["workflow", "phase", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow phase for release-before-review shard",
    );
    let handoff_json = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "handoff", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow handoff for release-before-review shard",
    );
    ...
    assert_eq!(handoff_json["recommended_skill"], "featureforge:document-release");
}

#[test]
fn workflow_phase_surfaces_blocked_security_review_from_authoritative_state() {
    let (repo_dir, state_dir) = init_repo("workflow-runtime-blocked-security-review");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let session_key = "workflow-runtime-blocked-security-review";
    ...
    let security_review_artifact = write_security_review_artifact(repo, state, "blocked");
    let record_json = run_featureforge_with_env(
        repo,
        state,
        &[
            "workflow",
            ...
            PLAN_REL,
            "--review-artifact",
            &security_review_artifact.display().to_string(),
            "--json",
        ],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow security-review record should accept blocked outcomes",
    );
    assert_eq!(record_json["status"], "ok");
    ...
    let phase_json = run_featureforge_with_env(
        ...
        &["workflow", "phase", "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow phase for blocked-security-review shard",
    );
    let gate_finish_json = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "gate", "finish", "--plan", PLAN_REL, "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow finish gate for blocked-security-review shard",
    );
    ...
    assert!(
        gate_finish_json["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code == "security_result_not_pass")),
        "gate-finish should surface the blocked security review outcome, got {gate_finish_json:?}"
    );
}

#[test]
fn workflow_phase_invalidates_security_review_when_untracked_file_contents_change() {
    let (repo_dir, state_dir) = init_repo("workflow-runtime-security-review-untracked-content-change");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let session_key = "workflow-runtime-security-review-untracked-content-change";
    ...
    write_file(&repo.join("notes.txt"), "alpha\n");
    let security_review_artifact = write_security_review_artifact(repo, state, "pass");
    let record_json = run_featureforge_with_env(
        repo,
        state,
        &[
        ...
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow security-review record should accept pass outcomes with untracked files",
    );
    assert_eq!(record_json["status"], "ok");

    write_file(&repo.join("notes.txt"), "beta\n");
    ...
    let gate_finish_json = run_featureforge_with_env(
        ...
        &["workflow", "gate", "finish", "--plan", PLAN_REL, "--json"],
        &[("FEATUREFORGE_SESSION_KEY", session_key)],
        "workflow finish gate for untracked-content-change shard",
    );
    ...
    assert!(
        gate_finish_json["reason_codes"]
            ...
            .is_some_and(|codes| codes
                .iter()
                .any(|code| code == "security_artifact_diff_fingerprint_mismatch")),
        "gate-finish should invalidate security review when untracked file contents change, got {gate_finish_json:?}"
    );
}
