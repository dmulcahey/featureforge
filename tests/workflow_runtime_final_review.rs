//! Workflow runtime final review integration/benchmark crate.
#[path = "support/bin.rs"]
mod bin_support;
...
fn update_authoritative_harness_state(repo: &Path, state: &Path, updates: &[(&str, Value)]) {
    ...
    let mut payload: Value = match fs::read_to_string(&state_path) {
        Ok(source) => {
            serde_json::from_str(&source).expect_or_abort("harness state should be valid json")
        }
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => json!({}),
        Err(error) => {
            featureforge::abort!("harness state should be readable for fixture mutation: {error}")
        }
    };
    let object = payload
        .as_object_mut()
        .expect_or_abort("harness state payload should remain a json object");
    object
    ...
    write_file(
        &state_path,
        &serde_json::to_string(&payload).expect_or_abort("harness state payload should serialize"),
    );
}
...
fn publish_authoritative_release_truth(
    ...
    let reviewed_state_id = format!("git_tree:{}", current_head_tree_sha(repo));
    let release_source =
        fs::read_to_string(release_path).expect_or_abort("release artifact should be readable");
...
fn publish_authoritative_final_review_truth(
    ...
    let reviewed_state_id = format!("git_tree:{}", current_head_tree_sha(repo));
    let review_source =
        fs::read_to_string(review_path).expect_or_abort("review artifact should be readable");
...
fn record_task_boundary_review_dispatch(repo: &Path, state_dir: &Path, plan_rel: &str) -> String {
    ...
    run_plan_execution(
        ...
        .as_str()
        .expect_or_abort(
            "status should expose strategy checkpoint fingerprint after review dispatch",
        )
        .to_owned()
}
...
fn write_task_boundary_strategy_checkpoint_state(
    ...
    let branch = branch_name(repo);
    let state_path = harness_state_path(state, &repo_slug(repo), &branch);
    let mut payload: Value = fs::read_to_string(&state_path).map_or_else(
        |_| json!({}),
        |source| {
            serde_json::from_str(&source).expect_or_abort("harness state should be valid json")
        },
...
fn replace_in_file(path: &Path, from: &str, to: &str) {
    let source = fs::read_to_string(path).expect_or_abort("file should be readable");
    fs::write(path, source.replace(from, to)).expect_or_abort("file should be writable");
}
...
fn mutate_reviewer_artifact_unreadable(
    ...
    fs::remove_file(&reviewer_artifact_path)
        .expect_or_abort("reviewer artifact should be removable");
}
...
fn mutate_reviewer_artifact_not_runtime_owned(
    ...
    let reviewer_source = fs::read_to_string(&reviewer_artifact_path)
        .expect_or_abort("reviewer artifact should be readable");
    let external_dir = state.join("external-reviewer-artifacts");
    fs::create_dir_all(&external_dir)
        .expect_or_abort("external reviewer artifact dir should be creatable");
    ...
    fs::write(&external_artifact_path, reviewer_source)
        .expect_or_abort("external reviewer artifact should be writable");
    let external_artifact_fingerprint = sha256_hex(
        &fs::read(&external_artifact_path)
            .expect_or_abort("external reviewer artifact should be readable"),
    );
...
fn mutate_reviewer_identity_mismatch(
    ...
    .expect_or_abort("reviewer artifact should be writable");
    let reviewer_artifact_fingerprint = sha256_hex(
        &fs::read(&reviewer_artifact_path).expect_or_abort("reviewer artifact should be readable"),
    );
    replace_review_reviewer_artifact_binding(
        review_path,
        &reviewer_artifact_path,
...
fn task_boundary_dispatch_does_not_release_next_task_without_task_closure() {
    {
        let (repo_dir, state_dir) =
            init_repo("workflow-runtime-task-boundary-final-review-required");
        let repo = repo_dir.path();
        let state = state_dir.path();
        ...
        let execution_run_id = complete_task1["execution_run_id"]
            .as_str()
            .expect_or_abort("execution run id should be present after task 1 completion")
            .to_owned();
        let checkpoint_sha = current_head_sha(repo);
        write_task_boundary_unit_review_receipt(
            repo,
            state,
            &execution_run_id,
            1,
        ...
        let begin_task2 = serde_json::from_slice::<Value>(&begin_task2_output.stderr)
            .expect_or_abort("blocked task 2 begin should emit json failure on stderr");
        assert_eq!(
...
fn document_release_precedes_final_review_after_release_truth_stales() {
    ...
    let phase_detail = status_json["phase_detail"]
        .as_str()
        .expect_or_abort("release-precedence regression should include phase_detail");
    assert!(
        matches!(
            phase_detail,
...
fn fs02_late_stage_drift_routes_consistently_across_operator_and_status() {
    ...
    let drifted_plan = format!(
        ...
        fs::read_to_string(&plan_path)
            .expect_or_abort("plan should be readable before FS-02 drift")
    );
    ...
    let phase_detail = status_json["phase_detail"]
        .as_str()
        .expect_or_abort("FS-02 late-stage drift regression should include phase_detail");
    assert_eq!(
