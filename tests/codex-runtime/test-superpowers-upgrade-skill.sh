...
require_pattern 'INSTALL_BIN_PWSH="$INSTALL_DIR/bin/superpowers-install-runtime.ps1"'
require_pattern 'bin/superpowers-install-runtime'
require_pattern 'CONFIG_BIN_PWSH="$INSTALL_DIR/bin/superpowers-config.ps1"'
require_pattern 'bin/superpowers-config.ps1'
require_pattern 'bin/superpowers-install-runtime.ps1'
require_pattern 'ERROR: superpowers upgrade failed during staged install/update'
require_pattern 'The shell snippets below use the bash entrypoints.'
require_pattern 'When continuing from PowerShell on Windows, use the matching wrapper paths instead: `bin/superpowers-config.ps1` and `bin/superpowers-install-runtime.ps1`.'
require_pattern 'Re-enable them with $CONFIG_BIN set update_check true on Unix-like shells or $CONFIG_BIN_PWSH set update_check true from PowerShell on Windows.'
...
require_pattern 'When continuing from PowerShell on Windows, invoke `$INSTALL_BIN_PWSH` instead of the bash entrypoint above.'
