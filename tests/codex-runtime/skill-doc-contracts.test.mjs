...
function retiredProductName() {
  ...
  assert.ok(
    provenanceLine,
    "README.md should keep the provenance attribution line",
  );
  const match = provenanceLine.match(/upstream ([A-Za-z]+):/);
  assert.ok(
    match,
    "README.md provenance line should expose the retired product name",
  );
...
const HELPER_COMMAND_PATTERN =
  /\bfeatureforge-(plan-contract|plan-execution|workflow-status|workflow|repo-safety|session-entry|config|slug|update-check|migrate-install)\b/;
...
const FORBIDDEN_RUNTIME_FALLBACK_EXECUTION_PATTERNS = [
  [
    ...
    "should not probe repo-local binaries from generated runtime docs",
  ],
  [
    /(?:^|\n)\s*"\$_FEATUREFORGE_ROOT\/bin\/featureforge"/,
    "should not execute runtime commands through a root-selected launcher",
  ],
  [
    /(?:^|\n)\s*"\$INSTALL_DIR\/bin\/featureforge"/,
    "should not execute runtime commands through an install-root-selected launcher",
  ],
  [
    /(?:^|\n)\s*"\$_FEATUREFORGE_ROOT\/bin\/featureforge\.exe"/,
    "should not execute runtime commands through a root-selected Windows launcher",
  ],
  [
    /(?:^|\n)\s*"\$INSTALL_DIR\/bin\/featureforge\.exe"/,
    "should not execute runtime commands through an install-root-selected Windows launcher",
  ],
  [
    /(?:^|\n)\s*FEATUREFORGE_RUNTIME_BIN="\$_FEATUREFORGE_ROOT\/bin\/featureforge"/,
    "should not assign the runtime command path from $_FEATUREFORGE_ROOT",
  ],
  [
    /(?:^|\n)\s*FEATUREFORGE_RUNTIME_BIN="\$INSTALL_DIR\/bin\/featureforge"/,
    "should not assign the runtime command path from INSTALL_DIR",
  ],
  [
    /(?:^|\n)\s*FEATUREFORGE_RUNTIME_BIN="\$_FEATUREFORGE_ROOT\/bin\/featureforge\.exe"/,
    "should not assign the runtime command path from a root-selected Windows launcher",
  ],
  [
    /(?:^|\n)\s*FEATUREFORGE_RUNTIME_BIN="\$INSTALL_DIR\/bin\/featureforge\.exe"/,
    "should not assign the runtime command path from an install-root-selected Windows launcher",
  ],
  [
    /\$\{_FEATUREFORGE_BIN:-featureforge\}/,
    "should not fall back to PATH-selected featureforge binaries",
  ],
  [
    /command -v featureforge/,
    "should not rediscover featureforge through PATH lookups",
  ],
];
...
function assertNoRuntimeFallbackExecution(content, label) {
  for (const [
    pattern,
    message,
  ] of FORBIDDEN_RUNTIME_FALLBACK_EXECUTION_PATTERNS) {
...
function assertForbidsDirectHelperCommandMutation(content, command, label) {
  ...
  const hasBoundary = windows.some((window) => {
    const hasProhibition =
      /(must not|do not|never|should not|cannot|can't)/i.test(window);
    const hasDirectAction = /(invoke|call|run|execute|direct(?:ly)?)/i.test(
      window,
    );
    const hasOwnerActor =
      /(coordinator|controller|helper|runtime|harness|gate)/i.test(window);
    const hasOwnerVerb =
      /(owns?|owned|authoritative|handles?|appl(?:y|ies)|executes?|invokes?|calls?|runs?|governs?)/i.test(
        window,
      );
    return (
      (hasProhibition && hasDirectAction) || (hasOwnerActor && hasOwnerVerb)
    );
  });
  assert.ok(
    hasBoundary,
...
function assertSeparatesCandidateArtifactsFromAuthoritativeMutations(
  content,
  label,
) {
  const hasCandidateSurface =
    /(candidate|task packet|task-packet|packet context|handoff|coverage matrix)/i.test(
      content,
    );
  const hasAuthoritativeSurface =
    /(authoritative|helper-owned|coordinator-owned|execution state|execution evidence|review gate|finish-gate|gate-review)/i.test(
      content,
    );
  const hasBoundaryLanguage =
    /(must not|do not|never|may not|only|owns?|owned|instead of|fail closed)/i.test(
      content,
    );
  assert.ok(
...
function assertDownstreamMaterialStaysGateAndHarnessAware(content, label) {
  const hasGateAwareness =
    /(gate-review|review gate|finish-gate|gate-finish|fail closed)/i.test(
      content,
    );
  const hasHarnessAwareness =
    /(execution evidence|task-packet|coverage matrix|source plan|source test plan|workflow-routed|artifact)/i.test(
      content,
    );
  assert.ok(
...
test("templates declare exactly one base or review preamble placeholder", () => {
  for (const skill of listGeneratedSkills()) {
    ...
    assert.notEqual(
      hasBase,
      hasReview,
      `${skill} should declare exactly one preamble placeholder`,
    );
  }
});
...
test("generated preamble bash block includes shared runtime-root, session, and contributor state", () => {
  for (const skill of listGeneratedSkills()) {
    ...
    const bashBlock = extractBashBlockUnderHeading(
      content,
      "Preamble (run first)",
    );
    ...
    assert.match(
      ...
      `${skill} should resolve runtime roots through the helper contract`,
    );
    assert.match(
      bashBlock,
      /\$HOME\/\.featureforge\/install/,
      `${skill} should pin runtime commands to the canonical install root`,
    );
    assert.match(
      bashBlock,
      /featureforge\.exe/,
      `${skill} should keep the Windows packaged launcher path in the install-root contract`,
    );
    assert.match(
      bashBlock,
      /"\$_FEATUREFORGE_BIN" update-check/,
      `${skill} should run update checks through the packaged install binary`,
    );
    assert.match(
      bashBlock,
      /"\$_FEATUREFORGE_BIN" config get featureforge_contributor/,
      `${skill} should load contributor mode through the packaged install binary`,
    );
    assert.doesNotMatch(
      bashBlock,
      /_IS_FEATUREFORGE_RUNTIME_ROOT\(\)/,
      `${skill} should not embed its own runtime-root detector`,
    );
    assertNoRuntimeFallbackExecution(bashBlock, `${skill} preamble`);
    assert.doesNotMatch(
      bashBlock,
      /sed -n/,
      `${skill} should not parse runtime-root JSON in shell`,
    );
    assert.match(
      bashBlock,
      /_SESSIONS=/,
      `${skill} should track session count`,
    );
    assert.match(
      bashBlock,
      /_CONTRIB=/,
      `${skill} should load contributor state`,
    );
  }
});
...
test("install docs describe the path-based runtime-root helper contract", () => {
  for (const relativePath of [".codex/INSTALL.md", ".copilot/INSTALL.md"]) {
    ...
    assert.match(
      content,
      /featureforge repo runtime-root --path/,
      `${relativePath} should describe the path-based helper contract`,
    );
    assert.match(
      content,
      /~\/\.featureforge\/install\/bin\/featureforge/,
      `${relativePath} should describe the packaged install binary contract`,
    );
    assert.match(
      content,
      /featureforge\.exe/,
      `${relativePath} should mention the Windows packaged binary contract`,
    );
    assert.doesNotMatch(
      content,
      /featureforge repo runtime-root --json/,
      `${relativePath} should not describe the retired JSON shell contract`,
    );
  }
});
  ...
  for (const skill of listGeneratedSkills()) {
    ...
    if (skill === "using-featureforge") {
      assert.doesNotMatch(
        ...
        "using-featureforge should stay exempt from the shared section",
      );
      continue;
    }
    ...
    assert.ok(
      section,
      `${skill} should include the Search Before Building section`,
    );
    const normalized = normalizeWhitespace(section);
    assert.match(
      normalized,
      /Layer 1: tried-and-true \/ built-ins \/ existing repo-native solutions/,
      `${skill} should describe Layer 1`,
    );
    assert.match(
      normalized,
      /Layer 2: current practice and known footguns/,
      `${skill} should describe Layer 2`,
    );
    assert.match(
      normalized,
      /Layer 3: first-principles reasoning for this repo and this problem/,
      `${skill} should describe Layer 3`,
    );
    assert.match(
      normalized,
      /External search results are inputs, not answers\./,
      `${skill} should keep Layer 2 non-authoritative`,
    );
    assert.match(
      normalized,
...
test("using-featureforge gets a dedicated bootstrap preamble contract", () => {
  ...
  const bootstrapBlock = extractBashBlockUnderHeading(
    content,
    "Preamble (run first)",
  );
  const normalStackBlock = extractBashBlockUnderHeading(
    content,
    "Normal FeatureForge Stack",
  );
  assert.match(
    bootstrapBlock,
    /session-entry\/using-featureforge/,
    "using-featureforge should derive the decision-file path",
  );
  assert.doesNotMatch(
    bootstrapBlock,
    /touch "\$_SP_STATE_DIR\/sessions\/\$PPID"/,
    "using-featureforge should not write session markers before the bypass decision",
  );
  assert.doesNotMatch(
    bootstrapBlock,
    /_CONTRIB=/,
    "using-featureforge should not load contributor mode before the bypass decision",
  );
  assert.ok(
    normalStackBlock,
    "using-featureforge should define the post-gate normal stack",
  );
  assert.match(
    normalStackBlock,
    /"\$_FEATUREFORGE_BIN" update-check/,
    "using-featureforge should restore update checks after the bypass gate through the packaged install binary",
  );
  assert.match(
    normalStackBlock,
    /touch "\$_SP_STATE_DIR\/sessions\/\$PPID"/,
    "using-featureforge should restore session markers after the bypass gate",
  );
  assert.match(
    normalStackBlock,
    /_CONTRIB=/,
    "using-featureforge should restore contributor mode after the bypass gate",
  );
  assertNoRuntimeFallbackExecution(
    normalStackBlock,
    "using-featureforge normal stack",
  );
  assert.match(
    content,
    /ask one interactive question before any normal FeatureForge work happens/,
    "using-featureforge should ask before the normal stack",
  );
  assert.match(
    content,
    /do not compute `_SESSIONS`/,
    "using-featureforge should exempt the opt-out gate from _SESSIONS handling",
  );
  assert.match(
    content,
    /session-entry bootstrap ownership is runtime-owned/,
    "using-featureforge should name runtime ownership for the bootstrap boundary",
  );
  assert.match(
    content,
    /missing or malformed decision state fails closed/,
    "using-featureforge should document fail-closed missing or malformed state",
  );
  assert.match(
    content,
    ...
    "using-featureforge should explicitly restore the normal stack after an enabled decision",
  );
  assert.match(
    content,
    /If the session decision file exists but contains malformed content:/,
    "using-featureforge should document malformed-state handling",
  );
  assert.match(
    content,
    ...
    "using-featureforge should treat explicit skill naming as re-entry",
  );
  assert.match(
    content,
    /If the user explicitly requests re-entry but the bootstrap cannot rewrite the session decision to `enabled`:/,
    "using-featureforge should document re-entry write-failure handling",
  );
  assert.match(
    content,
    /featureforge session-entry resolve --message-file <path>/,
    "using-featureforge should reference the canonical session-entry command",
  );
  assert.doesNotMatch(
    content,
    /featureforge-session-entry/,
    "using-featureforge should not keep helper-style session-entry commands",
  );
});
...
test("all shipped runtime docs keep execution pinned to the packaged binary contract", () => {
  ...
  const runtimeDocs = [
    [
      ...
      readUtf8(path.join(REPO_ROOT, "featureforge-upgrade", "SKILL.md")),
    ],
    ...listGeneratedSkills().map((skill) => [
...
test("upgrade instructions keep runtime command execution separate from companion-file lookup", () => {
  const upgradeSkill = readUtf8(
    path.join(REPO_ROOT, "featureforge-upgrade", "SKILL.md"),
  );
  ...
  assert.doesNotMatch(
    upgradeSkill,
    installRuntimeExecPattern,
    "upgrade flow should not execute runtime commands through INSTALL_RUNTIME_BIN",
  );
  assert.doesNotMatch(
    upgradeSkill,
    /FEATUREFORGE_RUNTIME_BIN="\$INSTALL_RUNTIME_BIN"/,
    "upgrade flow should not rebind FEATUREFORGE_RUNTIME_BIN from INSTALL_RUNTIME_BIN",
  );
});
...
test("generated preambles capture _BRANCH exactly once and keep helper BRANCH out of grounding", () => {
  ...
  for (const skill of listGeneratedSkills()) {
    ...
    assert.equal(
      ...
      1,
      `${skill} should include one _BRANCH assignment in the full doc`,
    );
    assert.equal(
      preambleAssignments.length,
      1,
      `${skill} should capture _BRANCH in the preamble`,
    );
    assert.doesNotMatch(
      bashBlock,
      /\bBRANCH=/,
      `${skill} should not define helper BRANCH in the preamble`,
    );
  }
});
...
test("generated branch-aware helper loads are guarded through _SLUG_ENV and eval the captured output only", () => {
  for (const skill of [
    ...
    assert.match(
      ...
      `${skill} should capture canonical command output into _SLUG_ENV`,
    );
    assert.match(
      content,
      /if \[ -n "\$_SLUG_ENV" \]; then\n\s+eval "\$_SLUG_ENV"\nfi/,
      `${skill} should only eval guarded helper output`,
    );
    assert.doesNotMatch(
      content,
      /eval "\$\("\$_FEATUREFORGE_BIN" repo slug\)/,
      `${skill} should not unguardedly eval command substitution`,
    );
  }
});
...
test("branch-aware skill docs consume the slug helper instead of inline sanitization fragments", () => {
  for (const skill of [
    ...
    assert.match(
      content,
      /"\$_FEATUREFORGE_BIN" repo slug/,
      `${skill} should use the canonical repo slug command through the packaged install binary`,
    );
    assert.doesNotMatch(
      content,
      /SAFE_BRANCH=\$\(/,
      `${skill} should not inline branch sanitization`,
    );
    assert.doesNotMatch(
      content,
      /(?:^|[^_])BRANCH=\$\(git rev-parse --abbrev-ref HEAD/,
      `${skill} should not inline raw branch capture`,
    );
    assert.doesNotMatch(
      content,
      /SLUG=\$\(printf '%s\\n' "\$REMOTE_URL"/,
      `${skill} should not inline repo slug derivation`,
    );
  }
});
...
test("helper BRANCH stays artifact-only in the branch-aware skill consumers", () => {
  for (const skill of ["qa-only", "finishing-a-development-branch"]) {
    ...
    assert.match(
      content,
      /\$BRANCH/,
      `${skill} should use helper BRANCH in artifact selection`,
    );
    assert.doesNotMatch(
      bashBlock,
      /\$BRANCH/,
      `${skill} should not use helper BRANCH in the grounding preamble`,
    );
  }
});
...
test("review skills include review-only preamble contract", () => {
  for (const skill of listGeneratedSkills()) {
    ...
    assert.match(
      bashBlock,
      /_TODOS_FORMAT=/,
      `${skill} should load TODO format state`,
    );
    assert.match(
      content,
      /## Agent Grounding/,
      `${skill} should include Agent Grounding`,
    );
  }
});
...
test("interactive question contract appears once per generated skill in normalized form", () => {
  ...
  for (const skill of listGeneratedSkills()) {
    const content = readUtf8(getSkillPath(skill));
    assert.equal(
      countOccurrences(content, "## Interactive User Question Format"),
      1,
      `${skill} should define the interactive question format once`,
    );
    const section = extractSection(content, "Interactive User Question Format");
    assert.ok(
      section,
      `${skill} should include the interactive question format section`,
    );
    const normalized = normalizeWhitespace(section);
    for (const bit of expectedBits) {
      assert.match(
        normalized,
        new RegExp(bit.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
        `${skill} should include ${bit}`,
      );
    }
  }
});
...
test("workflow fixture coverage uses local fixtures instead of historical docs paths", () => {
  const content = readUtf8(
    path.join(REPO_ROOT, "tests/runtime_instruction_contracts.rs"),
  );
  ...
  for (const [skill, patterns] of Object.entries(expected)) {
    ...
    for (const pattern of patterns) {
      assert.match(
        description,
        pattern,
        `${skill} description should broaden discovery with ${pattern}`,
      );
    }
  }
});
...
test("workflow-critical skill descriptions encode approval-stage prerequisites", () => {
  const expected = {
    "plan-ceo-review": [
      /written FeatureForge design or architecture spec/i,
      /before implementation planning/i,
    ],
    "writing-plans": [
      /CEO-approved FeatureForge spec/i,
      /write the implementation plan/i,
    ],
    "plan-fidelity-review": [
      /approved FeatureForge spec/i,
      /current draft plan/i,
    ],
    "plan-eng-review": [
      /written FeatureForge implementation plan/i,
      /CEO-approved spec/i,
    ],
    "subagent-driven-development": [
      /engineering-approved FeatureForge implementation plan/i,
      /mostly independent tasks/i,
    ],
    "executing-plans": [
      /engineering-approved FeatureForge implementation plan/i,
      /separate session/i,
    ...
    "requesting-code-review": [
      /after implementation work/i,
      /intentional review checkpoint/i,
    ],
    "finishing-a-development-branch": [
      /implementation is complete/i,
      /verification passes/i,
    ],
  };
  ...
  for (const [skill, patternOrPatterns] of Object.entries(expected)) {
    ...
    for (const pattern of patterns) {
      assert.match(
        description,
        pattern,
        `${skill} description should encode the required workflow gate`,
      );
    }
  }
});
...
test("execution and review skill docs keep candidate artifacts and downstream gates explicit", () => {
  ...
  const implementerPrompt = readUtf8(
    path.join(
      REPO_ROOT,
  ...
  assertSeparatesCandidateArtifactsFromAuthoritativeMutations(
    executingPlans,
  ...
  assertDownstreamMaterialStaysGateAndHarnessAware(
    reviewSkill,
  ...
  for (const skill of lateStageSkills) {
    ...
    for (const pattern of forbiddenPatterns) {
      assert.doesNotMatch(
        description,
        pattern,
        `${skill} description should not match ${pattern}`,
      );
    }
  }
});
...
test("execution workflow skills reference the plan-execution helper contract", () => {
  ...
  for (const skill of ["subagent-driven-development", "executing-plans"]) {
    ...
    assert.match(
      content,
      /Provides the approved plan and the execution preflight handoff/,
    );
  ...
  assert.doesNotMatch(
    readUtf8(getSkillPath("executing-plans")),
    /track the work in your platform's task checklist/,
  );
  assert.doesNotMatch(
    readUtf8(getSkillPath("subagent-driven-development")),
    /task-tracker checklist/,
  );
  assert.doesNotMatch(
    readUtf8(getSkillPath("subagent-driven-development")),
    /Mark task complete in task tracker/,
  );
  ...
  assert.match(
    finishSkill,
    /Run `featureforge plan execution status --plan <approved-plan-path>` and read the returned `evidence_path` before presenting completion options\./,
  );
  assert.match(
    finishSkill,
  ...
  assert.match(
    finishSkill,
    /The Step 2 `<base-branch>` value stays authoritative for Options A, B, and D\./,
  );
  assert.match(
    finishSkill,
  ...
  assert.match(
    reviewPrompt,
    /dedicated independent reviewer for the terminal whole-diff gate/,
  );
  ...
  assert.match(
    reviewPrompt,
    /When approved plan and execution evidence paths are provided, read both artifacts and verify that checked-off plan steps are semantically satisfied by the implementation and explicitly evidenced\./,
  );
  assert.match(
    reviewPrompt,
    /When execution evidence documents recorded topology downgrades or other execution deviations, explicitly inspect them and state whether those deviations pass final review\./,
  );
  assert.match(
    reviewPrompt,
  ...
  const subagentReviewPrompt = readUtf8(
    path.join(
      REPO_ROOT,
...
test("task-fidelity workflow docs and prompts require packet-backed plan contracts", () => {
  ...
  assert.match(
    writingPlans,
    /create .* worktrees? and run Tasks .* in parallel/i,
  );
  ...
  assert.match(
    planEngReview,
    /"\$_FEATUREFORGE_BIN" plan contract analyze-plan/,
  );
  ...
  assert.match(
    planEngReview,
    /missing, stale, or non-buildable for the approved plan revision/,
  );
  ...
  assert.match(
    subagentSkill,
    /The coordinator owns every `git commit`, `git merge`, and `git push` for this workflow/,
  );
  ...
  assert.match(
    implementerPrompt,
    /Prepare the change for coordinator-owned git actions; do not create commits, merges, or pushes yourself/,
  );
  ...
  const codeQualityPrompt = readUtf8(
    path.join(
      REPO_ROOT,
      "skills/subagent-driven-development/code-quality-reviewer-prompt.md",
    ),
...
test("repo-writing workflow skills document the protected-branch repo-safety gate consistently", () => {
  ...
  for (const [skill, targetPattern] of Object.entries(expectedTargets)) {
    ...
    assert.match(
      ...
      `${skill} should document the protected-branch gate`,
    );
    assert.match(
      content,
      /featureforge repo-safety check --intent write/,
      `${skill} should run the repo-safety check`,
    );
    assert.match(
      content,
      /featureforge repo-safety approve --stage/,
      `${skill} should document the approval rescue flow`,
    );
    assert.match(
      content,
      /featureforge:using-git-worktrees/,
      `${skill} should route blocked writes to using-git-worktrees`,
    );
    assert.match(
      content,
      /branch, the stage, and the blocking `failure_class`/,
      `${skill} should surface blocked-write diagnostics`,
    );
    assert.match(
      content,
      targetPattern,
      `${skill} should use the correct write target family`,
    );
  }
});
...
test("generated skills use canonical runtime commands instead of helper executables", () => {
  for (const skill of listGeneratedSkills()) {
    ...
    assert.doesNotMatch(
      ...
      `${skill} should not use helper-style executable names`,
    );
  }
});
...
test("workflow handoff skills make terminal ownership explicit", () => {
  ...
  assert.match(usingFeatureForge, /Plan revision:/);
  assert.match(usingFeatureForge, /Plan execution mode:/);
  assert.match(
    usingFeatureForge,
  ...
  assert.match(
    ceoReview,
    /Do not draft a plan or offer implementation options from `plan-ceo-review`\./,
  );
  ...
  assert.match(
    engReview,
    /Do not start implementation inside `plan-eng-review`\./,
  );
  assert.match(
    engReview,
...
test("planning review sync docs describe additive review summaries and richer QA handoff", () => {
  ...
  assert.match(
    engReview,
    /loading, empty, error, success, partial, navigation, responsive, and accessibility-critical states/i,
  );
  assert.match(
    engReview,
    /compatibility, retry\/timeout semantics, replay or backfill behavior, and rollback or migration verification/i,
  );
  assert.match(
    engReview,
    /Label the source as `cross-model` only when the outside voice definitely uses a different model\/provider than the main reviewer\./,
...
test("approved workflow-state artifacts document the finalized helper contract", () => {
  ...
  assert.match(
    specDoc,
    new RegExp(
      String.raw`\`${RETIRED_PRODUCT}-workflow-status\` must emit schema-versioned structured diagnostics including \`contract_state\`, \`reason_codes\`, \`diagnostics\`, \`scan_truncated\`, and candidate counts`,
    ),
  ...
  assert.match(
    specDoc,
    new RegExp(
      String.raw`\`${RETIRED_PRODUCT}-plan-execution\` must expose read-only \`preflight\`, \`gate-review\`, and \`gate-finish\` commands`,
    ),
    "approved spec should describe helper-owned execution gates",
  );

  const planDoc = readUtf8(
    path.join(
      REPO_ROOT,
      "docs/archive",
      RETIRED_PRODUCT,
...
test("workflow docs avoid stale ambiguity, commit-ownership, and review-freshness contradictions", () => {
  ...
  assert.match(
    usingFeatureForge,
    /conservatively for the exact relevant artifacts/,
  );
  ...
  assert.match(
    documentRelease,
    /does not own `git commit`, `git merge`, or `git push`/,
  );
  ...
  const routingScenarios = readUtf8(
    path.join(REPO_ROOT, "tests/evals/using-featureforge-routing.scenarios.md"),
  );
  assert.match(
    routingScenarios,
    /branch-completion language still routes to `requesting-code-review` when no fresh final review artifact exists/i,
  );
  assert.match(
    routingScenarios,
    /fresh code-review, QA, and release-readiness artifacts exist/i,
  );
...
test("active eval docs use featureforge state roots", () => {
  ...
  assert.doesNotMatch(
    evalReadme,
    new RegExp(String.raw`~\/\.${RETIRED_PRODUCT}\/(?:evals|projects)\/`),
  );

  const searchBeforeBuildingOrchestrator = readUtf8(
    path.join(
      REPO_ROOT,
  ...
  assert.match(
    searchBeforeBuildingOrchestrator,
    /~\/\.featureforge\/projects\/<slug>\/search-before-building-contract-r2\//,
  );
  assert.doesNotMatch(
    searchBeforeBuildingOrchestrator,
    new RegExp(String.raw`~\/\.${RETIRED_PRODUCT}\/projects\/`),
  );

  const evalObservability = readUtf8(
    path.join(REPO_ROOT, "tests/evals/helpers/eval-observability.mjs"),
  );
  assert.match(evalObservability, /FEATUREFORGE_STATE_DIR/);
  assert.match(evalObservability, /\.featureforge/);
  assert.doesNotMatch(
    evalObservability,
    new RegExp(String.raw`\b${RETIRED_PRODUCT.toUpperCase()}_STATE_DIR\b`),
  );
  assert.doesNotMatch(
    evalObservability,
    new RegExp(String.raw`\.${RETIRED_PRODUCT}`),
  );
});
