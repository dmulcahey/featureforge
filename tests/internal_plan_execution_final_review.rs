// Internal compatibility tests extracted from tests/plan_execution_final_review.rs.
...
fn serial_unit_review_receipt_path(repo: &Path, state: &Path) -> PathBuf {
    let branch = branch_name(repo);
    let safe_branch = branch_storage_key(&branch);
    let execution_run_id = format!("run-{safe_branch}-finish");
    let execution_unit_id = "task-1-step-1";
    harness_authoritative_artifacts_dir(state, &repo_slug(repo), &branch_name(repo)).join(format!(
        "unit-review-{execution_run_id}-{execution_unit_id}.md"
    ))
}
...
fn write_serial_unit_review_receipt(repo: &Path, state: &Path, active_contract_fingerprint: &str) {
    ...
    let receipt_path = serial_unit_review_receipt_path(repo, state);
...
#[test]
fn internal_only_compatibility_gate_finish_treats_missing_serial_unit_review_receipt_as_diagnostic()
{
    let (repo_dir, state_dir) = init_repo("plan-execution-final-review-missing-serial-receipt");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    assert!(
        ...
        "missing serial receipt should be diagnostic only: {}",
        pretty_json(&gate)
    );
}

#[test]
fn internal_only_compatibility_gate_finish_treats_corrupt_serial_unit_review_receipt_as_diagnostic()
{
    let (repo_dir, state_dir) = init_repo("plan-execution-final-review-corrupt-serial-receipt");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let receipt_path = serial_unit_review_receipt_path(repo, state);
    let receipt_source =
        fs::read_to_string(&receipt_path).expect("serial unit-review receipt should be readable");
    write_file(
        &receipt_path,
    ...
    assert!(
        ...
        "corrupt serial receipt should be diagnostic only: {}",
        pretty_json(&gate)
    );
}
