...
#[test]
fn workflow_doctor_and_handoff_expose_scope_and_release_snapshot_fields() {
    let (repo_dir, state_dir) = init_repo("workflow-doctor-handoff-scope-release-parity");
    let repo = repo_dir.path();
    let state = state_dir.path();
    let plan_rel = "docs/featureforge/plans/2026-03-22-runtime-integration-hardening.md";
    let base_branch = current_branch_name(repo);
    let session_key = "workflow-doctor-handoff-scope-release-parity";
    setup_document_release_pending_case(repo, state, plan_rel, &base_branch);
    enable_session_decision(state, session_key);
    let env = [("FEATUREFORGE_SESSION_KEY", session_key)];
    ...
    let doctor_json = run_featureforge_with_env_json(
        ...
        &["workflow", "doctor", "--json"],
        &env,
        "workflow doctor json for shell-smoke scope and release parity",
    );
    let handoff_json = run_featureforge_with_env_json(
        repo,
        state,
        &["workflow", "handoff", "--json"],
        &env,
        "workflow handoff json for shell-smoke scope and release parity",
    );
    let doctor_text_output = run_featureforge_with_env(
        repo,
        state,
        &["workflow", "doctor"],
        &env,
        "workflow doctor text for shell-smoke scope and release parity",
    );
    assert!(doctor_text_output.status.success());
    let doctor_text = String::from_utf8_lossy(&doctor_text_output.stdout);
    let handoff_text_output = run_featureforge_with_env(
        ...
        &["workflow", "handoff"],
        &env,
        "workflow handoff text for shell-smoke scope and release parity",
    );
    assert!(handoff_text_output.status.success());
    let handoff_text = String::from_utf8_lossy(&handoff_text_output.stdout);

    assert_eq!(
        doctor_json["scope_check"]["result"],
        handoff_json["scope_check"]["result"],
        "doctor and handoff should share the same scope-check snapshot result"
    );
    assert_eq!(
        doctor_json["release_readiness"]["release_surface"],
        handoff_json["release_readiness"]["release_surface"],
        "doctor and handoff should share the same release-surface snapshot"
    );
    assert_eq!(
        doctor_json["release_readiness"]["versioning_decision"],
        handoff_json["release_readiness"]["versioning_decision"],
        "doctor and handoff should share the same versioning snapshot"
    );
    assert!(doctor_text.contains(&format!(
        "Scope check: {}",
        doctor_json["scope_check"]["result"]
            .as_str()
            .expect("workflow doctor should expose scope_check.result")
    )));
    assert!(doctor_text.contains(&format!(
        "Release surface: {}",
        doctor_json["release_readiness"]["release_surface"]
            .as_str()
            .expect("workflow doctor should expose release_readiness.release_surface")
    )));
    assert!(doctor_text.contains(&format!(
        "Versioning decision: {}",
        doctor_json["release_readiness"]["versioning_decision"]
            .as_str()
            .expect("workflow doctor should expose release_readiness.versioning_decision")
    )));
    assert!(handoff_text.contains(&format!(
        "Scope check: {}",
        handoff_json["scope_check"]["result"]
            .as_str()
            .expect("workflow handoff should expose scope_check.result")
    )));
}
