//! Using featureforge skill integration/benchmark crate.
...
fn read_skill_doc() -> String {
    fs::read_to_string(skill_doc_path())
        .expect_or_abort("using-featureforge skill doc should be readable")
}
...
fn read_route_contract_fixture() -> String {
    fs::read_to_string(route_contract_fixture_path())
        .expect_or_abort("using-featureforge route contract fixture should be readable")
}
...
fn extract_last_nonempty_line(output: &[u8], context: &str) -> String {
    String::from_utf8(output.to_vec())
        .unwrap_or_else(|error| featureforge::abort!("{context} should emit utf8: {error}"))
        .lines()
        .rev()
        .find(|line| !line.trim().is_empty())
        .unwrap_or_else(|| featureforge::abort!("{context} should emit a non-empty line"))
        .to_owned()
}
...
fn parse_json_stdout_lines(output: &[u8], context: &str) -> Vec<Value> {
    ...
    let first_json_index = lines
        ...
        .unwrap_or_else(|| featureforge::abort!("{context} should emit at least one json line"));
    ...
    lines[first_json_index..]
        ...
        .map(|line| {
            serde_json::from_str(line).unwrap_or_else(|error| {
                featureforge::abort!(
                    "{context} should emit valid json lines after the preamble output: {error}"
                )
            })
        })
        .collect()
...
fn simulate_supported_route_selection_batch(
    ...
    fs::create_dir_all(&message_dir).unwrap_or_else(|error| {
        featureforge::abort!("{} should create message dir: {error}", harness.session_key);
    });
...
fn using_featureforge_skill_uses_shared_preamble_without_session_entry_gate() {
    ...
    let explicit_memory_route_index = content
        ...
        .expect_or_abort("using-featureforge skill should document explicit-memory routing precedence");
    let implementation_ready_index = content
        ...
        .expect_or_abort("using-featureforge skill should still document implementation-ready handoff routing");
    assert!(
    ...
    let temp_home = TempDir::new().expect_or_abort("home tempdir should exist");
...
fn using_featureforge_preamble_requires_the_packaged_runtime_binary() {
    ...
    let stdout = String::from_utf8(output.stdout)
        .expect_or_abort("runtime root without packaged binary should emit utf8");
    assert_eq!(
        stdout.trim_end(),
        "",
...
fn install_compiled_featureforge_skips_redundant_recopy_for_same_home() {
    ...
    let first_modified = fs::metadata(&first_target)
        .expect_or_abort("installed binary should exist after first install")
        .modified()
        .expect_or_abort("installed binary should expose modified time after first install");

    sleep(Duration::from_millis(1200));

    let second_target = install_compiled_featureforge(temp_home.path());
    let second_modified = fs::metadata(&second_target)
        .expect_or_abort("installed binary should exist after second install")
        .modified()
        .expect_or_abort("installed binary should expose modified time after second install");
...
fn using_featureforge_project_memory_carveout_stays_explicit_and_workflow_bound() {
    {
        ...
        for active_owner in [
            ...
            let entries =
                simulate_supported_route_selection_batch(state, home, &harness, &messages);
