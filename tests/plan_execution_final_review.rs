...
- Versioning Rationale: focused finish-gate fixture with no published package surface.
...
#[test]
fn gate_finish_requires_release_readiness_before_final_review_artifact() {
    let (repo_dir, state_dir) = init_repo("plan-execution-final-review-release-before-review");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    let gate = run_plan_execution_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate-finish release-readiness precedence should run",
    );

    assert_eq!(gate["allowed"], false);
    assert_eq!(gate["failure_class"], "ReleaseArtifactNotFresh");
    assert!(
        gate["reason_codes"]
            .as_array()
            .is_some_and(|codes| codes.iter().any(|code| code == "release_artifact_missing")),
        "gate-finish should require release readiness before final review when release docs are unresolved, got {gate:?}"
    );
}
...
#[test]
fn gate_finish_requires_scope_check_and_release_readiness_metadata() {
    let (repo_dir, state_dir) = init_repo("plan-execution-final-review-scope-and-release-metadata");
    let repo = repo_dir.path();
    let state = state_dir.path();
    ...
    fs::write(
        &review_path,
        review_source
            .replace("**Scope Check Result:** CLEAN\n", "")
            .replace("**Scope Check Resolution:** not_required\n", ""),
    )
    .expect("review artifact should be writable");
    let release_path = write_release_readiness_artifact(repo, state, &base_branch);
    let release_source = fs::read_to_string(&release_path).expect("release artifact should read");
    fs::write(
        &release_path,
        release_source
            .replace("**Versioning Decision:** none\n", "")
            .replace(
                "**Versioning Rationale:** Internal workflow runtime change with no published package bump.\n",
                "",
            ),
    ...
    let gate = run_plan_execution_json(
        ...
        state,
        &["gate-finish", "--plan", PLAN_REL],
        "gate-finish should require scope-check and release-readiness metadata",
    );

    assert_eq!(gate["allowed"], false);
    assert!(
        gate["reason_codes"].as_array().is_some_and(|codes| {
            codes.iter().any(|code| code == "review_receipt_scope_check_missing")
                || codes
                    .iter()
                    .any(|code| code == "release_readiness_versioning_missing")
        }),
        "gate-finish should block when final review and release readiness omit scope-check or versioning metadata"
    );
}
