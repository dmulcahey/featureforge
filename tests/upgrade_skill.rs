//! Upgrade skill integration/benchmark crate.
...
fn read_skill_doc() -> String {
    fs::read_to_string(skill_doc_path())
        .expect_or_abort("featureforge-upgrade skill doc should be readable")
}
...
fn assert_no_runtime_fallback_execution(content: &str, label: &str) {
    ...
    for needle in [
        concat!("${", "_FEATUREFORGE_BIN:-featureforge}"),
...
fn make_valid_install(dir: &Path, git_mode: &str) {
    fs::create_dir_all(dir.join("bin")).expect_or_abort("install bin dir should exist");
    ...
    match git_mode {
        "dir" => {
            fs::create_dir_all(dir.join(".git")).expect_or_abort(".git dir should exist");
        }
        "file" => {
            write_file(&dir.join(".git"), "gitdir: /tmp/fake-worktree\n");
        }
        _ => featureforge::abort!("unexpected git mode {git_mode}"),
    }
}

fn make_windows_only_install(dir: &Path, git_mode: &str) {
    fs::create_dir_all(dir.join("bin")).expect_or_abort("windows install bin dir should exist");
...
fn write_mock_featureforge(bin_dir: &Path, script_body: &str) {
    fs::create_dir_all(bin_dir).expect_or_abort("mock featureforge bin dir should exist");
...
fn upgrade_skill_contract_tracks_doc_patterns_and_install_root_resolution() {
    {
        ...
        for pattern in [
            ...
            concat!("FEATUREFORGE_RUNTIME_BIN=\"${", "_FEATUREFORGE_BIN:-}\""),
            ...
            concat!("INSTALL_DIR=\"${", "_FEATUREFORGE_ROOT:-}\""),
            ...
            concat!(
                "REMOTE_URL=\"${",
                "FEATUREFORGE_REMOTE_URL:-https://raw.githubusercontent.com/dmulcahey/featureforge/main/VERSION}\""
            ),
            "REMOTE_STATUS=",
            "VERSION_RELATION=",
        ...
        fs::create_dir_all(&home_dir).expect_or_abort("home should exist");
        let current_root = tmp_root.path().join("current-project");
        fs::create_dir_all(&current_root).expect_or_abort("project root should exist");
        ...
        install_mock_featureforge(
            ...
            &format!(
                ...
                resolved_runtime_root_path(
                    &renamed_root
                        .canonicalize()
                        .expect_or_abort("renamed root canonicalize")
                )
            ),
        );
        ...
        write_mock_featureforge(
            selected_bin
                .parent()
                .expect_or_abort("selected bin parent should exist"),
            &format!(
                "#!/usr/bin/env bash\nif [ \"${{1:-}}\" = \"repo\" ] && [ \"${{2:-}}\" = \"runtime-root\" ] && [ \"${{3:-}}\" = \"--path\" ]; then\n  printf '%s' '{}'\n  exit 0\nfi\nif [ \"${{1:-}}\" = \"repo\" ] && [ \"${{2:-}}\" = \"runtime-root\" ] && [ \"${{3:-}}\" = \"--field\" ] && [ \"${{4:-}}\" = \"upgrade-eligible\" ]; then\n  printf 'true\\n'\n  exit 0\nfi\nexit 0\n",
        ...
        if canonical_unix_helper.exists() {
            fs::remove_file(&canonical_unix_helper).expect_or_abort(
                "windows-only direct-upgrade fixture should remove the unix helper",
            );
        }
        ...
        fs::create_dir_all(&non_runtime_dir).expect_or_abort("non-runtime dir should exist");
        install_mock_featureforge(
            &home_dir,
            &format!(
                "#!/usr/bin/env bash\nif [ \"${{1:-}}\" = \"repo\" ] && [ \"${{2:-}}\" = \"runtime-root\" ] && [ \"${{3:-}}\" = \"--path\" ]; then\n  printf '%s' '{}'\n  exit 0\nfi\nif [ \"${{1:-}}\" = \"repo\" ] && [ \"${{2:-}}\" = \"runtime-root\" ] && [ \"${{3:-}}\" = \"--field\" ] && [ \"${{4:-}}\" = \"upgrade-eligible\" ]; then\n  printf 'false\\n'\n  exit 0\nfi\nexit 0\n",
        ...
        fs::create_dir_all(not_upgrade_eligible.join("bin"))
            .expect_or_abort("non-upgrade-eligible bin dir");
        ...
        fs::remove_file(canonical_install_bin(&home_dir))
            .expect_or_abort("mock helper should remove");
        fs::remove_file(
            canonical_install_root(&home_dir)
                .join("bin")
                .join("featureforge.exe"),
        )
        .expect_or_abort("windows packaged helper should remove");
...
fn valid_install_fixture_includes_checked_in_prebuilt_layout() {
    let install_root = TempDir::new().expect_or_abort("install root tempdir should exist");
...
fn upgrade_skill_version_resolution_matches_shell_contract() {
    {
        ...
        let version_root = TempDir::new().expect_or_abort("version root should exist");
