...
- `B)` `~/.config/featureforge/worktrees/<project-name>/` (global location)
...
### For Global Directory (~/.config/featureforge/worktrees)
...
case $LOCATION in
  .worktrees|worktrees)
    ...
    ;;
  ~/.config/featureforge/worktrees/*)
    path="~/.config/featureforge/worktrees/$project/$BRANCH_NAME"
    ;;
esac
