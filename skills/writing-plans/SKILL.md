...
1. After the plan is written or updated, continue using the same repo-relative plan path in plan-fidelity-review and workflow/operator handoffs.

2. Invoke `featureforge:plan-fidelity-review`.
3. Do NOT invoke `featureforge:plan-eng-review` directly from `writing-plans`; `plan-fidelity-review` owns dedicated independent review plus runtime-owned receipt recording.
4. Keep this handoff fail-closed:
   - if plan fidelity fails, return to `featureforge:writing-plans`
   - if plan fidelity passes and the receipt records, continue to `featureforge:plan-eng-review`

**The terminal state is invoking `featureforge:plan-fidelity-review`.**
