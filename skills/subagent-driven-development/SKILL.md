...
`featureforge workflow operator --plan <approved-plan-path>` is authoritative for `phase`, `next_action`, and `recommended_command`. `featureforge plan execution status --plan <approved-plan-path>` is supporting diagnostic detail for execution telemetry, active blockers, and checkpoint fingerprints; it does not replace the operator routing contract.
...
| Task review and verification are both green | Dedicated-independent review is green and verification receipt exists | `featureforge plan execution close-current-task --plan <approved-plan-path> --task <n> --dispatch-id <dispatch-id> --review-result pass|fail --review-summary-file <review-summary> --verification-result pass|fail --verification-summary-file <verification-summary>` | Current task closure is recorded for the still-current reviewed state | If review-state drift blocks closure, run `featureforge plan execution repair-review-state --plan <approved-plan-path>` |
| Operator reports stale-unreviewed, escaped, or ambiguous review-state drift | Operator review-state is not clean | `featureforge plan execution repair-review-state --plan <approved-plan-path>` | Review-state blocker is reconciled or restated as the authoritative next action | Compatibility-only fallback: `featureforge plan execution explain-review-state --plan <approved-plan-path>` or `featureforge plan execution reconcile-review-state --plan <approved-plan-path>` |
...
| Operator reports release-readiness or final-review late-stage recording ready | Current branch closure exists and operator recommends late-stage advance | `featureforge plan execution advance-late-stage --plan <approved-plan-path> ...` | Aggregate late-stage trace names the delegated primitive and recorded milestone keyed by `stage_path` | Compatibility-only fallback: use `record-release-readiness` or `record-final-review` only for explicit debug or compatibility paths |
