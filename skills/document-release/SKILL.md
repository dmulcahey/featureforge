...
When late-stage workflow gates are unresolved, this pass runs before `featureforge:requesting-code-review` so final code review sees the near-finished repo state instead of staling immediately after release-doc work lands.

Use the approved plan's `Risk & Gate Signals` and `Release & Distribution Notes` as the canonical routing truth for release surface, versioning, and distribution expectations.
...
3. If the change introduces or modifies a user-facing artifact, entry point, package, route, flag, workflow, or deployable surface, is publishability / distribution status explicitly checked and documented?
4. Is there an explicit `Versioning Decision` and rationale whenever the repo has a versioned release surface?
5. Are rollout notes documented when the change meaningfully affects release or operations?
6. Are rollback notes documented when rollback is non-trivial?
7. Are known risks or operator-facing caveats documented when they matter?
8. Are monitoring or verification expectations documented when the change introduces operational risk?
...
**Versioning Rationale:** Internal workflow/runtime change with no published package or release-number bump.
**Result:** pass
