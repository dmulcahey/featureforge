...
When a dispatched child will start a fresh FeatureForge conversation, set `FEATUREFORGE_SPAWNED_SUBAGENT=1` in the child environment so the runtime suppresses first-turn session-entry noise by default. Add `FEATUREFORGE_SPAWNED_SUBAGENT_OPT_IN=1` only when you intentionally want the nested child to re-enter FeatureForge and permit enabled-state persistence.
