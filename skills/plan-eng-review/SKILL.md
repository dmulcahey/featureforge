...
description: Use when a written FeatureForge implementation plan from a CEO-approved spec needs engineering review before execution or when workflow/operator explicitly routes to late refresh-test-plan regeneration
---
...
- Treat `Requirement Index`, `Requirement Coverage Matrix`, canonical `## Task N:` headings, `Spec Coverage`, `Goal`, `Context`, `Constraints`, `Done when`, and `Files:` blocks as required plan contract surface for engineering approval. `$_FEATUREFORGE_ROOT/review/plan-task-contract.md` is the authoritative field, determinism, spec-reference, obligation-index, migration, and hard-fail reuse law.
...
```bash
$_FEATUREFORGE_BIN repo-safety check --intent write --stage featureforge:plan-eng-review --task-id <current-plan-review> --path docs/featureforge/plans/YYYY-MM-DD-<feature-name>.md --write-target plan-artifact-write
```
...
```bash
$_FEATUREFORGE_BIN repo-safety approve --stage featureforge:plan-eng-review --task-id <current-plan-review> --reason "<explicit user approval>" --path docs/featureforge/plans/YYYY-MM-DD-<feature-name>.md --write-target plan-artifact-write
$_FEATUREFORGE_BIN repo-safety check --intent write --stage featureforge:plan-eng-review --task-id <current-plan-review> --path docs/featureforge/plans/YYYY-MM-DD-<feature-name>.md --write-target plan-artifact-write
```
...
plan-eng-review also owns the late refresh-test-plan lane only when workflow/operator explicitly routes to `qa_pending` with `phase_detail=test_plan_refresh_required` for the current approved plan revision. Missing or stale source test-plan projections on current QA recording or finish readiness are diagnostic-only and must not be treated as a refresh request by themselves.
...
- Use the existing ENG review sections as canonical section boundaries and brief the reviewer with skill-local `accelerated-reviewer-prompt.md`.
- That reviewer prompt plus `$_FEATUREFORGE_ROOT/review/review-accelerator-packet-contract.md` defines required section-packet schema and keeps the reviewer limited to draft-only output.
...
8. After a current pass plan-fidelity review artifact exists for the approval-stable plan-fidelity binding surfaced by workflow routing, return here for the final approval-header mutation.
...
Engineering approval must also fail closed unless `plan_fidelity_review.state == pass` for the current plan-fidelity binding/spec fingerprint.
...
- If workflow/operator JSON returns `phase` `executing`, present the normal execution preflight handoff below.
- If workflow/operator JSON returns a later phase such as `task_closure_pending`, `document_release_pending`, `final_review_pending`, `qa_pending`, or `ready_for_branch_completion`, follow that reported route instead of reopening execution preflight; use the Installed Control Plane section and canonical route reference for any public mutation.
- `$_FEATUREFORGE_BIN plan execution status --plan <approved-plan-path>` is supporting diagnostic detail only; do not let it override workflow/operator routing.
- If workflow/operator is unavailable or fails, stop and report the doctor/operator diagnostic. Fix only obvious installed-binary, state-dir, or repo-root binding, then rerun workflow/operator JSON; do not reconstruct execution handoff from artifacts manually.
