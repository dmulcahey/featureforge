...
7. If the matching plan-fidelity receipt is missing, stale, malformed, non-pass, or non-independent, stop and hand control back to `featureforge:plan-fidelity-review`.
...
Write to `$_FEATUREFORGE_STATE_DIR/projects/$SLUG/{user}-{safe-branch}-test-plan-{datetime}.md` with:
...
Set `**Browser QA Required:** yes` when approved-plan `**QA Requirement:** required` and the branch-specific routes or interactions define the tester-facing browser QA handoff. Otherwise write `no`. This field scopes the QA artifact for testers; it is not the authoritative finish-gate policy source.
...
- Treat workflow/operator `phase`, `phase_detail`, `review_state_status`, `next_action`, and `recommended_command` as authoritative for public routing.
...
During that handoff, keep routing anchored to `featureforge workflow operator --plan <approved-plan-path>` output and present the runtime-selected execution owner skill first.
