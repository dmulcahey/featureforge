...
- If the handoff reports a later phase such as `document_release_pending`, `final_review_pending`, `qa_pending`, `review_blocked`, or `ready_for_branch_completion`, follow that reported phase and `next_action` instead of resuming `featureforge:subagent-driven-development` or `featureforge:executing-plans` just because `execution_started` is `yes`.
