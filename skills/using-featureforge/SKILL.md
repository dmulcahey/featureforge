...
- Treat tracked plan/evidence/review/readiness/QA/strategy projection files as optional materialized exports. Normal routing comes from workflow/operator and event-authoritative status, not from editing or refreshing those files.
- Use `featureforge plan execution materialize-projections --plan <approved-plan-path> --scope execution|late-stage|all --tracked` only when a tracked human-readable export is explicitly requested; never require it for normal progress.
