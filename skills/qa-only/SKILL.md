...
For workflow-routed QA, this project artifact is the structured finish-gate input. Copy `Source Plan` and `Source Plan Revision` from the resolved test-plan artifact when it exists, and point `Source Test Plan` at that exact artifact path.
