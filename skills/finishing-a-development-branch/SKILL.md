...
If the operator reports `qa_pending` with `phase_detail=qa_recording_required`, use input shape `featureforge plan execution advance-late-stage --plan <approved-plan-path> --result pass|fail --summary-file <qa-report>` only after substituting concrete values, then rerun `featureforge workflow operator --plan <approved-plan-path>`.
