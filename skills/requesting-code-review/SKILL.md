...
- Required checkpoint behavior:
  - `review_remediation`: runtime records this automatically when remediation work is reopened after non-pass review findings.
  - `cycle_break`: runtime records this automatically when the same task hits three reopen/remediation cycles in one run.
- Cycle-break trigger: cap remediation churn at 3 reopen cycles per task. On the third cycle, runtime enters `cycle_break` strategy automatically (no human replanning loopback required).
