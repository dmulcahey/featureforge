...
For workflow-routed finish work, this is the independent final code-review stage after required pre-final-review completion gates such as `document-release` and `security-review`, and before verification gates such as `qa-only`.
...
- Persist explicit scope-check disposition. Every workflow-routed final review must compare intended scope against the actual implementation and record `Scope Check Result: CLEAN | DRIFT_DETECTED | REQUIREMENTS_MISSING` plus `Scope Check Resolution`.
- `CLEAN` must pair with `Scope Check Resolution: not_required`.
- `DRIFT_DETECTED` must pair with an explicit resolution such as `accepted`, `todo_follow_up`, or `reopen_execution`.
- `REQUIREMENTS_MISSING` is a blocking review outcome. Do not record a passing final-review artifact until execution has been reopened or the missing requirements are otherwise resolved.
