...
function normalizeRelativePath(input) {
  if (input.length === 0 || import_node_path.default.isAbsolute(input)) {
    return null;
  }
...
// src/core/plan-execution.ts
var PlanExecutionFailure = class extends Error {
  failureClass;
...
function activeSummaryFromTitle(title) {
  return truncateWithEllipsis(normalizeWhitespace(title), 120);
}
function reopenNoteSummary(reason) {
  return truncateWithEllipsis(`Reopened: ${normalizeWhitespace(reason)}`, 120);
}
function currentTimestamp() {
  return (/* @__PURE__ */ new Date()).toISOString().replace(/\.\d{3}Z$/, "Z");
}
function toCommandResult(exitCode, stdout = "", stderr = "") {
  return { exitCode, stdout, stderr };
}
function errorResult(failureClass, message) {
...
function stripPlanFileReferenceSuffix(value) {
  return value.replace(/:[0-9]+(?:[:-][0-9]+)*$/, "");
}
function normalizePlanScopePath(value) {
  return normalizeRepoRelativePath(stripPlanFileReferenceSuffix(value));
}
function planRelToAbs(repoRoot, relativePath) {
  return import_node_path2.default.join(repoRoot, relativePath);
}
function computeExecutionFingerprint(planText, evidenceText, evidenceEmptyState) {
...
function validatePersistedNormalizedText(rawValue, emptyMessage, overlongMessage, maxLength) {
...
function validatePersistedRepoRelativePath(rawValue, message) {
...
function validatePersistedExecutionSource(rawValue, planExecutionMode) {
  const normalized = normalizeWhitespace(rawValue);
  if (normalized !== "superpowers:executing-plans" && normalized !== "superpowers:subagent-driven-development") {
    fail(
...
function validateRequiredNormalizedText(rawValue, emptyMessage, overlongMessage, maxLength) {
  const normalized = normalizeWhitespace(rawValue);
  if (normalized.length === 0) {
...
function validateNoteMessage(message) {
...
function validateExecutionModeSource(source, state) {
...
function firstMatchingGroup(text, pattern) {
...
function validateSourceSpec(repoRoot, sourceRel, expectedRevision) {
...
function parsePlanFile(planText) {
  const steps = [];
  let planWorkflowState = "";
  let planRevision = "";
  let planExecutionMode = "";
  let planSourceSpec = "";
  ...
  for (const line of splitNormalizedLines(planText)) {
    ...
    if (match) {
      ...
      pendingStep.noteState = noteMatch[1];
  ...
  let planActiveTask = null;
  let planActiveStep = null;
  let planBlockingTask = null;
  let planBlockingStep = null;
  let planResumeTask = null;
  let planResumeStep = null;
...
function parseEvidenceFileMetadata(evidenceText, expectedPlanRel, expectedRevision) {
...
function parseEvidenceAttempts(evidenceText, planExecutionMode) {
  const attempts = [];
  const lines = splitNormalizedLines(evidenceText);
  let currentTask = "";
  let currentStep = "";
  ...
  let index = 0;
  let persistedEvidenceSource = null;
  ...
  for (const attempt of attempts) {
    ...
    if (attempt.recordedAt.length === 0 || attempt.source.length === 0 || attempt.claim.length === 0 || attempt.files.length === 0 || attempt.verification.length === 0) {
      fail(
        "MalformedExecutionState",
...
function loadExecutionState(planRel, environment) {
  ...
  if (parsedPlan.planExecutionMode !== "none" && parsedPlan.planExecutionMode !== "superpowers:executing-plans" && parsedPlan.planExecutionMode !== "superpowers:subagent-driven-development") {
    fail(
      "PlanNotExecutionReady",
  ...
  const evidenceMetadata = evidenceExists ? parseEvidenceFileMetadata(evidenceText, planRelPath, parsedPlan.planRevision) : { evidenceEmptyState: true, evidenceAttemptCount: 0 };
  const attempts = evidenceExists && !evidenceMetadata.evidenceEmptyState ? parseEvidenceAttempts(evidenceText, parsedPlan.planExecutionMode) : [];
  ...
  let executionStarted = parsedPlan.planExecutionMode === "none" ? "no" : "yes";
  if (parsedPlan.planCheckedCount > 0 || parsedPlan.planNoteCount > 0 || evidenceMetadata.evidenceAttemptCount > 0) {
...
function serializeStatusJson(state) {
  const payload = {
...
function renderStatus(planRel, environment) {
  const state = loadExecutionState(planRel, environment);
  return toCommandResult(0, serializeStatusJson(state));
}
function deriveEvidenceRelPath(planRel, revision) {
  const base = import_node_path2.default.posix.basename(planRel, ".md");
  return `docs/superpowers/execution-evidence/${base}-r${revision}-evidence.md`;
}
function deriveTasksIndependentFromPlan(planText) {
  const taskScopes = [];
  const taskWriteCounts = [];
  const taskFileBlocks = [];
  const taskScopeParseable = [];
...
function serializeRecommendJson(payload) {
  return `${JSON.stringify(payload)}
`;
}
function assertExpectedFingerprint(state, expectedFingerprint) {
...
function findLatestAttemptIndex(state, task, step) {
  let latestIndex = -1;
  let latestNumber = 0;
...
function nextAttemptNumber(state, task, step) {
...
function appendAttempt(state, task, step, source, claim, files, verification) {
  state.attempts.push({
    task,
    step,
...
function invalidateAttempt(state, attemptIndex, source, reason) {
...
function renderEvidenceFile(state) {
  const lines = [];
  const topic = import_node_path2.default.posix.basename(state.planRelPath, ".md");
...
function writeTempFile(targetPath, contents) {
  ensureDirectoryExists(import_node_path2.default.dirname(targetPath));
  const tempPath = `${targetPath}.tmp-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  import_node_fs2.default.writeFileSync(tempPath, contents, "utf8");
  return tempPath;
}
...
function commitPlanOnly(state, operation) {
  ...
  try {
    import_node_fs2.default.renameSync(tempPlanPath, state.planAbsPath);
  } catch {
    cleanupFile(tempPlanPath);
    fail("EvidenceWriteFailed", "Could not persist the plan mutation.");
  }
}
function commitPlanAndEvidence(state, operation) {
  ...
  try {
    import_node_fs2.default.renameSync(tempEvidencePath, state.evidenceAbsPath);
  } catch {
    cleanupFile(tempPlanPath);
    cleanupFile(tempEvidencePath);
    try {
      import_node_fs2.default.writeFileSync(state.planAbsPath, originalPlan, "utf8");
    } catch {
    }
  ...
  try {
    ...
    try {
      ...
      if (originalEvidenceExists) {
        import_node_fs2.default.writeFileSync(state.evidenceAbsPath, originalEvidence, "utf8");
      } else {
...
function parseDiffNameStatus(output, query) {
  ...
  while (index < tokens.length) {
    ...
    if (status.startsWith("R") || status.startsWith("C")) {
      const pathOne2 = tokens[index] ?? "";
      const pathTwo = tokens[index + 1] ?? "";
      index += 2;
      if (query === pathOne2 || query === pathTwo) {
        return pathTwo;
      }
      continue;
    }
...
function resolvePathFromCurrentChangeSet(state, query) {
  const cached = (0, import_node_child_process.spawnSync)("git", ["-C", state.repoRoot, "diff", "--name-status", "-z", "-M", "--cached"], {
    encoding: "utf8"
  });
  if (cached.status === 0) {
  ...
  const working = (0, import_node_child_process.spawnSync)("git", ["-C", state.repoRoot, "diff", "--name-status", "-z", "-M"], {
    encoding: "utf8"
  });
  if (working.status === 0) {
  ...
  const untracked = (0, import_node_child_process.spawnSync)(
    "git",
...
function buildFilesEntry(state, inputs) {
  if (inputs.length === 0) {
  ...
  for (const input of inputs) {
    ...
    if (pathExists(absolutePath) || import_node_fs2.default.existsSync(absolutePath)) {
...
function commandStatus(args, environment) {
  let planRel = "";
...
function commandRecommend(args, environment) {
  let planRel = "";
  let isolatedAgents = "unknown";
  let sessionIntent = "unknown";
  let workspacePrepared = "unknown";
  for (let index = 0; index < args.length; ) {
    const current = args[index] ?? "";
    switch (current) {
      case "--plan": {
  ...
  try {
    ...
    const recommendedSkill = tasksIndependent === "yes" && isolatedAgents === "yes" && sameSessionViable === "yes" ? "superpowers:subagent-driven-development" : "superpowers:executing-plans";
    const reason = recommendedSkill === "superpowers:subagent-driven-development" ? "Independent tasks and same-session isolated execution are viable." : "Defaulting conservatively because the available signals do not positively justify isolated same-session execution.";
    return toCommandResult(
      0,
...
function commandBegin(args, environment) {
  let planRel = "";
  let task = "";
  let step = "";
  ...
  try {
    ...
    if (state.planExecutionMode === "none") {
      if (executionModeInput !== "superpowers:executing-plans" && executionModeInput !== "superpowers:subagent-driven-development") {
        return errorResult(
          "InvalidExecutionMode",
    ...
    if (state.planActiveTask !== null || state.planBlockingTask !== null || state.planResumeTask !== null) {
      ...
      if (state.planBlockingTask !== null && (state.planBlockingTask !== task || state.planBlockingStep !== step)) {
        return errorResult("InvalidStepTransition", "Blocked work must resume on the same step.");
      }
      if (state.planResumeTask !== null && state.planActiveTask === null && state.planBlockingTask === null && (state.planResumeTask !== task || state.planResumeStep !== step)) {
        return errorResult("InvalidStepTransition", "Interrupted work must resume on the same step.");
      }
      if (state.planActiveTask !== null && (state.planActiveTask !== task || state.planActiveStep !== step)) {
        return errorResult("InvalidStepTransition", "A different step is already active.");
      }
      if (state.planBlockingTask === task && state.planBlockingStep === step) {
      } else if (state.planResumeTask === task && state.planResumeStep === step) {
...
function commandTransfer(args, environment) {
  let planRel = "";
  let repairTask = "";
  let repairStep = "";
  let source = "";
  ...
  if (planRel.length === 0 || repairTask.length === 0 || repairStep.length === 0 || source.length === 0 || reason.length === 0 || expectedFingerprint.length === 0) {
    return errorResult(
      "InvalidCommandInput",
...
function commandComplete(args, environment) {
  let planRel = "";
  let task = "";
  let step = "";
  let source = "";
  ...
  if (planRel.length === 0 || task.length === 0 || step.length === 0 || source.length === 0 || claim.length === 0 || expectedFingerprint.length === 0) {
    return errorResult(
      "InvalidCommandInput",
...
function commandNote(args, environment) {
  let planRel = "";
  let task = "";
  let step = "";
  let noteState = "";
  ...
  if (planRel.length === 0 || task.length === 0 || step.length === 0 || noteState.length === 0 || message.length === 0 || expectedFingerprint.length === 0) {
    return errorResult(
      "InvalidCommandInput",
  ...
  try {
    ...
    } else if (noteState === "blocked" && state.planActiveTask === null && state.planResumeTask === task && state.planResumeStep === step) {
    } else {
      return errorResult(
        "InvalidStepTransition",
...
function commandReopen(args, environment) {
  let planRel = "";
  let task = "";
  let step = "";
  let source = "";
  ...
  if (planRel.length === 0 || task.length === 0 || step.length === 0 || source.length === 0 || reason.length === 0 || expectedFingerprint.length === 0) {
    return errorResult(
      "InvalidCommandInput",
...
function runPlanExecutionCommand(args, environment = {}) {
