...
impl AuthoritativeTransitionState {
    ...
    pub(crate) fn record_reopen_strategy_checkpoint(
        ...
        let trigger = vec![format!("task-{task}:step-{step}:cycle-{cycle_count}")];
        if cycle_count >= 3 {
            self.record_strategy_checkpoint(
                context,
                "cycle_break",
                execution_mode,
                &trigger,
                "Runtime detected churn after three review-remediation cycles for the same task and auto-entered cycle-break strategy.",
                true,
            )?;
