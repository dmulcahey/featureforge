...
pub struct AnalyzePlanReport {
    ...
    /// Runtime field.
    pub coverage_complete: BoolFlag,
    /// Runtime field.
    pub open_questions_resolved: BoolFlag,
    /// Runtime field.
    pub task_structure_valid: BoolFlag,
    /// Runtime field.
    pub files_blocks_valid: BoolFlag,
    /// Runtime field.
    pub execution_strategy_present: BoolFlag,
    /// Runtime field.
    pub dependency_diagram_present: BoolFlag,
    /// Runtime field.
    pub execution_topology_valid: BoolFlag,
    /// Runtime field.
    pub serial_hazards_resolved: BoolFlag,
    /// Runtime field.
    pub parallel_lane_ownership_valid: BoolFlag,
    /// Runtime field.
    pub parallel_workspace_isolation_valid: BoolFlag,
    /// Runtime field.
...
pub fn analyze_documents(spec: &SpecDocument, plan: &PlanDocument) -> AnalyzePlanReport {
    {
        ...
        AnalyzePlanReport {
            ...
            coverage_complete: coverage_complete.into(),
            open_questions_resolved: open_questions_resolved.into(),
            task_structure_valid: task_structure_valid.into(),
            files_blocks_valid: files_blocks_valid.into(),
...
pub fn evaluate_plan_fidelity_receipt_at_path(
...
) -> PlanFidelityGateReport {
    {
        ...
        let stale_spec_binding = (
            receipt.spec_path.as_str(),
            receipt.spec_revision,
            receipt.spec_fingerprint.as_str(),
        ) != (
            spec.path.as_str(),
            spec.spec_revision,
            spec_fingerprint.as_str(),
        );
        let stale_plan_binding = (
            receipt.plan_path.as_str(),
        ...
        let stale_binding = stale_spec_binding || stale_plan_binding;
        if stale_binding {
            push_diagnostic(
                &mut diagnostics,
                &mut reason_codes,
...
fn parse_plan_step_line(line: &str) -> Result<Option<(u32, String)>, DiagnosticError> {
    ...
    let mark = rest.chars().next().ok_or_else(|| {
        DiagnosticError::new(
            FailureClass::InstructionParseFailed,
            format!("Malformed step entry: {line}"),
        )
    ...
    let rest = &rest[mark.len_utf8()..];
    let rest = rest.strip_prefix("] **Step ").ok_or_else(|| {
        DiagnosticError::new(
            FailureClass::InstructionParseFailed,
            format!("Malformed step entry: {line}"),
        )
...
pub(crate) fn analyze_execution_topology(
...
) -> ExecutionTopologyAnalysis {
    {
            ...
            for task in &sorted_tasks {
                if !task_numbers.contains(task) {
                    push_diagnostic(
                        ...
                        &format!(
                            "Execution Strategy references Task {task} but the task does not exist in the plan."
                        ),
                    );
                    continue;
                }
                    ...
                    .is_some()
                {
                    push_diagnostic(
                        ...
                        &format!("Execution Strategy assigns Task {task} more than once."),
                    );
                }
            }
                ...
                ExecutionDirectiveKind::Parallel => {
                    ...
                    if directive.declared_worktrees != Some(required_worktrees) {
                        ...
                        push_diagnostic(
                            ...
                            &format!(
                                "Parallel execution groups must declare one isolated worktree per task; Tasks {sorted_tasks:?} declare {declared_worktrees} worktree(s) but require {required_worktrees}."
                            ),
                        );
                    }
                ...
                ExecutionDirectiveKind::Last => {
                        ...
                        && task != *task_numbers.last().unwrap_or(&task)
                    {
                        ...
                        push_diagnostic(
                            ...
                            &format!(
                                "`Execute Task {task} last` is only valid for the numerically final task in the plan."
                            ),
                        );
                    }
                }
        ...
        for task in &task_numbers {
            if !task_assignment.contains_key(task) {
                push_diagnostic(
                    ...
                    &format!(
                        "Execution Strategy does not assign Task {task} to a serial or parallel execution directive."
                    ),
                );
            }
        }
            ...
            match directive.kind {
                ExecutionDirectiveKind::Serial => {
                    ...
                    let objectively_justified = if sorted_tasks.len() > 1 {
                        sorted_tasks.windows(2).all(|pair| {
                            tasks_share_write_scope(&task_scope_index, pair[0], pair[1])
                        }) || seam_justified
        ...
        ExecutionTopologyAnalysis {
            execution_strategy_present: execution_strategy_present.into(),
            dependency_diagram_present: dependency_diagram_present.into(),
            execution_topology_valid: execution_topology_valid.into(),
            serial_hazards_resolved: serial_hazards_resolved.into(),
            parallel_lane_ownership_valid: parallel_lane_ownership_valid.into(),
            parallel_workspace_isolation_valid: parallel_workspace_isolation_valid.into(),
...
const fn is_dependency_connector(ch: char) -> bool {
    matches!(ch, '|' | '-' | '+' | 'v' | '>')
}
...
fn parse_execution_strategy_directives(
    ...
    while index < lines.len() {
        ...
        if trimmed.starts_with("- After ") {
            if let Some((directive, consumed)) = parse_parallel_directive(&lines, index) {
