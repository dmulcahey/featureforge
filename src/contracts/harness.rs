...
pub fn read_execution_contract(path: impl AsRef<Path>) -> Result<ExecutionContract, DiagnosticError> {
...
fn parse_execution_contract(source: &str) -> Result<ExecutionContract, DiagnosticError> {
    ...
    let source_plan_fingerprint = parse_string_header(
        source,
    ...
    let source_spec_fingerprint = parse_string_header(
        source,
    ...
    let criteria_section = parse_section_between_markers(
        &lines,
        0,
        "**Criteria:**",
        "**Non Goals:**",
    ...
    let non_goals = parse_list_between_markers(
        &lines,
        0,
        "**Non Goals:**",
        "**Verifiers:**",
...
fn parse_evaluation_report(source: &str) -> Result<EvaluationReport, DiagnosticError> {
    ...
    let summary = parse_string_header_after(&lines, recommended_action_idx + 1, "Summary", "summary")?;
    let generated_by = parse_string_header(source, "Generated By", "generated_by")?;
    let generated_at = parse_string_header(source, "Generated At", "generated_at")?;
    let report_fingerprint = parse_string_header(source, "Report Fingerprint", "report_fingerprint")?;
...
fn parse_execution_handoff(source: &str) -> Result<ExecutionHandoff, DiagnosticError> {
    ...
    if next_action.trim().is_empty() {
        return Err(parse_error("ExecutionHandoff is missing a concrete next action."));
    }
    let workspace_notes = parse_string_header(source, "Workspace Notes", "workspace_notes")?;
    let commands_run = parse_list_between_markers(
        &lines,
        0,
        "**Commands Run:**",
        "**Risks:**",
    ...
    let risks = parse_list_between_markers(
        &lines,
        0,
        "**Risks:**",
        "**Generated By:**",
    ...
    blocks
        ...
        .map(|block| {
            Ok(CriterionResult {
                ...
                requirement_ids: parse_list_from_block(block, "Requirement IDs", "requirement_ids")?,
...
fn parse_evaluation_evidence_refs(section: &str) -> Result<Vec<EvaluationEvidenceRef>, DiagnosticError> {
    if section.trim() == "[]" {
        return Ok(Vec::new());
    }
    ...
    blocks
        ...
        .map(|block| {
            Ok(EvaluationEvidenceRef {
                evidence_ref_id: parse_string_from_block(block, "Evidence Ref ID", "evidence_ref_id")?,
...
fn parse_blocks(section: &str, block_prefix: &str, field_name: &str) -> Result<Vec<String>, DiagnosticError> {
...
fn parse_string_header(source: &str, header: &str, field_name: &str) -> Result<String, DiagnosticError> {
    parse_header_value(
...
fn parse_list_between_markers(
    ...
    let section = parse_section_between_markers(
        lines,
        start_search,
        start_marker,
        end_marker,
...
fn parse_string_from_block(block: &str, header: &str, field_name: &str) -> Result<String, DiagnosticError> {
    parse_header_value(&block.lines().collect::<Vec<_>>(), 0, header, field_name)
}

fn parse_list_from_block(block: &str, header: &str, field_name: &str) -> Result<Vec<String>, DiagnosticError> {
