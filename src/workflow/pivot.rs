...
const GENERATED_BY: &str = "featureforge:workflow-pivot-record";
...
pub(crate) fn pivot_decision_reason_codes(
    ...
    if follow_up_requires_pivot {
        reason_codes.push(String::from("follow_up_override_repair_review_state"));
    }
...
fn workflow_pivot_record_matches_decision(
    ...
    headers.get("Source Plan") == Some(&format!("`{}`", identity.plan_path))
        ...
        && headers
            .get("Generated By")
            .is_some_and(|generated_by| workflow_pivot_generated_by_matches(generated_by))
}

fn workflow_pivot_generated_by_matches(generated_by: &str) -> bool {
    generated_by == GENERATED_BY || legacy_workflow_pivot_generated_by_matches(generated_by)
}

fn legacy_workflow_pivot_generated_by_matches(generated_by: &str) -> bool {
    let Some(action) = generated_by.strip_prefix("featureforge:workflow-") else {
        return false;
    };
    let Some(action) = action.strip_suffix("-pivot") else {
        return false;
    };
    action == "record"
}
