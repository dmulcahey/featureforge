...
When an approved plan path is provided for workflow-routed final review, include review-result metadata for the controller to bind to runtime-owned state. The metadata must bind to the exact review target without lossy translation. Include:
...
Do not create, repair, search for, or reference runtime receipt files. The controller/runtime owns any state binding after the review result is returned.
